"use strict";
exports.__esModule = true;

var util_1 = require("../util");
var items_1 = require("../items");
var result_1 = require("../result");
var util_2 = require("./util");
function calculateSMSSSV(gen, attacker, defender, move, field) {
    var _a;
    (0, util_2.checkAirLock)(attacker, field);
    (0, util_2.checkAirLock)(defender, field);
    (0, util_2.checkTeraformZero)(attacker, field);
    (0, util_2.checkTeraformZero)(defender, field);
    (0, util_2.checkForecast)(attacker, field.weather);
    (0, util_2.checkForecast)(defender, field.weather);
    (0, util_2.checkItem)(attacker, field.isMagicRoom);
    (0, util_2.checkItem)(defender, field.isMagicRoom);
    (0, util_2.checkWonderRoom)(attacker, field.isWonderRoom);
    (0, util_2.checkWonderRoom)(defender, field.isWonderRoom);
    (0, util_2.checkSeedBoost)(attacker, field);
    (0, util_2.checkSeedBoost)(defender, field);
    (0, util_2.checkTypeChange)(attacker, field);
    (0, util_2.checkTypeChange)(defender, field);
    (0, util_2.checkDauntlessShield)(attacker, gen, field);
    (0, util_2.checkDauntlessShield)(defender, gen, field);
    (0, util_2.checkEmbody)(attacker, gen);
    (0, util_2.checkEmbody)(defender, gen);
    (0, util_2.computeFinalStats)(gen, attacker, defender, field, "def", "spd", "spe");
    (0, util_2.checkIntimidate)(gen, attacker, defender);
    (0, util_2.checkIntimidate)(gen, defender, attacker);
    (0, util_2.checkDownload)(attacker, defender, field);
    (0, util_2.checkDownload)(defender, attacker, field);
    (0, util_2.checkIntrepidSword)(attacker, gen, field);
    (0, util_2.checkIntrepidSword)(defender, gen, field);
    (0, util_2.checkWindRider)(attacker, field.attackerSide);
    (0, util_2.checkWindRider)(defender, field.defenderSide);
    if (move.named("Meteor Beam", "Electro Shot")) {
        attacker.boosts.spa += attacker.hasAbility("Simple")
            ? 2
            : attacker.hasAbility("Contrary")
                ? -1
                : 1;
        attacker.boosts.spa = Math.min(6, Math.max(-6, attacker.boosts.spa));
    }
    (0, util_2.computeFinalStats)(gen, attacker, defender, field, "atk", "spa");
    (0, util_2.checkInfiltrator)(attacker, field.defenderSide);
    (0, util_2.checkInfiltrator)(defender, field.attackerSide);
    var desc = {
        attackerName: attacker.name,
        moveName: move.name,
        defenderName: defender.name,
        isDefenderDynamaxed: defender.isDynamaxed,
        isWonderRoom: field.isWonderRoom
    };
    if (attacker.teraType !== "Stellar" ||
        move.name === "Tera Blast" ||
        move.isStellarFirstUse) {
        desc.isStellarFirstUse =
            attacker.name !== "Terapagos-Stellar" &&
                move.name === "Tera Blast" &&
                attacker.teraType === "Stellar" &&
                move.isStellarFirstUse;
        desc.attackerTera = attacker.teraType;
    }
    if (defender.teraType !== "Stellar")
        desc.defenderTera = defender.teraType;
    if (move.named("Tera Blast") && attacker.teraType) {
        move.category =
            attacker.stats.atk > attacker.stats.spa ? "Physical" : "Special";
    }
    var result = new result_1.Result(gen, attacker, defender, move, field, 0, desc);
    if (move.category === "Status" && !move.named("Nature Power")) {
        return result;
    }
    if (move.flags.punch && attacker.hasItem("Punching Glove")) {
        desc.attackerItem = attacker.item;
        move.flags.contact = 0;
    }
    var breaksProtect = move.breaksProtect ||
        move.isZ ||
        attacker.isDynamaxed ||
        (attacker.hasAbility("Unseen Fist") && move.flags.contact);
    if (field.defenderSide.isProtected && !breaksProtect) {
        desc.isProtected = true;
        return result;
    }
    if (move.name === "Pain Split") {
        var average = Math.floor((attacker.curHP() + defender.curHP()) / 2);
        var damage_1 = Math.max(0, defender.curHP() - average);
        result.damage = damage_1;
        return result;
    }
    var defenderAbilityIgnored = defender.hasAbility("Armor Tail", "Aroma Veil", "Aura Break", "Battle Armor", "Big Pecks", "Bulletproof", "Clear Body", "Contrary", "Damp", "Dazzling", "Disguise", "Dry Skin", "Earth Eater", "Filter", "Flash Fire", "Flower Gift", "Flower Veil", "Fluffy", "Friend Guard", "Fur Coat", "Good as Gold", "Grass Pelt", "Guard Dog", "Heatproof", "Heavy Metal", "Hyper Cutter", "Ice Face", "Ice Scales", "Illuminate", "Immunity", "Inner Focus", "Insomnia", "Keen Eye", "Leaf Guard", "Levitate", "Light Metal", "Lightning Rod", "Limber", "Magic Bounce", "Magma Armor", "Marvel Scale", "Mind's Eye", "Mirror Armor", "Motor Drive", "Multiscale", "Oblivious", "Overcoat", "Own Tempo", "Pastel Veil", "Punk Rock", "Purifying Salt", "Queenly Majesty", "Sand Veil", "Sap Sipper", "Shell Armor", "Shield Dust", "Simple", "Snow Cloak", "Solid Rock", "Soundproof", "Sticky Hold", "Storm Drain", "Sturdy", "Suction Cups", "Sweet Veil", "Tangled Feet", "Telepathy", "Tera Shell", "Thermal Exchange", "Thick Fat", "Unaware", "Vital Spirit", "Volt Absorb", "Water Absorb", "Water Bubble", "Water Veil", "Well-Baked Body", "White Smoke", "Wind Rider", "Wonder Guard", "Wonder Skin");
    var attackerIgnoresAbility = attacker.hasAbility("Mold Breaker", "Teravolt", "Turboblaze");
    var moveIgnoresAbility = move.named("G-Max Drum Solo", "G-Max Fire Ball", "G-Max Hydrosnipe", "Light That Burns the Sky", "Menacing Moonraze Maelstrom", "Moongeist Beam", "Photon Geyser", "Searing Sunraze Smash", "Sunsteel Strike");
    if (defenderAbilityIgnored &&
        (attackerIgnoresAbility || moveIgnoresAbility)) {
        if (attackerIgnoresAbility)
            desc.attackerAbility = attacker.ability;
        if (defender.hasItem("Ability Shield")) {
            desc.defenderItem = defender.item;
        }
        else {
            defender.ability = "";
        }
    }
    var ignoresNeutralizingGas = [
        "As One (Glastrier)",
        "As One (Spectrier)",
        "Battle Bond",
        "Comatose",
        "Disguise",
        "Gulp Missile",
        "Ice Face",
        "Multitype",
        "Neutralizing Gas",
        "Power Construct",
        "RKS System",
        "Schooling",
        "Shields Down",
        "Stance Change",
        "Tera Shift",
        "Zen Mode",
        "Zero to Hero",
    ];
    if (attacker.hasAbility("Neutralizing Gas") &&
        !ignoresNeutralizingGas.includes(defender.ability || "")) {
        desc.attackerAbility = attacker.ability;
        if (defender.hasItem("Ability Shield")) {
            desc.defenderItem = defender.item;
        }
        else {
            defender.ability = "";
        }
    }
    if (defender.hasAbility("Neutralizing Gas") &&
        !ignoresNeutralizingGas.includes(attacker.ability || "")) {
        desc.defenderAbility = defender.ability;
        if (attacker.hasItem("Ability Shield")) {
            desc.attackerItem = attacker.item;
        }
        else {
            attacker.ability = "";
        }
    }
    var isCritical = !defender.hasAbility("Battle Armor", "Shell Armor") &&
        (move.isCrit ||
            (attacker.hasAbility("Merciless") &&
                (defender.hasStatus("psn", "tox") ||
                    field.hasTerrain("Corrosive", "Corrosive Mist", "Murkwater", "Wasteland")))) &&
        move.timesUsed === 1;
    var type = move.type;
    if (move.originalName === "Weather Ball") {
        var holdingUmbrella = attacker.hasItem("Utility Umbrella");
        type =
            field.hasWeather("Sun", "Harsh Sunshine") && !holdingUmbrella
                ? "Fire"
                : field.hasWeather("Rain", "Heavy Rain") && !holdingUmbrella
                    ? "Water"
                    : field.hasWeather("Sand")
                        ? "Rock"
                        : field.hasWeather("Hail", "Snow")
                            ? "Ice"
                            : "Normal";
        desc.weather = field.weather;
        desc.moveType = type;
    }
    else if ((move.named("Judgment") ||
        (move.named("Nature Power") && field.hasTerrain("Holy"))) &&
        attacker.item &&
        attacker.item.includes("Plate")) {
        type = (0, items_1.getItemBoostType)(attacker.item);
    }
    else if (move.originalName === "Techno Blast" &&
        attacker.item &&
        attacker.item.includes("Drive")) {
        type = (0, items_1.getTechnoBlast)(attacker.item);
        desc.moveType = type;
    }
    else if (move.originalName === "Multi-Attack" && attacker.item && attacker.item.includes("Memory")) {
        type = (0, items_1.getMultiAttack)(attacker.item);
        desc.moveType = type;
    }
    else if (move.originalName === "Multi-Attack" && attacker.name.includes('Silvally') && !attacker.item.includes("Memory")) {
        type = attacker.types[0];
        desc.moveType = type;
    }
    else if (move.named("Natural Gift") && ((_a = attacker.item) === null || _a === void 0 ? void 0 : _a.endsWith("Berry"))) {
        var gift = (0, items_1.getNaturalGift)(gen, attacker.item);
        type = gift.t;
        desc.moveType = type;
        desc.attackerItem = attacker.item;
    }
    else if (move.named("Nature Power") ||
        (move.originalName === "Terrain Pulse" && (0, util_2.isGrounded)(attacker, field))) {
        type = field.hasTerrain("Ashen Beach", "Inverse", "Misty", "Psychic")
            ? "Psychic"
            : field.hasTerrain("Big Top WEAK", "Big Top OK", "Big Top NICE", "Big Top POWER", "Big Top 9000")
                ? "Flying"
                : field.hasTerrain("Burning", "Superheated")
                    ? "Fire"
                    : field.hasTerrain("Cave", "Chess Board", "Crystal Fire", "Crystal Water", "Crystal Grass", "Crystal Psychic", "Mountain")
                        ? "Rock"
                        : field.hasTerrain("Corrosive", "Corrosive Mist", "Murkwater", "Wasteland")
                            ? "Poison"
                            : field.hasTerrain("Dark Crystal")
                                ? "Dark"
                                : field.hasTerrain("Desert")
                                    ? "Ground"
                                    : field.hasTerrain("Dragon's Den", "New World")
                                        ? "Dragon"
                                        : field.hasTerrain("Electric", "Short-Circuit 0.5", "Short-Circuit 0.8", "Short-Circuit 1.2", "Short-Circuit 1.5", "Short-Circuit 2")
                                            ? "Electric"
                                            : field.hasTerrain("Factory", "Mirror", "Fairy Tale")
                                                ? "Steel"
                                                : field.hasTerrain("Rocky")
                                                    ? "Fighting"
                                                    : field.hasTerrain("Flower Garden 1", "Flower Garden 2", "Flower Garden 3", "Flower Garden 4", "Flower Garden 5", "Forest", "Grassy")
                                                        ? "Grass"
                                                        : field.hasTerrain("Icy", "Rainbow", "Snowy Mt")
                                                            ? "Ice"
                                                            : field.hasTerrain("Starlight")
                                                                ? "Fairy"
                                                                : field.hasTerrain("Swamp", "Underwater", "Water")
                                                                    ? "Water"
                                                                    : "Normal";
        desc.terrain = field.terrain;
        desc.moveType = type;
        if (move.isMax) {
            desc.moveType = type;
        }
        if (!(move.named("Nature Power") && attacker.hasAbility("Prankster")) &&
            (defender.types.includes("Dark") ||
                (field.hasTerrain("Psychic") && (0, util_2.isGrounded)(defender, field)))) {
            desc.moveType = type;
        }
    }
    else if (move.originalName === "Revelation Dance") {
        if (attacker.teraType) {
            type = attacker.teraType;
        }
        else {
            type = attacker.types[0];
        }
    }
    else if (move.named("Aura Wheel")) {
        if (attacker.named("Morpeko")) {
            type = "Electric";
        }
        else if (attacker.named("Morpeko-Hangry")) {
            type = "Dark";
        }
    }
    else if (move.named("Raging Bull")) {
        if (attacker.named("Tauros-Paldea-Combat")) {
            type = "Fighting";
        }
        else if (attacker.named("Tauros-Paldea-Blaze")) {
            type = "Fire";
        }
        else if (attacker.named("Tauros-Paldea-Aqua")) {
            type = "Water";
        }
        field.defenderSide.isReflect = false;
        field.defenderSide.isLightScreen = false;
        field.defenderSide.isAuroraVeil = false;
    }
    else if (move.named("Ivy Cudgel")) {
        if (attacker.name.includes("Ogerpon-Cornerstone")) {
            type = "Rock";
        }
        else if (attacker.name.includes("Ogerpon-Hearthflame")) {
            type = "Fire";
        }
        else if (attacker.name.includes("Ogerpon-Wellspring")) {
            type = "Water";
        }
    }
    else if (move.named("Tera Starstorm") &&
        attacker.name === "Terapagos-Stellar") {
        move.target = "allAdjacentFoes";
        type = "Stellar";
    }
    else if (move.named("Brick Break", "Psychic Fangs")) {
        field.defenderSide.isReflect = false;
        field.defenderSide.isLightScreen = false;
        field.defenderSide.isAuroraVeil = false;
    }
    var hasAteAbilityTypeChange = false;
    var isAerilate = false;
    var isPixilate = false;
    var isRefrigerate = false;
    var isGalvanize = false;
    var isLiquidVoice = false;
    var isNormalize = false;
    var noTypeChange = move.named("Revelation Dance", "Judgment", "Nature Power", "Techno Blast", "Multi-Attack", "Natural Gift", "Weather Ball", "Terrain Pulse", "Struggle") ||
        (move.named("Tera Blast") && attacker.teraType);
    if (!move.isZ && !noTypeChange) {
        var normal = type === "Normal";
        var glitchConverted = (move.hasType("Fairy", "Steel", "Dark") && field.hasTerrain('Glitch'));
        if ((isAerilate =
            attacker.hasAbility("Aerilate") && (normal || glitchConverted))) {
            type = "Flying";
        }
        else if ((isGalvanize =
            attacker.hasAbility("Galvanize") && (normal || glitchConverted))) {
            type = "Electric";
        }
        else if ((isLiquidVoice =
            attacker.hasAbility("Liquid Voice") && !!move.flags.sound)) {
            if (field.hasTerrain("Icy")) {
                type = "Ice";
            }
            else {
                type = "Water";
            }
        }
        else if ((isPixilate =
            attacker.hasAbility("Pixilate") && (normal || glitchConverted))) {
            type = "Fairy";
        }
        else if ((isRefrigerate =
            attacker.hasAbility("Refrigerate") && (normal || glitchConverted))) {
            type = "Ice";
        }
        else if ((isNormalize = attacker.hasAbility("Normalize"))) {
            type = "Normal";
        }
        if (isGalvanize ||
            isPixilate ||
            isRefrigerate ||
            isAerilate ||
            isNormalize) {
            desc.attackerAbility = attacker.ability;
            hasAteAbilityTypeChange = true;
        }
        else if (isLiquidVoice) {
            desc.attackerAbility = attacker.ability;
        }
    }
    if (move.named("Tera Blast") && attacker.teraType) {
        type = attacker.teraType;
    }
    if (field.hasTerrain("Fairy Tale") &&
        move.named("Cut", "Sacred Sword", "Secret Sword", "Slash")) {
        type = "Steel";
    }
    else if (field.hasTerrain("Murkwater") &&
        move.named("Mud Bomb", "Mud Shot", "Mud-Slap", "Thousand Waves")) {
        type = "Water";
    }
    else if (field.hasTerrain("Starlight") &&
        move.named("Solar Beam", "Solar Blade")) {
        type = "Fairy";
    }
    else if (field.hasTerrain("Glitch") &&
        move.hasType("Dark", "Steel", "Fairy")) {
        type = "Normal";
    }
    else if (field.hasTerrain("Ashen Beach") && move.named("Strength")) {
        type = "Fighting";
    }
    move.type = type;
    if (field.hasTerrain("Glitch")) {
        if (move.hasType("Normal", "Fighting", "Ghost", "Poison", "Bug", "Flying", "Ground", "Rock")) {
            move.category = "Physical";
        }
        else {
            move.category = "Special";
        }
    }
    if ((attacker.hasPiece('K') && field.hasTerrain('Chess Board')) ||
        (attacker.hasAbility("Triage") && move.drain) ||
        (attacker.hasAbility("Gale Wings") &&
            move.hasType("Flying") &&
            attacker.curHP() === attacker.maxHP())) {
        move.priority = 1;
        desc.attackerAbility = attacker.ability;
    }
    var isGhostRevealed = attacker.hasAbility("Scrappy") ||
        attacker.hasAbility("Mind's Eye") ||
        field.defenderSide.isForesight;
    var isRingTarget = defender.hasItem("Ring Target") && !defender.hasAbility("Klutz");
    var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, defender.types[0], isGhostRevealed, field.isGravity, isRingTarget);
    if (field.hasTerrain('Inverse') !== field.isInverseMode) {
        if (type1Effectiveness === 0) {
            type1Effectiveness = 2;
        }
        else if (type1Effectiveness === 0.5) {
            type1Effectiveness = 2;
        }
        else if (type1Effectiveness === 2) {
            type1Effectiveness = 0.5;
        }
        if (move.named('Freeze Dry') && defender.types[0] === 'Water') {
            type1Effectiveness = 2;
        }
    }
    if (type1Effectiveness === 0 && move.hasType('Ground') && field.hasTerrain('Cave')) {
        type1Effectiveness = 1
    }
    if (field.hasWeather("Strong Winds") && defender.types[0] === 'Flying' && type1Effectiveness > 1) {
        type1Effectiveness = 1;
        desc.weather = field.weather;
    }
    if (field.hasTerrain("Dragon's Den") && defender.hasAbility("Multiscale") && defender.types[0] === 'Dragon' && type1Effectiveness > 1) {
        type1Effectiveness = 1;
    }
    if (field.hasTerrain("Flower Garden 4", "Flower Garden 5") && defender.types[0] === "Grass" && type1Effectiveness > 1) {
        type1Effectiveness = 1;
    }
    if (field.hasTerrain('Underwater') && move.hasType('Water') && defender.types[0] === "Water") {
        type1Effectiveness = 1;
    }
    if (field.hasTerrain('Fairy Tale') && move.hasType('Steel') && defender.types[0] === "Dragon") {
        type1Effectiveness = 2;
    }
    if (field.hasTerrain('Glitch')) {
        if (move.hasType('Bug') && defender.types[0] === 'Poison') {
            type1Effectiveness = 2;
        }
        if (move.hasType('Poison') && defender.types[0] === 'Bug') {
            type1Effectiveness = 2;
        }
        if (move.hasType('Ice') && defender.types[0] === 'Fire') {
            type1Effectiveness = 1;
        }
        if (move.hasType('Dragon')) {
            if (defender.types[0] === "Fairy") {
                type1Effectiveness = 1;
            }
            if (defender.types[0] === "Steel") {
                type1Effectiveness = 1;
            }
            if (defender.types[0] === "Dragon") {
                type1Effectiveness = 1;
            }
        }
        if (defender.hasItem("Synthetic Seed")) {
            type1Effectiveness = 1;
        }
        if (move.hasType('Ghost') && defender.types[0] === 'Psychic') {
            type1Effectiveness = 0;
        }
    }
    if (field.hasTerrain('Holy') && move.hasType('Normal')) {
        if (defender.types[0] === 'Ghost') {
            type1Effectiveness = 2;
        }
        if (defender.types[0] === 'Dark') {
            type1Effectiveness = 2;
        }
    }
    var type2Effectiveness = defender.types[1] ? (0, util_2.getMoveEffectiveness)(gen, move, defender.types[1], isGhostRevealed, field.isGravity, isRingTarget) : 1;
    if (defender.types[1]) {
        if (field.hasTerrain('Inverse') !== field.isInverseMode) {
            if (type2Effectiveness === 0) {
                type2Effectiveness = 2;
            }
            else if (type2Effectiveness === 0.5) {
                type2Effectiveness = 2;
            }
            else if (type2Effectiveness === 2) {
                type2Effectiveness = 0.5;
            }
            if (move.named('Freeze Dry') && defender.types[1] === 'Water') {
                type2Effectiveness = 2;
            }
        }
        if (type2Effectiveness === 0 && move.hasType('Ground') && field.hasTerrain('Cave')) {
            type2Effectiveness = 1
        }
        if (field.hasWeather("Strong Winds") && defender.types[1] === 'Flying' && type2Effectiveness > 1) {
            type2Effectiveness = 1;
            desc.weather = field.weather;
        }
        if (field.hasTerrain("Dragon's Den") && defender.hasAbility("Multiscale") && defender.types[1] === 'Dragon' && type2Effectiveness > 1) {
            type2Effectiveness = 1;
        }
        if (field.hasTerrain("Flower Garden 4", "Flower Garden 5") && defender.types[1] === "Grass" && type2Effectiveness > 1) {
            type2Effectiveness = 1;
        }
        if (field.hasTerrain('Underwater') && move.hasType('Water') && defender.types[1] === "Water") {
            type2Effectiveness = 1;
        }
        if (field.hasTerrain('Fairy Tale') && move.hasType('Steel') && defender.types[1] === "Dragon") {
            type2Effectiveness = 2;
        }
        if (field.hasTerrain('Glitch')) {
            if (move.hasType('Bug') && defender.types[1] === 'Poison') {
                type2Effectiveness = 2;
            }
            if (move.hasType('Poison') && defender.types[1] === 'Bug') {
                type2Effectiveness = 2;
            }
            if (move.hasType('Ice') && defender.types[1] === 'Fire') {
                type2Effectiveness = 1;
            }
            if (move.hasType('Dragon')) {
                if (defender.types[1] === "Fairy") {
                    type2Effectiveness = 1;
                }
                if (defender.types[1] === "Steel") {
                    type2Effectiveness = 1;
                }
                if (defender.types[1] === "Dragon") {
                    type2Effectiveness = 1;
                }
            }
            if (defender.hasItem("Synthetic Seed")) {
                type2Effectiveness = 1;
            }
            if (move.hasType('Ghost') && defender.types[1] === 'Psychic') {
                type2Effectiveness = 0;
            }
        }
        if (field.hasTerrain('Holy') && move.hasType('Normal')) {
            if (defender.types[1] === 'Ghost') {
                type2Effectiveness = 2;
            }
            if (defender.types[1] === 'Dark') {
                type2Effectiveness = 2;
            }
        }
    }
    var typeEffectiveness = type1Effectiveness * type2Effectiveness;
    if (defender.teraType && defender.teraType !== "Stellar") {
        typeEffectiveness = (0, util_2.getMoveEffectiveness)(gen, move, defender.teraType, isGhostRevealed, field.isGravity, isRingTarget);
    }
    if (field.hasTerrain('Forest','Flower Garden 2','Flower Garden 3','Flower Garden 4','Flower Garden 5') && move.named('Cut')) {
        if (defender.hasType('Grass')) {
            typeEffectiveness *= 2;
        }
    }
    if (typeEffectiveness === 0 &&
        move.hasType("Ground") &&
        defender.hasItem("Iron Ball") &&
        !defender.hasAbility("Klutz")) {
        typeEffectiveness = 1;
    }
    if ((typeEffectiveness === 0 && move.named("Thousand Arrows"))) {
        typeEffectiveness = 1;
    }
    if ((move.named("Sky Drop") &&
        (defender.hasType("Flying") ||
            defender.weightkg >= 200 ||
            field.isGravity)) ||
        (move.named("Synchronoise") &&
            !defender.hasType(attacker.types[0]) &&
            (!attacker.types[1] || !defender.hasType(attacker.types[1]))) ||
        (move.named("Dream Eater") &&
            !(defender.hasStatus("slp") || defender.hasAbility("Comatose"))) ||
        (move.named("Steel Roller") && !field.terrain) ||
        (move.named("Poltergeist") &&
            (!defender.item || (0, util_2.isQPActive)(defender, field)))) {
        return result;
    }
    if ((field.hasWeather("Harsh Sunshine") && move.hasType("Water")) ||
        (field.hasWeather("Heavy Rain") && move.hasType("Fire"))) {
        desc.weather = field.weather;
        return result;
        }
    if (move.type === "Stellar") {
        desc.defenderTera = defender.teraType;
        typeEffectiveness = !defender.teraType ? 1 : 2;
    }
    var moveType = typeEffectiveness;
    var addedType;
    var laterCalc = false;
    switch (field.terrain) {        
        case "Ashen Beach":
            addedType = gen.types.get("psychic");
            if (move.named("Strength")) {
                laterCalc = true;
            }
            break;
        case "Burning":
        case "New World":
        case "Dragon's Den":
            addedType = gen.types.get("fire");
            if (move.named("Doom Desire") && field.hasTerrain("New World")) {
                moveType =
                    laterCalc = true;
                if (defender.hasAbility("Flash Fire") ||
                    field.hasWeather("Heavy Rain")) {
                    moveType *= 0;
                }
            }
            else if (move.named("Smack Down", "Thousand Arrows", "Clear Smog", "Smog") &&
                field.hasTerrain("Burning")) {
                if (move.named("Thousand Arrows") && moveType === 1) {
                    laterCalc = true;
                }
                else {
                    laterCalc = true;
                }
                if (defender.hasAbility("Flash Fire") || field.hasWeather("Heavy Rain")) {
                    moveType *= 0;
                }
            }
            else if (move.named("Smack Down", "Thousand Arrows", "Continental Crush", "Tectonic Rage") &&
                field.hasTerrain("Dragon's Den")) {
                if (move.named("Thousand Arrows") && moveType === 1) {
                    laterCalc = true;
                }
                else {
                    laterCalc = true;
                }
                if (defender.hasAbility("Flash Fire", "Magma Armor") ||
                    field.hasWeather("Heavy Rain")) {
                    moveType *= 0;
                }
            }
            break;
        case "Corrosive":
        case "Wasteland":
        case "Corrosive Mist":
            addedType = gen.types.get("poison");
            if ((move.named("Mud Bomb", "Mud Shot", "Mud-Slap", "Muddy Water", "Smack Down", "Thousand Arrows", "Whirlpool") || move.hasType("Grass")) && field.hasTerrain("Corrosive")) {
                if (move.named("Thousand Arrows") && moveType === 1) {
                    laterCalc = true;
                }
                else {
                    laterCalc = true;
                }
            }
            else if (move.named("Mud Bomb", "Mud-Slap", "Mud Shot") && field.hasTerrain("Wasteland")) {
                laterCalc = true;
            }
            if ((move.named("Bubble", "Bubble Beam", "Sparkling Aria", "Energy Ball") ||
                (move.hasType("Flying") && move.category === "Special")) &&
                field.hasTerrain("Corrosive Mist")) {
                laterCalc = true;
            }
            break;
        case "Chess Board":
        case "Rocky":
            addedType = gen.types.get("rock");
            if (move.named("Ancient Power", "Nature Power", "Psychic", "Secret Power", "Strength", "Continental Crush", "Shattered Psyche") &&
                field.hasTerrain("Chess Board")) {
                laterCalc = true;
            }
            else if (move.named("Bulldoze", "Earthquake", "Magnitude", "Rock Climb", "Strength") &&
                field.hasTerrain("Rocky")) {
                laterCalc = true;
            }
            break;
        case "Crystal Fire":
        case "Crystal Water":
        case "Crystal Grass":
        case "Crystal Psychic":
            if (field.hasTerrain("Crystal Fire") &&
                (move.named("Judgment", "Multi-Attack", "Rock Climb", "Strength", "Prismatic Laser") ||
                    move.hasType("Rock"))) {
                addedType = gen.types.get("fire");
                laterCalc = true;
                if (defender.hasAbility("Flash Fire") ||
                    field.hasWeather("Heavy Rain")) {
                    moveType *= 0;
                }
            }
            else if (field.hasTerrain("Crystal Water") &&
                (move.named("Judgment", "Multi-Attack", "Rock Climb", "Strength", "Prismatic Laser") ||
                    move.hasType("Rock"))) {
                addedType = gen.types.get("water");
                laterCalc = true;
                if (defender.hasAbility("Water Absorb", "Dry Skin", "Storm Drain") ||
                    field.hasWeather("Harsh Sunshine")) {
                    moveType *= 0;
                }
            }
            else if (field.hasTerrain("Crystal Grass") &&
                (move.named("Judgment", "Multi-Attack", "Rock Climb", "Strength", "Prismatic Laser") ||
                    move.hasType("Rock"))) {
                addedType = gen.types.get("grass");
                laterCalc = true;
                if (defender.hasAbility("Sap Sipper")) {
                    moveType *= 0;
                }
            }
            else if (field.hasTerrain("Crystal Psychic") &&
                (move.named("Judgment", "Multi-Attack", "Rock Climb", "Strength", "Prismatic Laser") ||
                    move.hasType("Rock"))) {
                addedType = gen.types.get("psychic");
                laterCalc = true;
            }
            break;
        case "Electric":
            addedType = gen.types.get("electric");
            if (move.named("Explosion", "Hurricane", "Muddy Water", "Self-Destruct", "Smack Down", "Surf", "Thousand Arrows", "Hydro Vortex")) {
                if (move.named("Thousand Arrows") && moveType === 1) {
                    laterCalc = true;
                }
                else {
                    laterCalc = true;
                }
                if (defender.hasAbility("Volt Absorb", "Lightning Rod", "Motor Drive")) {
                    moveType *= 0;
                }
            }
            break;
        case "Fairy Tale":
            addedType = gen.types.get("dragon");
            if (move.hasType("Fire")) {
                laterCalc = true;
            }
            break;
        case "Icy":
        case "Snowy Mt":
            addedType = gen.types.get("ice");
            if (move.hasType("Rock")) {
                laterCalc = true;
            }
            break;
        case "Murkwater":
            if (move.named("Sludge Wave", "Nature Power")) {
                addedType = gen.types.get("water");
                laterCalc = true;
                if (defender.hasAbility("Water Absorb", "Dry Skin", "Storm Drain") ||
                    field.hasWeather("Harsh Sunshine")) {
                    moveType *= 0;
                }
            }
            else if (move.hasType("Water") || move.named("Smack Down")) {
                addedType = gen.types.get("poison");
                laterCalc = true;
            }
            break;
        case "Short-Circuit 0.5":
        case "Short-Circuit 0.8":
        case "Short-Circuit 1.2":
        case "Short-Circuit 1.5":
        case "Short-Circuit 2":
            if (move.named("Flash Cannon", "Gear Grind", "Gyro Ball", "Magnet Bomb", "Muddy Water", "Surf") ||
                (attacker.hasAbility("Steelworker") && move.hasType("Steel"))) {
                addedType = gen.types.get("electric");
                laterCalc = true;
                if (defender.hasAbility("Volt Absorb", "Lightning Rod", "Motor Drive")) {
                    moveType *= 0;
                }
            }
            break;
        case "Starlight":
            if (move.hasType("Dark")) {
                addedType = gen.types.get("fairy");
                laterCalc = true;
            }
            else if (move.named("Doom Desire")) {
                addedType = gen.types.get("fire");
                laterCalc = true;
                if (defender.hasAbility("Flash Fire") ||
                    field.hasWeather("Heavy Rain")) {
                    moveType *= 0;
                }
            }
            break;
        case "New World":
            if (move.named("Doom Desire")) {
                addedType = gen.types.get("fire");
                laterCalc = true;
                if (defender.hasAbility("Flash Fire") ||
                    field.hasWeather("Heavy Rain")) {
                    moveType *= 0;
                }
            }
            break;
        case "Swamp":
        case "Underwater":
            addedType = gen.types.get("water");
            if (move.named("Smack Down", "Thousand Arrows") &&
                field.hasTerrain("Swamp")) {
                if (move.named("Thousand Arrows") && moveType === 1) {
                    laterCalc = true;
                }
                else {
                    laterCalc = true;
                }
                if (defender.hasAbility("Water Absorb", "Dry Skin", "Storm Drain") ||
                    field.hasWeather("Harsh Sunshine")) {
                    moveType *= 0;
                }
            }
            else if (move.hasType("Ground") && field.hasTerrain("Underwater")) {
                laterCalc = true;
                if (defender.hasAbility("Water Absorb", "Dry Skin", "Storm Drain") ||
                    field.hasWeather("Harsh Sunshine")) {
                    moveType *= 0;
                }
            }
            
            break;
        default:
            typeEffectiveness = typeEffectiveness;
    }
    if (laterCalc === true) {
        var typeAdd1Effectiveness = addedType.effectiveness[defender.types[0]];
        if (field.hasTerrain('Inverse') !== field.isInverseMode) {
            if (typeAdd1Effectiveness === 0) {
                typeAdd1Effectiveness = 2;
            }
            else if (typeAdd1Effectiveness === 0.5) {
                typeAdd1Effectiveness = 2;
            }
            else if (typeAdd1Effectiveness === 2) {
                typeAdd1Effectiveness = 0.5;
            }
            if (move.named('Freeze Dry') && defender.types[0] === 'Water') {
                typeAdd1Effectiveness = 2;
            }
        }
        if (typeAdd1Effectiveness === 0 && move.hasType('Ground') && field.hasTerrain('Cave')) {
            typeAdd1Effectiveness = 1
        }
        if (field.hasWeather("Strong Winds") && defender.types[0] === 'Flying' && typeAdd1Effectiveness > 1) {
            typeAdd1Effectiveness = 1;
            desc.weather = field.weather;
        }
        if (field.hasTerrain("Dragon's Den") && defender.hasAbility("Multiscale") && defender.types[0] === 'Dragon' && typeAdd1Effectiveness > 1) {
            typeAdd1Effectiveness = 1;
        }
        if (field.hasTerrain("Flower Garden 4", "Flower Garden 5") && defender.types[0] === "Grass" && typeAdd1Effectiveness > 1) {
            typeAdd1Effectiveness = 1;
        }
        if (field.hasTerrain('Underwater') && move.hasType('Water') && defender.types[0] === "Water") {
            typeAdd1Effectiveness = 1;
        }
        if (field.hasTerrain('Fairy Tale') && move.hasType('Steel') && defender.types[0] === "Dragon") {
            typeAdd1Effectiveness = 2;
        }
        if (field.hasTerrain('Glitch')) {
            if (move.hasType('Bug') && defender.types[0] === 'Poison') {
                typeAdd1Effectiveness = 2;
            }
            if (move.hasType('Poison') && defender.types[0] === 'Bug') {
                typeAdd1Effectiveness = 2;
            }
            if (move.hasType('Ice') && defender.types[0] === 'Fire') {
                typeAdd1Effectiveness = 1;
            }
            if (move.hasType('Dragon')) {
                if (defender.types[0] === "Fairy") {
                    typeAdd1Effectiveness = 1;
                }
                if (defender.types[0] === "Steel") {
                    typeAdd1Effectiveness = 1;
                }
                if (defender.types[0] === "Dragon") {
                    typeAdd1Effectiveness = 1;
                }
            }
            if (defender.hasItem("Synthetic Seed")) {
                typeAdd1Effectiveness = 1;
            }
            if (move.hasType('Ghost') && defender.types[0] === 'Psychic') {
                typeAdd1Effectiveness = 0;
            }
        }
        if (field.hasTerrain('Holy') && move.hasType('Normal')) {
            if (defender.types[0] === 'Ghost') {
                typeAdd1Effectiveness = 2;
            }
            if (defender.types[0] === 'Dark') {
                typeAdd1Effectiveness = 2;
            }
        }
        var typeAdd2Effectiveness = defender.types[1] ? addedType.effectiveness[defender.types[1]] : 1;
        if (defender.types[1]) {
            if (field.hasTerrain('Inverse') !== field.isInverseMode) {
                if (typeAdd2Effectiveness === 0) {
                    typeAdd2Effectiveness = 2;
                }
                else if (typeAdd2Effectiveness === 0.5) {
                    typeAdd2Effectiveness = 2;
                }
                else if (typeAdd2Effectiveness === 2) {
                    typeAdd2Effectiveness = 0.5;
                }
                if (move.named('Freeze Dry') && defender.types[1] === 'Water') {
                    typeAdd2Effectiveness = 2;
                }
            }
            if (typeAdd2Effectiveness === 0 && move.hasType('Ground') && field.hasTerrain('Cave')) {
                typeAdd2Effectiveness = 1
            }
            if (field.hasWeather("Strong Winds") && defender.types[1] === 'Flying' && typeAdd2Effectiveness > 1) {
                typeAdd2Effectiveness = 1;
                desc.weather = field.weather;
            }
            if (field.hasTerrain("Dragon's Den") && defender.hasAbility("Multiscale") && defender.types[1] === 'Dragon' && typeAdd2Effectiveness > 1) {
                typeAdd2Effectiveness = 1;
            }
            if (field.hasTerrain("Flower Garden 4", "Flower Garden 5") && defender.types[1] === "Grass" && typeAdd2Effectiveness > 1) {
                typeAdd2Effectiveness = 1;
            }
            if (field.hasTerrain('Underwater') && move.hasType('Water') && defender.types[1] === "Water") {
                typeAdd2Effectiveness = 1;
            }
            if (field.hasTerrain('Fairy Tale') && move.hasType('Steel') && defender.types[1] === "Dragon") {
                typeAdd2Effectiveness = 2;
            }
            if (field.hasTerrain('Glitch')) {
                if (move.hasType('Bug') && defender.types[1] === 'Poison') {
                    typeAdd2Effectiveness = 2;
                }
                if (move.hasType('Poison') && defender.types[1] === 'Bug') {
                    typeAdd2Effectiveness = 2;
                }
                if (move.hasType('Ice') && defender.types[1] === 'Fire') {
                    typeAdd2Effectiveness = 1;
                }
                if (move.hasType('Dragon')) {
                    if (defender.types[1] === "Fairy") {
                        typeAdd2Effectiveness = 1;
                    }
                    if (defender.types[1] === "Steel") {
                        typeAdd2Effectiveness = 1;
                    }
                    if (defender.types[1] === "Dragon") {
                        typeAdd2Effectiveness = 1;
                    }
                }
                if (defender.hasItem("Synthetic Seed")) {
                    typeAdd2Effectiveness = 1;
                }
                if (move.hasType('Ghost') && defender.types[1] === 'Psychic') {
                    typeAdd2Effectiveness = 0;
                }
            }
            if (field.hasTerrain('Holy') && move.hasType('Normal')) {
                if (defender.types[1] === 'Ghost') {
                    typeAdd2Effectiveness = 2;
                }
                if (defender.types[1] === 'Dark') {
                    typeAdd2Effectiveness = 2;
                }
            }
        }
        var typeAddEffectiveness = typeAdd1Effectiveness * typeAdd2Effectiveness;
        typeEffectiveness = moveType * typeAddEffectiveness
    }
    var turn2typeEffectiveness = typeEffectiveness;
    if (defender.hasAbility("Tera Shell") &&
        defender.curHP() === defender.maxHP() &&
        ((!field.defenderSide.isSR &&
            (!field.defenderSide.spikes || defender.hasType("Flying"))) ||
            defender.hasItem("Heavy-Duty Boots"))) {
        typeEffectiveness = 0.5;
        desc.defenderAbility = defender.ability;
    }
    if ((defender.hasAbility("Wonder Guard") && typeEffectiveness <= 1) ||
        (move.hasType("Grass") && defender.hasAbility("Sap Sipper")) ||
        (move.hasType("Fire") && defender.hasAbility("Flash Fire", "Well-Baked Body")) ||
        (move.hasType("Water") && defender.hasAbility("Dry Skin", "Storm Drain", "Water Absorb")) ||
        (move.hasType("Electric") && defender.hasAbility("Lightning Rod", "Motor Drive", "Volt Absorb")) ||
        (move.hasType("Ground") && !field.isGravity && !move.named("Thousand Arrows") && !field.hasTerrain("Cave") && !defender.hasItem("Iron Ball") && defender.hasAbility("Levitate")) ||
        (move.flags.bullet && defender.hasAbility("Bulletproof")) ||
        (move.flags.sound && !move.named("Clangorous Soul") && defender.hasAbility("Soundproof")) ||
        (move.priority > 0 && defender.hasAbility("Queenly Majesty", "Dazzling", "Armor Tail")) ||
        (move.hasType("Ground") && defender.hasAbility("Earth Eater")) ||
        (move.flags.wind && defender.hasAbility("Wind Rider")) ||
        (move.hasType('Fire') && defender.hasAbility('Magma Armor') && field.hasTerrain('Dragon\'s Den')) ||
        (move.hasType('Rock') && field.hasTerrain('Crystal Grass') && defender.hasAbility("Sap Sipper")) ||
        ((move.hasType('Rock')) && field.hasTerrain('Crystal Fire') && defender.hasAbility("Flash Fire")) ||
        (move.hasType('Rock') && field.hasTerrain('Crystal Water') && (defender.hasAbility("Water Absorb") || defender.hasAbility('Storm Drain'))) ||
        (move.hasType('Rock')) && field.hasTerrain('Crystal Psychic') && defender.hasType('Dark') ||
        (move.flags.bullet && (defender.hasAbility('Bulletproof') || (defender.boosts.def > 0 && field.hasTerrain('Rocky'))))) {
        desc.defenderAbility = defender.ability;
        return result;
    }
    if (move.hasType("Ground") && !move.named("Thousand Arrows") && !field.hasTerrain("Cave") && !field.isGravity && defender.hasItem("Air Balloon")) {
        desc.defenderItem = defender.item;
        return result;
    }
    if (move.hasType('Ground') && !move.named('Thousand Arrows') && !field.isGravity && defender.hasItem('Synthetic Seed') && field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2')) {
        desc.defenderItem = defender.item;
        return result;
    }
    if (move.priority > 0 && field.hasTerrain("Psychic") && (0, util_2.isGrounded)(defender, field)) {
        desc.terrain = field.terrain;
        return result;
    }
    var weightBasedMove = move.named("Heat Crash", "Heavy Slam", "Low Kick", "Grass Knot");
    if (defender.isDynamaxed && weightBasedMove) {
        return result;
    }
    desc.HPEVs = (0, util_2.getStatDescriptionText)(gen, defender, "hp");
    var fixedDamage = (0, util_2.handleFixedDamageMoves)(attacker, move);
    if (field.hasTerrain('Rainbow') && move.named('Sonic Boom')) {
        var lostHP = 140;
        result.damage = lostHP;
        return result;
    }
    if (fixedDamage) {
        if (attacker.hasAbility("Parental Bond")) {
            result.damage = [fixedDamage, fixedDamage];
            desc.attackerAbility = attacker.ability;
        }
        else {
            result.damage = fixedDamage;
        }
        return result;
    }
    if (move.named("Final Gambit")) {
        result.damage = attacker.curHP();
        return result;
    }
    if (move.named("Guardian of Alola")) {
        var zLostHP = Math.floor((defender.curHP() * 3) / 4);
        if (field.defenderSide.isProtected &&
            attacker.item &&
            attacker.item.includes(" Z")) {
            zLostHP = Math.ceil(zLostHP / 4 - 0.5);
        }
        result.damage = zLostHP;
        return result;
    }
    if (move.named('Nature\'s Madness')) {
        var lostHP = 0;
        if (field.hasTerrain('Grassy', 'Forest', 'Holy', 'New World')) {
            lostHP = field.defenderSide.isProtected ? 0 : Math.floor(defender.curHP() * 0.75);
        }
        else {
            lostHP = field.defenderSide.isProtected ? 0 : Math.floor(defender.curHP() * 0.5);
        }
        result.damage = lostHP;
        return result;
    }
    if (move.named("Spectral Thief")) {
        var stat = void 0;
        for (stat in defender.boosts) {
            if (defender.boosts[stat] > 0) {
                attacker.boosts[stat] += attacker.hasAbility("Contrary")
                    ? -defender.boosts[stat]
                    : defender.boosts[stat];
                if (attacker.boosts[stat] > 6)
                    attacker.boosts[stat] = 6;
                if (attacker.boosts[stat] < -6)
                    attacker.boosts[stat] = -6;
                attacker.stats[stat] = (0, util_2.getModifiedStat)(attacker.rawStats[stat], attacker.boosts[stat]);
                defender.boosts[stat] = 0;
                defender.stats[stat] = defender.rawStats[stat];
            }
        }
    }
    if (move.hits > 1) {
        desc.hits = move.hits;
    }
    if (typeEffectiveness === 0) {
        return result;
    }
    var turnOrder = attacker.stats.spe > defender.stats.spe ? "first" : "last";
    var basePower = calculateBasePowerSMSSSV(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc);
    if (basePower === 0) {
        return result;
    }
    var attack = calculateAttackSMSSSV(gen, attacker, defender, move, field, desc, isCritical);
    var attackSource = move.named('Foul Play') ? defender : attacker;
    if (1 === 1) {
        // Glitch Field + Photon Geyser/LTBTS/Shell Side Arm BULLSHIT OH MY FUCKING GOD
        // Smart Category Attack
        var atkPrelim = attackSource.stats.atk;
        if (attackSource.boosts.atk === 0 || (isCritical && attackSource.boosts.atk < 0)) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else if (defender.hasAbility('Unaware')) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else {
            atkPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.atk, attackSource.boosts.atk);
        }
        if (attacker.hasAbility('Hustle')) {
            atkPrelimMods = (0, util_2.pokeRound)((atkPrelimMods * 3) / 2);
        }
        var atkPrelimMods = [];
        if (attacker.hasAbility('Guts') && attacker.status) {
            atkPrelimMods.push(6144);
        }
        else if (attacker.hasStatus('brn')) {
            atkPrelimMods.push(2048);
        }
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if ((attacker.hasAbility('Slow Start') && attacker.abilityOn) ||
            (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2)) {
            atkPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Gorilla Tactics') && !attacker.isDynamaxed)) {
            atkPrelimMods.push(6144);
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'atk') {
                atkPrelimMods.push(5325);
            }
        }
        if (attacker.hasAbility('Orichalcum Pulse') && field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) {
            atkPrelimMods.push(5461);
        }
        var isTabletsOfRuinActive = (defender.hasAbility('Tablets of Ruin') || field.isTabletsOfRuin) && !attacker.hasAbility('Tablets of Ruin');
        if (isTabletsOfRuinActive) {
            atkPrelimMods.push(3072);
        }
        if (attacker.hasAbility('Huge Power') || (attacker.hasAbility('Pure Power') && !field.hasTerrain('Psychic'))) {
            atkPrelimMods.push(8192);
        }
        if ((attacker.hasItem('Thick Club') && attacker.named('Cubone', 'Marowak', 'Marowak-Alola', 'Marowak-Alola-Totem')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            atkPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax &&
            (attacker.hasItem('Choice Band'))) {
            atkPrelimMods.push(6144);
        }
        if (field.hasTerrain('Underwater') && !move.hasType('Water') && attacker.hasAbility('Steelworker', 'Swift Swim')) {
            atkPrelimMods.push(6144);
        }
        atkPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((atkPrelim * (0, util_2.chainMods)(atkPrelimMods, 0, 524288)) / 4096)));
        var offense = atkPrelim;
        // Smart Category Defense
        var defPrelim = defender.stats.def;
        if (defender.boosts.def === 0 || (isCritical && defender.boosts.def > 0) || move.ignoreDefensive) {
            defPrelim = defender.rawStats.def;
        }
        else if (attacker.hasAbility('Unaware')) {
            defPrelim = defender.rawStats.def;
        }
        else {
            defPrelim = (0, util_2.getModifiedStat)(defender.rawStats.def, defender.boosts.def);
        }
        if (field.hasWeather('Snow') && defender.hasType('Ice')) {
            defPrelim = (0, util_2.pokeRound)((defPrelim * 3) / 2);
        }
        var defPrelimMods = [];
        if ((defender.hasAbility('Marvel Scale') && (defender.status || field.hasTerrain('Misty', 'Rainbow', 'Fairy Tale', "Dragon's Den", 'Starlight'))) ||
            (defender.hasAbility('Grass Pelt') && field.hasTerrain('Grassy', 'Forest'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasAbility('Fur Coat')) {
            defPrelimMods.push(8192);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'def') {
                defPrelimMods.push(5325);
            }
        }
        var isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        var isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && field.isWonderRoom) || (isSwordOfRuinActive && !field.isWonderRoom)) {
            defPrelimMods.push(3072);
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasItem('Metal Powder') && defender.named('Ditto')) {
            defPrelimMods.push(8192);
        }
        if (field.hasTerrain('Glitch') && move.named('Explosion', 'Self-Destruct')) {
            defPrelimMods.push(2048);
        }
        if (field.hasTerrain('Icy', 'Snowy Mt') && defender.hasType('Ice') && field.hasWeather('Hail')) {
            defPrelimMods.push(6144);
        }
        defPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((defPrelim * (0, util_2.chainMods)(defPrelimMods, 0, 524288)) / 4096)));
        var defense = defPrelim;
        // Glitch + Smart Category Spatk
        var spatkPrelim = attackSource.stats.spa;
        if (attackSource.boosts.spa === 0 || (isCritical && attackSource.boosts.spa < 0)) {
            spatkPrelim = attackSource.rawStats.spa;
        }
        else if (defender.hasAbility('Unaware')) {
            spatkPrelim = attackSource.rawStats.spa;
        }
        else {
            spatkPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.spa, attackSource.boosts.spa);
        }
        var spatkPrelimMods = [];
        if (attacker.hasAbility('Plus', 'Minus') && (attacker.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            spatkPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2) {
            spatkPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine'))) {
            spatkPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Pure Power') && field.hasTerrain('Psychic')) {
            spatkPrelimMods.push(8192);
        }
        var isVesselOfRuinActive = (defender.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) && !attacker.hasAbility('Vessel of Ruin');
        if (isVesselOfRuinActive) {
            spatkPrelimMods.push(3072);
        }
        if ((0, isQPActive)(attacker, field)) {
            if ((0, getQPBoostedStat)(attacker) === 'spa') {
                spatkPrelimMods.push(5325);
            }
        }
        if ((attacker.hasAbility('Hadron Engine') && field.hasTerrain('Electric'))) {
            spatkPrelimMods.push(5461);
        }
        if ((attacker.hasItem('Deep Sea Tooth') && attacker.named('Clamperl')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            spatkPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax && attacker.hasItem('Choice Specs')) {
            spatkPrelimMods.push(6144);
        }
        spatkPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spatkPrelim * (0, util_2.chainMods)(spatkPrelimMods, 1, 524288)) / 4096)));
        var spoffense = spatkPrelim;
        // Glitch Field Use Special Defense?
        var spdefAttackerPrelim = attacker.stats.spd;
        if (attacker.boosts.spd === 0 || (isCritical && attacker.boosts.spd > 0) || move.ignoreDefensive) {
            spdefAttackerPrelim = attacker.rawStats.spd;
        }
        else if (defender.hasAbility('Unaware')) {
            spdefAttackerPrelim = attacker.rawStats.spd;
        }
        else {
            spdefAttackerPrelim = (0, util_2.getModifiedStat)(attacker.rawStats.spd, attacker.boosts.spd);
        }
        if (field.hasWeather('Sand') && attacker.hasType('Rock')) {
            spdefAttackerPrelim = (0, util_2.pokeRound)((spdefAttackerPrelim * 3) / 2);
        }
        var spdefAttackerPrelimMods = [];
        if ((field.hasTerrain('Misty') && attacker.hasType('Fairy')) || (field.hasTerrain('Desert') && attacker.hasType('Ground'))) {
            spdefAttackerPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefAttackerPrelimMods.push(6144);
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefAttackerPrelimMods.push(6144);
        }
        isSwordOfRuinActive = (defender.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !attacker.hasAbility('Sword of Ruin');
        isBeadsOfRuinActive = (defender.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !attacker.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && !field.isWonderRoom) || (isSwordOfRuinActive && field.isWonderRoom)) {
            spdefAttackerPrelimMods.push(3072);
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'spd') {
                spdefAttackerPrelimMods.push(5324);
            }
        }
        if ((attacker.hasItem('Eviolite') && (attacker.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(attacker.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
                (attacker.hasItem('Assault Vest')) ||
                (attacker.hasItem('Light Ball', 'Pikanium Z') && attacker.name.includes('Pikachu'))) {
                spdefAttackerPrelimMods.push(6144);
            }
        else if (attacker.hasItem('Deep Sea Scale') && attacker.named('Clamperl')) {
            spdefAttackerPrelimMods.push(8192);
        }
        spdefAttackerPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spdefAttackerPrelim * (0, util_2.chainMods)(spdefAttackerPrelimMods, 1, 524288)) / 4096)));
        // Smart Category Special Defense
        var spdefDefenderPrelim = defender.stats.spd;
        if (defender.boosts.spd === 0 || (isCritical && defender.boosts.spd > 0) || move.ignoreDefensive) {
            spdefDefenderPrelim = defender.rawStats.spd;
        }
        else if (attacker.hasAbility('Unaware')) {
            spdefDefenderPrelim = defender.rawStats.spd;
        }
        else {
            spdefDefenderPrelim = (0, util_2.getModifiedStat)(defender.rawStats.spd, defender.boosts.spd);
        }
        if (field.hasWeather('Sand') && defender.hasType('Rock')) {
            spdefDefenderPrelim = (0, util_2.pokeRound)((spdefDefenderPrelim * 3) / 2);
        }
        var spdefDefenderPrelimMods = [];
        if ((field.hasTerrain('Misty') && defender.hasType('Fairy')) || (field.hasTerrain('Desert') && defender.hasType('Ground'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        if (defender.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !defender.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        if (field.defenderSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && !field.isWonderRoom) || (isSwordOfRuinActive && field.isWonderRoom)) {
            spdefDefenderPrelimMods.push(3072);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'spd') {
                spdefDefenderPrelimMods.push(5324);
            }
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (defender.hasItem('Assault Vest')) ||
            (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        else if (defender.hasItem('Deep Sea Scale') && defender.named('Clamperl')) {
            spdefDefenderPrelimMods.push(8192);
        }
        spdefDefenderPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spdefDefenderPrelim * (0, util_2.chainMods)(spdefDefenderPrelimMods, 1, 524288)) / 4096)));
        var spdefense = spdefDefenderPrelim;
    }
    if (move.named("Shell Side Arm") &&
        (0, util_2.getShellSideArmCategory)(offense, defense, spoffense, spdefense) === "Physical") {
        move.flags.contact = 1;
    }
    // Glitch Field Special Stat
    var attackerUseSpdef = false;
    if (field.hasTerrain('Glitch') && move.category === "Special") {
        attackerUseSpdef = spdefAttackerPrelim > spatkPrelim;
    }
    // ACTUAL FUCKING CODE
    var attackStat = 'atk';    
    move.named("Shell Side Arm") && (0, util_2.getShellSideArmCategory)(offense, defense, spoffense, spdefense) === "Physical" ? attackStat = "atk"
        : move.named('Photon Geyser', 'Light That Burns the Sky') && util_2.getPhotonGeyserCategory(offense, spoffense) === "Physical" ? attackStat = "atk"
            : move.named("Body Press") ? attackStat = "def"
                : attackerUseSpdef === true ? attackStat = 'spd'
                    : move.category === "Special" ? attackStat = "spa"
                        : attackStat = "atk";
    var defense = calculateDefenseSMSSSV(gen, attacker, defender, move, field, desc, isCritical);
    if (1 === 1) {
        // Glitch Field + Photon Geyser/LTBTS/Shell Side Arm BULLSHIT OH MY FUCKING GOD
        // Smart Category Attack
        var atkPrelim = attackSource.stats.atk;
        if (attackSource.boosts.atk === 0 || (isCritical && attackSource.boosts.atk < 0)) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else if (defender.hasAbility('Unaware')) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else {
            atkPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.atk, attackSource.boosts.atk);
        }
        if (attacker.hasAbility('Hustle')) {
            atkPrelimMods = (0, util_2.pokeRound)((atkPrelimMods * 3) / 2);
        }
        var atkPrelimMods = [];
        if (attacker.hasAbility('Guts') && attacker.status) {
            atkPrelimMods.push(6144);
        }
        else if (attacker.hasStatus('brn')) {
            atkPrelimMods.push(2048);
        }
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if ((attacker.hasAbility('Slow Start') && attacker.abilityOn) ||
            (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2)) {
            atkPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Gorilla Tactics') && !attacker.isDynamaxed)) {
            atkPrelimMods.push(6144);
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'atk') {
                atkPrelimMods.push(5325);
            }
        }
        if (attacker.hasAbility('Orichalcum Pulse') && field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) {
            atkPrelimMods.push(5461);
        }
        var isTabletsOfRuinActive = (defender.hasAbility('Tablets of Ruin') || field.isTabletsOfRuin) && !attacker.hasAbility('Tablets of Ruin');
        if (isTabletsOfRuinActive) {
            atkPrelimMods.push(3072);
        }
        if (attacker.hasAbility('Huge Power') || (attacker.hasAbility('Pure Power') && !field.hasTerrain('Psychic'))) {
            atkPrelimMods.push(8192);
        }
        if ((attacker.hasItem('Thick Club') && attacker.named('Cubone', 'Marowak', 'Marowak-Alola', 'Marowak-Alola-Totem')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            atkPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax &&
            (attacker.hasItem('Choice Band'))) {
            atkPrelimMods.push(6144);
        }
        if (field.hasTerrain('Underwater') && !move.hasType('Water') && attacker.hasAbility('Steelworker', 'Swift Swim')) {
            atkPrelimMods.push(6144);
        }
        atkPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((atkPrelim * (0, util_2.chainMods)(atkPrelimMods, 0, 524288)) / 4096)));
        var offense = atkPrelim;
        // Smart Category Defense
        var defPrelim = defender.stats.def;
        if (defender.boosts.def === 0 || (isCritical && defender.boosts.def > 0) || move.ignoreDefensive) {
            defPrelim = defender.rawStats.def;
        }
        else if (attacker.hasAbility('Unaware')) {
            defPrelim = defender.rawStats.def;
        }
        else {
            defPrelim = (0, util_2.getModifiedStat)(defender.rawStats.def, defender.boosts.def);
        }
        if (field.hasWeather('Snow') && defender.hasType('Ice')) {
            defPrelim = (0, util_2.pokeRound)((defPrelim * 3) / 2);
        }
        var defPrelimMods = [];
        if ((defender.hasAbility('Marvel Scale') && (defender.status || field.hasTerrain('Misty', 'Rainbow', 'Fairy Tale', "Dragon's Den", 'Starlight'))) ||
            (defender.hasAbility('Grass Pelt') && field.hasTerrain('Grassy', 'Forest'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasAbility('Fur Coat')) {
            defPrelimMods.push(8192);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'def') {
                defPrelimMods.push(5325);
            }
        }
        var isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        var isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && field.isWonderRoom) || (isSwordOfRuinActive && !field.isWonderRoom)) {
            defPrelimMods.push(3072);
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasItem('Metal Powder') && defender.named('Ditto')) {
            defPrelimMods.push(8192);
        }
        if (field.hasTerrain('Glitch') && move.named('Explosion', 'Self-Destruct')) {
            defPrelimMods.push(2048);
        }
        if (field.hasTerrain('Icy', 'Snowy Mt') && defender.hasType('Ice') && field.hasWeather('Hail')) {
            defPrelimMods.push(6144);
        }
        defPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((defPrelim * (0, util_2.chainMods)(defPrelimMods, 0, 524288)) / 4096)));
        var smartdefense = defPrelim;
        // Smart Category Spatk
        var spatkAttackerPrelim = attackSource.stats.spa;
        if (attackSource.boosts.spa === 0 || (isCritical && attackSource.boosts.spa < 0)) {
            spatkAttackerPrelim = attackSource.rawStats.spa;
        }
        else if (defender.hasAbility('Unaware')) {
            spatkAttackerPrelim = attackSource.rawStats.spa;
        }
        else {
            spatkAttackerPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.spa, attackSource.boosts.spa);
        }
        var spatkAttackerPrelimMods = [];
        if (attacker.hasAbility('Plus', 'Minus') && (attacker.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            spatkAttackerPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2) {
            spatkAttackerPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine'))) {
            spatkAttackerPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Pure Power') && field.hasTerrain('Psychic')) {
            spatkAttackerPrelimMods.push(8192);
        }
        var isVesselOfRuinActive = (defender.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) && !attacker.hasAbility('Vessel of Ruin');
        if (isVesselOfRuinActive) {
            spatkAttackerPrelimMods.push(3072);
        }
        if ((0, isQPActive)(attacker, field)) {
            if ((0, getQPBoostedStat)(attacker) === 'spa') {
                spatkAttackerPrelimMods.push(5325);
            }
        }
        if ((attacker.hasAbility('Hadron Engine') && field.hasTerrain('Electric'))) {
            spatkAttackerPrelimMods.push(5461);
        }
        if ((attacker.hasItem('Deep Sea Tooth') && attacker.named('Clamperl')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            spatkAttackerPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax && attacker.hasItem('Choice Specs')) {
            spatkAttackerPrelimMods.push(6144);
        }
        spatkAttackerPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spatkAttackerPrelim * (0, util_2.chainMods)(spatkAttackerPrelimMods, 1, 524288)) / 4096)));
        var spoffense = spatkAttackerPrelim;
        // Glitch Defender Use Special Attack?
        var spatkDefenderPrelim = defender.stats.spa;
        if (defender.boosts.spa === 0 || (isCritical && defender.boosts.spa < 0)) {
            spatkDefenderPrelim = defender.rawStats.spa;
        }
        else if (attacker.hasAbility('Unaware')) {
            spatkDefenderPrelim = defender.rawStats.spa;
        }
        else {
            spatkDefenderPrelim = (0, util_2.getModifiedStat)(defender.rawStats.spa, defender.boosts.spa);
        }
        var spatkDefenderPrelimMods = [];
        if (defender.hasAbility('Plus', 'Minus') && (defender.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            spatkDefenderPrelimMods.push(6144);
        }
        if (defender.hasAbility('Defeatist') && defender.curHP() <= defender.maxHP() / 2) {
            spatkDefenderPrelimMods.push(2048);
        }
        if ((defender.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine'))) {
            spatkDefenderPrelimMods.push(6144);
        }
        if (defender.hasAbility('Pure Power') && field.hasTerrain('Psychic')) {
            spatkDefenderPrelimMods.push(8192);
        }
        var isVesselOfRuinActive = (attacker.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) && !defender.hasAbility('Vessel of Ruin');
        if (isVesselOfRuinActive) {
            spatkDefenderPrelimMods.push(3072);
        }
        if ((0, isQPActive)(defender, field)) {
            if ((0, getQPBoostedStat)(defender) === 'spa') {
                spatkDefenderPrelimMods.push(5325);
            }
        }
        if ((defender.hasAbility('Hadron Engine') && field.hasTerrain('Electric'))) {
            spatkDefenderPrelimMods.push(5461);
        }
        if ((defender.hasItem('Deep Sea Tooth') && defender.named('Clamperl')) ||
            (defender.hasItem('Light Ball') && defender.name.includes('Pikachu') && !move.isZ)) {
            spatkDefenderPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax && defender.hasItem('Choice Specs')) {
            spatkDefenderPrelimMods.push(6144);
        }
        spatkDefenderPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spatkDefenderPrelim * (0, util_2.chainMods)(spatkDefenderPrelimMods, 1, 524288)) / 4096)));
        // Glitch + Smart Category Spdef
        var spdefPrelim = defender.stats.spd;
        if (defender.boosts.spd === 0 || (isCritical && defender.boosts.spd > 0) || move.ignoreDefensive) {
            spdefPrelim = defender.rawStats.spd;
        }
        else if (defender.hasAbility('Unaware')) {
            spdefPrelim = defender.rawStats.spd;
        }
        else {
            spdefPrelim = (0, util_2.getModifiedStat)(defender.rawStats.spd, defender.boosts.spd);
        }
        if (field.hasWeather('Sand') && defender.hasType('Rock')) {
            spdefPrelim = (0, util_2.pokeRound)((spdefPrelim * 3) / 2);
        }
        var spdefPrelimMods = [];
        if ((field.hasTerrain('Misty') && defender.hasType('Fairy')) || (field.hasTerrain('Desert') && defender.hasType('Ground'))) {
            spdefPrelimMods.push(6144);
        }
        if (defender.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !defender.hasItem('Utility Umbrella')) 
            || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefPrelimMods.push(6144);
        }
        if (field.defenderSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') 
            || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefPrelimMods.push(6144);
        }
        isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && !field.isWonderRoom) || (isSwordOfRuinActive && field.isWonderRoom)) {
            spdefPrelimMods.push(3072);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'spd') {
                spdefPrelimMods.push(5324);
            }
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
                (defender.hasItem('Assault Vest')) ||
                (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
                spdefPrelimMods.push(6144);
            }
        else if (defender.hasItem('Deep Sea Scale') && defender.named('Clamperl')) {
            spdefPrelimMods.push(8192);
        }
        spdefPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spdefPrelim * (0, util_2.chainMods)(spdefPrelimMods, 1, 524288)) / 4096)));
        var spdefense = spdefPrelim;
    }
    var defenderUseSpatk = false;
    if (field.hasTerrain('Glitch')) {
        defenderUseSpatk = spatkDefenderPrelim > spdefPrelim;
    }
    // ACTUAL FUCKING CODE
    var hitsPhysical = move.overrideDefensiveStat === "def" || move.category === "Physical" || (move.named("Shell Side Arm") && (0, util_2.getShellSideArmCategory)(offense, smartdefense, spoffense, spdefense) === "Physical") || (move.named('Photon Geyser', 'Light That Burns the Sky') && util_2.getPhotonGeyserCategory(offense, spoffense) === "Physical");
    var defenseStat = hitsPhysical ? 'def'
        : !hitsPhysical && field.hasTerrain('Glitch') && defenderUseSpatk ? 'spa'
            : 'spd';
    var baseDamage = calculateBaseDamageSMSSSV(gen, attacker, defender, basePower, attack, defense, move, field, desc, isCritical);
    if (hasTerrainSeed(defender) &&
        field.hasTerrain(defender.item.substring(0, defender.item.indexOf(" "))) &&
        items_1.SEED_BOOSTED_STAT[defender.item] === defenseStat) {
        desc.defenderItem = defender.item;
    }
    var preStellarStabMod = (0, util_2.getStabMod)(attacker, move, desc);
    var stabMod = (0, util_2.getStellarStabMod)(attacker, move, preStellarStabMod);
    var applyBurn = attacker.hasStatus("brn") &&
        move.category === "Physical" &&
        !attacker.hasAbility("Guts") &&
        !move.named("Facade");
    desc.isBurned = applyBurn;
    var finalMods = calculateFinalModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness);
    var protect = false;
    if (field.defenderSide.isProtected &&
        (attacker.isDynamaxed ||
            (move.isZ && attacker.item && attacker.item.includes(" Z")))) {
        protect = true;
        desc.isProtected = true;
    }
    var finalMod = (0, util_2.chainMods)(finalMods, 1, 524288);
    var isSpread = field.gameType !== "Singles" &&
        ["allAdjacent", "allAdjacentFoes"].includes(move.target);
    var childDamage;
    if (attacker.hasAbility("Parental Bond") && move.hits === 1 && !isSpread) {
        var child = attacker.clone();
        child.ability = "Parental Bond (Child)";
        (0, util_2.checkMultihitBoost)(gen, child, defender, move, field, desc);
        childDamage = calculateSMSSSV(gen, child, defender, move, field)
            .damage;
        desc.attackerAbility = attacker.ability;
    }
    var damage = [];
    for (var i = 0; i < 16; i++) {
        damage[i] = (0, util_2.getFinalDamage)(baseDamage, i, typeEffectiveness, applyBurn, stabMod, finalMod, protect);
    }
    desc.attackBoost = move.named("Foul Play")
        ? defender.boosts[attackStat]
        : attacker.boosts[attackStat];
    if (move.timesUsed > 1 || move.hits > 1) {
        var origDefBoost = desc.defenseBoost;
        var origAtkBoost = desc.attackBoost;
        var numAttacks = 1;
        if (move.timesUsed > 1) {
            desc.moveTurns = "over ".concat(move.timesUsed, " turns");
            numAttacks = move.timesUsed;
        }
        else {
            numAttacks = move.hits;
        }
        var usedItems = [false, false];
        var _loop_1 = function (times) {
            usedItems = (0, util_2.checkMultihitBoost)(gen, attacker, defender, move, field, desc, usedItems[0], usedItems[1]);
            var newAttack = calculateAttackSMSSSV(gen, attacker, defender, move, field, desc, isCritical);
            var newDefense = calculateDefenseSMSSSV(gen, attacker, defender, move, field, desc, isCritical);
            hasAteAbilityTypeChange =
                hasAteAbilityTypeChange &&
                    attacker.hasAbility("Aerilate", "Galvanize", "Pixilate", "Refrigerate", "Normalize");
            if (move.timesUsed > 1) {
                preStellarStabMod = (0, util_2.getStabMod)(attacker, move, desc);
                typeEffectiveness = turn2typeEffectiveness;
                stabMod = (0, util_2.getStellarStabMod)(attacker, move, preStellarStabMod, times);
            }
            var newBasePower = calculateBasePowerSMSSSV(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc, times + 1);
            var newBaseDamage = calculateBaseDamageSMSSSV(gen, attacker, defender, newBasePower, newAttack, newDefense, move, field, desc, isCritical);
            var newFinalMods = calculateFinalModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness, times);
            var newFinalMod = (0, util_2.chainMods)(newFinalMods, 41, 524288);
            var damageMultiplier = 0;
            damage = damage.map(function (affectedAmount) {
                var newFinalDamage = (0, util_2.getFinalDamage)(newBaseDamage, damageMultiplier, typeEffectiveness, applyBurn, stabMod, newFinalMod, protect);
                damageMultiplier++;
                return affectedAmount + newFinalDamage;
            });
        };
        for (var times = 1; times < numAttacks; times++) {
            _loop_1(times);
        }
        desc.defenseBoost = origDefBoost;
        desc.attackBoost = origAtkBoost;
    }
    result.damage = childDamage ? [damage, childDamage] : damage;
    if (field.hasTerrain('Corrosive Mist') && move.named('Eruption', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive', 'Explosion', 'Self-Destruct')) {
        var lostHP = 0;
        if (defender.hasAbility('Damp', 'Flash Fire') || field.defenderSide.isProtected) {
            return result;
        }
        else if (defender.hasAbility('Sturdy') && defender.curHP() == defender.maxHP()) {
            lostHP = defender.maxHP();
            result.damage = lostHP - 1;
            return result;
        }
        else {
            lostHP = defender.maxHP();
            result.damage = lostHP;
            return result;
        }
    }
    if (field.hasTerrain('Mirror') && move.named('Boomburst', 'Bulldoze', 'Earthquake', 'Explosion', 'Hyper Voice', 'Magnitude', 'Self-Destruct', 'Tectonic Rage')) {
        var lostHP_1 = 0;
        if (defender.hasAbility('Battle Armor', 'Shell Armor') || field.defenderSide.isProtected) {
            return result;
        }
        else {
            lostHP_1 = Math.floor(defender.maxHP() / 2);
            var finaldmg = damage.map(function (num) { return num + lostHP_1; });
            result.damage = finaldmg;
            return result;
        }
    }
    // Magnitude was here, no longer here. Players can manually change Magnitude Power
    return result;
}
exports.calculateSMSSSV = calculateSMSSSV;
function calculateBasePowerSMSSSV(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc, hit) {
    var _a;
    if (hit === void 0) { hit = 1; }
    var turnOrder = attacker.stats.spe > defender.stats.spe ? 'first' : 'last';
    var basePower;
    switch (move.name) {
        case 'Payback':
            basePower = move.bp * (turnOrder === 'last' ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Bolt Beak':
        case 'Fishious Rend':
            basePower = move.bp * (turnOrder !== 'last' ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Pursuit':
            var switching = field.defenderSide.isSwitching === 'out';
            basePower = move.bp * (switching ? 2 : 1);
            if (switching)
                desc.isSwitching = 'out';
            desc.moveBP = basePower;
            break;
        case 'Electro Ball':
            var r = Math.floor(attacker.stats.spe / defender.stats.spe);
            basePower = r >= 4 ? 150 : r >= 3 ? 120 : r >= 2 ? 80 : r >= 1 ? 60 : 40;
            if (defender.stats.spe === 0)
                basePower = 40;
            desc.moveBP = basePower;
            break;
        case 'Gyro Ball':
            basePower = Math.min(150, Math.floor((25 * defender.stats.spe) / attacker.stats.spe) + 1);
            if (attacker.stats.spe === 0)
                basePower = 1;
            desc.moveBP = basePower;
            break;
        case 'Punishment':
            basePower = Math.min(200, 60 + 20 * (0, util_2.countBoosts)(gen, defender.boosts));
            desc.moveBP = basePower;
            break;
        case 'Low Kick':
        case 'Grass Knot':
            var w = (0, util_2.getWeight)(defender, desc, 'defender');
            basePower = w >= 200 ? 120 : w >= 100 ? 100 : w >= 50 ? 80 : w >= 25 ? 60 : w >= 10 ? 40 : 20;
            desc.moveBP = basePower;
            break;
        case 'Hex':
        case 'Infernal Parade':
            basePower = move.bp * (defender.status || defender.hasAbility('Comatose') ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Barb Barrage':
            basePower = move.bp * (defender.hasStatus('psn', 'tox') ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Heavy Slam':
        case 'Heat Crash':
            var wr = (0, util_2.getWeight)(attacker, desc, 'attacker') /
                (0, util_2.getWeight)(defender, desc, 'defender');
            basePower = wr >= 5 ? 120 : wr >= 4 ? 100 : wr >= 3 ? 80 : wr >= 2 ? 60 : 40;
            desc.moveBP = basePower;
            break;
        case 'Stored Power':
        case 'Power Trip':
            basePower = 20 + 20 * (0, util_2.countBoosts)(gen, attacker.boosts);
            desc.moveBP = basePower;
            break;
        case 'Acrobatics':
            basePower = move.bp * (attacker.hasItem('Flying Gem') ||
                (field.hasTerrain("Big Top WEAK", "Big Top OK", "Big Top NICE", "Big Top POWER", "Big Top 9000") || !attacker.item || (0, util_2.isQPActive)(attacker, field)) ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Assurance':
            basePower = move.bp * (defender.hasAbility('Parental Bond (Child)') ? 2 : 1);
            break;
        case 'Wake-Up Slap':
            basePower = move.bp * (defender.hasStatus('slp') || defender.hasAbility('Comatose') ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Smelling Salts':
            basePower = move.bp * (defender.hasStatus('par') ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Weather Ball':
            basePower = move.bp * (field.weather && !field.hasWeather('Strong Winds') ? 2 : 1);
            if (field.hasWeather('Sun', 'Harsh Sunshine', 'Rain', 'Heavy Rain') &&
                attacker.hasItem('Utility Umbrella'))
                basePower = move.bp;
            desc.moveBP = basePower;
            break;
        case 'Terrain Pulse':
            basePower = move.bp * ((0, util_2.isGrounded)(attacker, field) && field.terrain ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Rising Voltage':
            basePower = move.bp * (((0, util_2.isGrounded)(defender, field) && field.hasTerrain('Electric')) ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Psyblade':
            basePower = move.bp * (field.hasTerrain('Electric') ? 1.5 : 1);
            if (field.hasTerrain('Electric')) {
                desc.moveBP = basePower;
                desc.terrain = field.terrain;
            }
            break;
        case 'Fling':
            basePower = (0, items_1.getFlingPower)(attacker.item);
            desc.moveBP = basePower;
            desc.attackerItem = attacker.item;
            break;
        case 'Dragon Energy':
        case 'Eruption':
        case 'Water Spout':
            basePower = Math.max(1, Math.floor((150 * attacker.curHP()) / attacker.maxHP()));
            desc.moveBP = basePower;
            break;
        case 'Flail':
        case 'Reversal':
            var p = Math.floor((48 * attacker.curHP()) / attacker.maxHP());
            basePower = p <= 1 ? 200 : p <= 4 ? 150 : p <= 9 ? 100 : p <= 16 ? 80 : p <= 32 ? 40 : 20;
            desc.moveBP = basePower;
            break;
        case 'Natural Gift':
            if ((_a = attacker.item) === null || _a === void 0 ? void 0 : _a.endsWith('Berry')) {
                var gift = (0, items_1.getNaturalGift)(gen, attacker.item);
                basePower = gift.p;
                desc.attackerItem = attacker.item;
                desc.moveBP = move.bp;
            }
            else {
                basePower = move.bp;
            }
            break;
        case 'Nature Power':
            move.category = 'Special';
            move.secondaries = true;
            switch (field.terrain) {
                case 'Ashen Beach':
                    basePower = 0;
                    desc.moveName = 'Meditate';
                    break;
                case 'Big Top WEAK':
                case 'Big Top OK':
                case 'Big Top NICE':
                case 'Big Top POWER':
                case 'Big Top 9000':
                    basePower = 110;
                    move.category = 'Physical';
                    desc.moveName = 'Acrobatics';
                    break;
                case 'Burning':
                    basePower = 90;
                    desc.moveName = 'Flamethrower';
                    break;
                case 'Cave':
                    basePower = 60;
                    move.category = 'Physical';
                    desc.moveName = 'Rock Tomb';
                    break;
                case 'Corrosive':
                    basePower = 40;
                    desc.moveName = 'Acid Spray';
                    break;
                case 'Corrosive Mist':
                    basePower = 130;
                    desc.moveName = 'Venoshock';
                    break;
                case 'Crystal Fire':
                case 'Crystal Water':
                case 'Crystal Grass':
                case 'Crystal Psychic':
                    basePower = 80;
                    desc.moveName = 'Power Gem';
                    break;
                case 'Dark Crystal':
                    basePower = 80;
                    desc.moveName = 'Dark Pulse';
                    break;
                case 'Desert':
                    basePower = 35;
                    move.category = 'Physical';
                    desc.moveName = 'Sand Tomb';
                    break;
                case 'Dragon\'s Den':
                    basePower = 85;
                    desc.moveName = 'Dragon Pulse';
                    break;
                case 'Chess Board':
                    basePower = 60;
                    desc.moveName = 'Ancient Power';
                    break;
                case 'Electric':
                    basePower = 90;
                    desc.moveName = 'Thunderbolt';
                    break;
                case 'Factory':
                    basePower = 100;
                    move.category = 'Physical';
                    desc.moveName = 'Gear Grind';
                    break;
                case 'Fairy Tale':
                    basePower = 85;
                    move.overrideDefensiveStat = 'def';
                    desc.moveName = 'Secret Sword';
                    break;
                case 'Flower Garden 1':
                case 'Flower Garden 2':
                case 'Flower Garden 3':
                case 'Flower Garden 4':
                    basePower = 0;
                    desc.moveName = 'Growth';
                    break;
                case 'Flower Garden 5':
                    basePower = 90;
                    move.category = 'Physical';
                    move.target = 'allAdjacent';
                    desc.moveName = 'Petal Blizzard';
                    break;
                case 'Forest':
                    basePower = 120;
                    move.category = 'Physical';
                    desc.moveName = 'Wood Hammer';
                    break;
                case 'Glitch':
                    basePower = 0;
                    desc.moveName = 'Metronome';
                    break;
                case 'Grassy':
                    basePower = 90;
                    desc.moveName = 'Energy Ball';
                    break;
                case 'Holy':
                    basePower = 100;
                    desc.moveName = 'Judgment';
                    break;
                case 'Icy':
                    basePower = 90;
                    desc.moveName = 'Ice Beam';
                    break;
                case 'Inverse':
                    basePower = 0;
                    desc.moveName = 'Trick Room';
                    break;
                case 'Mirror':
                    basePower = 65;
                    desc.moveName = 'Mirror Shot';
                    break;
                case 'Misty':
                    basePower = 70;
                    desc.moveName = 'Mist Ball';
                    break;
                case 'Mountain':
                    basePower = 75;
                    move.category = 'Physical';
                    move.target = 'allAdjacentFoes';
                    desc.moveName = 'Rock Slide';
                    break;
                case 'Murkwater':
                    basePower = 95;
                    move.target = 'allAdjacent';
                    desc.moveName = 'Sludge Wave';
                    break;
                case 'New World':
                    basePower = 100;
                    desc.moveName = 'Spacial Rend';
                    break;
                case 'Psychic':
                    basePower = 90;
                    desc.moveName = 'Psychic';
                    break;
                case 'Rainbow':
                    basePower = 65;
                    desc.moveName = 'Aurora Beam';
                    break;
                case 'Rocky':
                    basePower = 40;
                    move.category = 'Physical';
                    desc.moveName = 'Rock Smash';
                    break;
                case 'Short-Circuit 0.5':
                case 'Short-Circuit 0.8':
                case 'Short-Circuit 1.2':
                case 'Short-Circuit 1.5':
                case 'Short-Circuit 2':
                    basePower = 80;
                    move.target = 'allAdjacent';
                    desc.moveName = 'Discharge';
                    break;
                case 'Snowy Mt':
                    basePower = 60;
                    move.category = 'Physical';
                    desc.moveName = 'Avalanche';
                    break;
                case 'Starlight':
                    basePower = 95;
                    desc.moveName = 'Moonblast';
                    break;
                case 'Superheated':
                    basePower = 95;
                    move.target = 'allAdjacentFoes';
                    desc.moveName = 'Heat Wave';
                    break;
                case 'Swamp':
                    basePower = 90;
                    move.target = 'allAdjacent';
                    desc.moveName = 'Muddy Water';
                    break;
                case 'Underwater':
                    basePower = 60;
                    desc.moveName = 'Water Pulse';
                    break;
                case 'Wasteland':
                    basePower = 120;
                    move.category = 'Physical';
                    desc.moveName = 'Gunk Shot';
                    break;
                case 'Water':
                    basePower = 35;
                    desc.moveName = 'Whirlpool';
                    break;
                default:
                    basePower = 80;
                    desc.moveName = 'Tri Attack';
            }
            break;
        case 'Plasma Fists':
            basePower = move.bp;
            desc.moveBP = basePower;
            break;
        case 'Stoked Sparksurfer':
        case 'Genesis Supernova':
            basePower = move.bp;
            desc.moveBP = basePower;
            break;
        case 'Bloom Doom':
            basePower = move.bp;
            desc.moveBP = basePower;
            break;
        case 'Water Shuriken':
            basePower = attacker.named('Greninja-Ash') && attacker.hasAbility('Battle Bond') ? 20 : 15;
            desc.moveBP = basePower;
            break;
        case 'Triple Axel':
            basePower = hit * 20;
            desc.moveBP = move.hits === 2 ? 60 : move.hits === 3 ? 120 : 20;
            break;
        case 'Triple Kick':
            basePower = hit * 10;
            desc.moveBP = move.hits === 2 ? 30 : move.hits === 3 ? 60 : 10;
            break;
        case 'Crush Grip':
        case 'Wring Out':
            basePower = 100 * Math.floor((defender.curHP() * 4096) / defender.maxHP());
            basePower = Math.floor(Math.floor((120 * basePower + 2048 - 1) / 4096) / 100) || 1;
            desc.moveBP = basePower;
            break;
        case 'Hard Press':
            basePower = 100 * Math.floor((defender.curHP() * 4096) / defender.maxHP());
            basePower = Math.floor(Math.floor((100 * basePower + 2048 - 1) / 4096) / 100) || 1;
            desc.moveBP = basePower;
            break;
        case 'Tera Blast':
            basePower = attacker.teraType === 'Stellar' ? 100 : 80;
            desc.moveBP = basePower;
            break;
        default:
            basePower = move.bp;
    }
    switch (field.terrain) {
        case 'Murkwater':
            if (move.hasType('Ground') && !move.named('Mud Shot', 'Mud-Slap', 'Thousand Waves', 'Mud Bomb')) {
                basePower *= 0;
            }
            break;
        case 'Cave':
            if (move.named('Sky Drop')) {
                basePower *= 0;
            }
            break;
        case 'Dark Crystal':
            if (move.named('Solar Blade', 'Solar Beam')) {
                basePower *= 0;
            }
            break;
        case 'Electric':
            if (move.named('Focus Punch')) {
                basePower *= 0;
            }
            break;
        case 'Misty':
            if (move.named('Explosion', 'Mind Blown', 'Self-Destruct')) {
                basePower *= 0;
            }
            break;
        case 'Swamp':
            if (move.named('Explosion', 'Mind Blown', 'Self-Destruct')) {
                basePower *= 0;
            }
            break;
        case 'Underwater':
            if (move.hasType('Fire')) {
                basePower *= 0;
            }
            break;
        case 'Water':
            if (move.hasType('Ground')) {
                basePower *= 0;
            }
            break;
        default:
            desc.moveBP = basePower;
            desc.terrain = field.terrain;
            desc.moveType = move.type;
    }
    if (basePower === 0) {
        return 0;
    }
    var bpMods = calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder, hit);
    basePower = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((basePower * (0, util_2.chainMods)(bpMods, 41, 2097152)) / 4096)));
    desc.moveBP = Math.floor(basePower);
    if (attacker.teraType && move.type === attacker.teraType &&
        attacker.hasType(attacker.teraType) && move.hits === 1 && !move.multiaccuracy &&
        move.priority <= 0 && move.bp > 0 && !move.named('Dragon Energy', 'Eruption', 'Water Spout') &&
        basePower < 60 && gen.num >= 9) {
        basePower = 60;
        desc.moveBP = 60;
    }
    return basePower;
}
exports.calculateBasePowerSMSSSV = calculateBasePowerSMSSSV;
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder, hit) {
    var bpMods = [];
    var defenderItem = (defender.item && defender.item !== '')
        ? defender.item : defender.disabledItem;
    var resistedKnockOffDamage = (!defenderItem || (0, util_2.isQPActive)(defender, field)) ||
        (defender.named('Dialga-Origin') && defenderItem === 'Adamant Crystal') ||
        (defender.named('Palkia-Origin') && defenderItem === 'Lustrous Globe') ||
        (defender.name.includes('Giratina-Origin') && defenderItem.includes('Griseous')) ||
        (defender.name.includes('Arceus') && defenderItem.includes('Plate')) ||
        (defender.name.includes('Genesect') && defenderItem.includes('Drive')) ||
        (defender.named('Groudon', 'Groudon-Primal') && defenderItem === 'Red Orb') ||
        (defender.named('Kyogre', 'Kyogre-Primal') && defenderItem === 'Blue Orb') ||
        (defender.name.includes('Silvally') && defenderItem.includes('Memory')) ||
        defenderItem.includes(' Z') ||
        (defender.named('Zacian') && defenderItem === 'Rusted Sword') ||
        (defender.named('Zamazenta') && defenderItem === 'Rusted Shield') ||
        (defender.name.includes('Ogerpon-Cornerstone') && defenderItem === 'Cornerstone Mask') ||
        (defender.name.includes('Ogerpon-Hearthflame') && defenderItem === 'Hearthflame Mask') ||
        (defender.name.includes('Ogerpon-Wellspring') && defenderItem === 'Wellspring Mask') ||
        (defender.named('Venomicon-Epilogue') && defenderItem === 'Vile Vial');
    if (!resistedKnockOffDamage && defenderItem) {
        var item = gen.items.get(util_1.toID(defenderItem));
        resistedKnockOffDamage = !!item.megaEvolves && defender.name.includes(item.megaEvolves);
    }
    if (!resistedKnockOffDamage && defenderItem) {
        var item = gen.items.get((0, util_1.toID)(defenderItem));
        if (item.name == "Elemental Seed" && field.hasTerrain('Electric', 'Grassy', 'Misty', 'Burning', 'Corrosive Mist', 'Icy', 'Water', 'Underwater', 'Murkwater', "Dragon's Den")) {
            resistedKnockOffDamage = true;
        }
        else if (item.name == "Synthetic Seed" && field.hasTerrain('Chess Board', 'Factory', 'Glitch', 'Mirror', 'Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2', 'Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5', "Big Top WEAK", "Big Top OK", "Big Top NICE", "Big Top POWER", "Big Top 9000")) {
            resistedKnockOffDamage = true;
        }
        else if (item.name == "Telluric Seed" && field.hasTerrain('Swamp', 'Corrosive', 'Desert', 'Rocky', 'Forest', 'Superheated', 'Wasteland', 'Ashen Beach', 'Cave', 'Mountain', 'Snowy Mt')) {
            resistedKnockOffDamage = true;
        }
        else if (item.name == "Magical Seed" && field.hasTerrain('Dark Crystal', 'Rainbow', 'Crystal Fire', 'Crystal Water', 'Crystal Grass', 'Crystal Psychic', 'Holy', 'Fairy Tale', 'Starlight', 'New World', 'Inverse', 'Psychic')) {
            resistedKnockOffDamage = true;
        }
    }
    if (!resistedKnockOffDamage && hit > 1 && !defender.hasAbility('Sticky Hold')) {
        resistedKnockOffDamage = true;
    }    
    if ((move.named('Facade') && attacker.hasStatus('brn', 'par', 'psn', 'tox')) ||
        (move.named('Brine') && defender.curHP() <= defender.maxHP() / 2) ||
        (move.named('Venoshock') && (defender.hasStatus('psn', 'tox') || field.hasTerrain('Corrosive', 'Corrosive Mist', 'Murkwater', 'Wasteland'))) ||
        (move.named('Lash Out') && ((0, util_2.countBoosts)(gen, attacker.boosts) < 0))) {
        bpMods.push(8192);
        desc.moveBP = basePower * 2;
    }
    else if (move.named('Expanding Force') && (0, util_2.isGrounded)(attacker, field) && field.hasTerrain('Psychic')) {
        move.target = 'allAdjacentFoes';
        bpMods.push(6144);
        desc.moveBP = basePower * 1.5;
    }
    else if ((move.named('Knock Off') && !resistedKnockOffDamage) ||
        (move.named('Misty Explosion') && (0, util_2.isGrounded)(attacker, field) && field.hasTerrain('Misty')) ||
        (move.named('Grav Apple') && field.isGravity)) {
        bpMods.push(6144);
        desc.moveBP = basePower * 1.5;
    }
    else if (move.named('Solar Beam', 'Solar Blade') &&
        field.hasWeather('Rain', 'Heavy Rain', 'Sand', 'Hail', 'Snow')) {
        bpMods.push(2048);
        desc.moveBP = basePower / 2;
        desc.weather = field.weather;
    }
    else if (move.named('Collision Course', 'Electro Drift')) {
        var isGhostRevealed = attacker.hasAbility('Scrappy') || attacker.hasAbility('Mind\'s Eye') ||
            field.defenderSide.isForesight;
        var isRingTarget = defender.hasItem('Ring Target') && !defender.hasAbility('Klutz');
        var types = defender.teraType && defender.teraType !== 'Stellar'
            ? [defender.teraType] : defender.types;
        var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, types[0], isGhostRevealed, field.isGravity, isRingTarget);
        var type2Effectiveness = types[1] ? (0, util_2.getMoveEffectiveness)(gen, move, types[1], isGhostRevealed, field.isGravity, isRingTarget) : 1;
        if (type1Effectiveness * type2Effectiveness >= 2) {
            bpMods.push(5461);
            desc.moveBP = basePower * (5461 / 4096);
        }
    }
    if (field.attackerSide.isHelpingHand || (field.hasTerrain("Big Top WEAK", "Big Top OK", "Big Top NICE", "Big Top POWER", "Big Top 9000") && attacker.hasItem('Synthetic Seed'))) {
        bpMods.push(6144);
        desc.isHelpingHand = true;
    }
    if ((attacker.hasAbility('Flare Boost') && (attacker.hasStatus('brn') || field.hasTerrain('Burning')) && move.category === 'Special') ||
        (attacker.hasAbility('Toxic Boost') && (attacker.hasStatus('psn', 'tox') || field.hasTerrain('Corrosive', 'Murkwater', 'Wasteland')) && move.category === 'Physical') ||
        (attacker.hasAbility('Mega Launcher') && move.flags.pulse) ||
        (attacker.hasAbility('Strong Jaw') && move.flags.bite) ||
        (attacker.hasAbility('Sharpness') && move.flags.slicing)) {
        bpMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    if (attacker.hasAbility('Technician')) {
        if (field.hasTerrain('Factory') && basePower <= 80) {
            bpMods.push(6144);
            desc.attackerAbility = attacker.ability;
        }
        else if (basePower <= 60) {
            bpMods.push(6144);
            desc.attackerAbility = attacker.ability;
        }
    }
    var aura = "".concat(move.type, " Aura");
    var isAttackerAura = attacker.hasAbility(aura);
    var isDefenderAura = defender.hasAbility(aura);
    var isUserAuraBreak = attacker.hasAbility('Aura Break') || defender.hasAbility('Aura Break');
    var isFieldAuraBreak = field.isAuraBreak;
    var isFieldFairyAura = field.isFairyAura && move.type === 'Fairy';
    var isFieldDarkAura = field.isDarkAura && move.type === 'Dark';
    var auraActive = isAttackerAura || isDefenderAura || isFieldFairyAura || isFieldDarkAura;
    var auraBreak = isFieldAuraBreak || isUserAuraBreak;
    if (auraActive) {
        if (auraBreak) {
            bpMods.push(3072);
            desc.attackerAbility = attacker.ability;
            desc.defenderAbility = defender.ability;
        }
        else {
            bpMods.push(5448);
            if (isAttackerAura)
                desc.attackerAbility = attacker.ability;
            if (isDefenderAura)
                desc.defenderAbility = defender.ability;
        }
    }
    if ((attacker.hasAbility('Sheer Force') && (move.secondaries || move.named('Order Up')) && !move.isMax) ||
        (attacker.hasAbility('Sand Force') && (field.hasWeather('Sand') || field.hasTerrain('Desert', 'Ashen Beach')) && move.hasType('Rock', 'Ground', 'Steel')) ||
        (attacker.hasAbility('Analytic') && (turnOrder !== 'first' || field.defenderSide.isSwitching === 'out')) ||
        (attacker.hasAbility('Tough Claws') && move.flags.contact) ||
        (attacker.hasAbility('Punk Rock') && move.flags.sound)) {
        bpMods.push(5325);
        desc.attackerAbility = attacker.ability;
    }
    if (field.attackerSide.isBattery && move.category === 'Special') {
        bpMods.push(5325);
        desc.isBattery = true;
    }
    if (field.attackerSide.isPowerSpot) {
        bpMods.push(5325);
        desc.isPowerSpot = true;
    }
    if (attacker.hasAbility('Rivalry') && ![attacker.gender, defender.gender].includes('N')) {
        if (attacker.gender === defender.gender) {
            bpMods.push(5120);
            desc.rivalry = 'buffed';
        }
        else {
            bpMods.push(3072);
            desc.rivalry = 'nerfed';
        }
        desc.attackerAbility = attacker.ability;
    }
    if (!move.isMax && hasAteAbilityTypeChange) {
        if (attacker.hasAbility('Galvanize')) {
            if (field.hasTerrain('Electric', 'Factory')) {
                bpMods.push(6144);
            }
            else if (field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2')) {
                bpMods.push(8192);
            }
            else {
                bpMods.push(4915);
            }
        }
        else if (attacker.hasAbility('Pixilate')) {
            if (field.hasTerrain('Misty')) {
                bpMods.push(6144);
            }
            else {
                bpMods.push(4915);
            }
        }
        else if (attacker.hasAbility('Aerilate')) {
            if (field.hasTerrain('Mountain', 'Snowy Mt')) {
                bpMods.push(6144);
            }
            else {
                bpMods.push(4915);
            }
        }
        else if (attacker.hasAbility('Refrigerate')) {
            if (field.hasTerrain('Icy', 'Snowy Mt')) {
                bpMods.push(6144);
            }
            else {
                bpMods.push(4915);
            }
        }
    }
    if ((attacker.hasAbility('Reckless') && (move.recoil || move.hasCrashDamage)) ||
        (attacker.hasAbility('Iron Fist') && move.flags.punch)) {
        bpMods.push(4915);
        desc.attackerAbility = attacker.ability;
    }
    if (gen.num <= 8 && defender.hasAbility('Heatproof') && move.hasType('Fire')) {
        bpMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    else if (defender.hasAbility('Dry Skin') && move.hasType('Fire')) {
        bpMods.push(5120);
        desc.defenderAbility = defender.ability;
    }
    if (attacker.hasAbility('Supreme Overlord') && attacker.alliesFainted) {
        var powMod = [4096, 4506, 4915, 5325, 5734, 6144];
        bpMods.push(powMod[Math.min(5, attacker.alliesFainted)]);
        desc.attackerAbility = attacker.ability;
        desc.alliesFainted = attacker.alliesFainted;
    }
    if (attacker.hasItem("".concat(move.type, " Gem"))) {
        bpMods.push(5325);
        desc.attackerItem = attacker.item;
    }
    else if ((((attacker.hasItem('Adamant Crystal') && attacker.named('Dialga-Origin')) ||
        (attacker.hasItem('Adamant Orb') && attacker.named('Dialga'))) &&
        move.hasType('Steel', 'Dragon')) ||
        (((attacker.hasItem('Lustrous Orb') &&
            attacker.named('Palkia')) ||
            (attacker.hasItem('Lustrous Globe') && attacker.named('Palkia-Origin'))) &&
            move.hasType('Water', 'Dragon')) ||
        (((attacker.hasItem('Griseous Orb') || attacker.hasItem('Griseous Core')) &&
            (attacker.named('Giratina-Origin') || attacker.named('Giratina'))) &&
            move.hasType('Ghost', 'Dragon')) ||
        (attacker.hasItem('Vile Vial') &&
            attacker.named('Venomicon-Epilogue') &&
            move.hasType('Poison', 'Flying')) ||
        (attacker.hasItem('Soul Dew') &&
            attacker.named('Latios', 'Latias', 'Latios-Mega', 'Latias-Mega') &&
            move.hasType('Psychic', 'Dragon')) ||
        attacker.item && move.hasType((0, items_1.getItemBoostType)(attacker.item)) ||
        (attacker.name.includes('Ogerpon-Cornerstone') && attacker.hasItem('Cornerstone Mask')) ||
        (attacker.name.includes('Ogerpon-Hearthflame') && attacker.hasItem('Hearthflame Mask')) ||
        (attacker.name.includes('Ogerpon-Wellspring') && attacker.hasItem('Wellspring Mask'))) {
        bpMods.push(4915);
        desc.attackerItem = attacker.item;
    }
    else if ((attacker.hasItem('Muscle Band') && move.category === 'Physical') ||
        (attacker.hasItem('Wise Glasses') && move.category === 'Special')) {
        bpMods.push(4505);
        desc.attackerItem = attacker.item;
    }
    else if (attacker.hasItem('Punching Glove') && move.flags.punch) {
        bpMods.push(4506);
    }
    if (field.hasTerrain('Ashen Beach')) {
        if (move.named('Mud Bomb', 'Mud Shot', 'Mud-Slap', 'Sand Tomb')) {
            bpMods.push(8192)
        }
        if (move.named('Hidden Power', 'Hidden Power Bug', 'Hidden Power Dark', 'Hidden Power Dragon', 'Hidden Power Electric', 'Hidden Power Fighting', 'Hidden Power Fairy', 'Hidden Power Fire', 'Hidden Power Flying', 'Hidden Power Ghost', 'Hidden Power Grass', 'Hidden Power Ground', 'Hidden Power Ice', 'Hidden Power Poison', 'Hidden Power Psychic', 'Hidden Power Rock', 'Hidden Power Steel', 'Hidden Power Water', 'Land\'s Wrath', 'Muddy Water', 'Strength', 'Surf', 'Thousand Waves', 'Clangorous Soulblaze')) {
            bpMods.push(6144);
        }
        if (move.named('Splintered Stormshards', 'Aura Sphere', 'Focus Blast', 'Stored Power', 'Zen Headbutt')) {
            bpMods.push(5325);
        }
        if (move.named('Psychic')) {
             bpMods.push(4915);
        }
    }
    else if (field.hasTerrain("Big Top WEAK", "Big Top OK", "Big Top NICE", "Big Top POWER", "Big Top 9000")) {
        if (move.flags.sound || move.named('Fiery Dance', 'Fire Lash', 'First Impression', 'Fly', 'Petal Dance', 'Power Whip', 'Revelation Dance', 'Vine Whip')) {
            bpMods.push(6144);
        }
        else if (move.named('Pay Day')) {
            bpMods.push(8192);
        }
        if (move.named('Acrobatics', 'Nature Power')) {
            bpMods.push(6144);
        }
        if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if (move.named('Blaze Kick', 'Body Slam', 'Bounce', 'Brutal Swing', 'Bulldoze', 'Crabhammer', 'Dragon Hammer', 'Dragon Rush', 'Dual Chop', 'Earthquake', 'Giga Impact', 'Heat Crash', 'Heavy Slam', 'High Horsepower', 'Ice Hammer', 'Icicle Crash', 'Iron Tail', 'Magnitude', 'Meteor Mash', 'Pound', 'Sky Drop', 'Slam', 'Smack Down', 'Stomp', 'Stomping Tantrum', 'Strength', 'Wood Hammer') || (move.hasType('Fighting') && move.category == 'Physical')) {
            if (field.hasTerrain('Big Top WEAK')) {
                bpMods.push(2048);
            }
            else if (field.hasTerrain('Big Top NICE')) {
                bpMods.push(6144);
            }
            else if(field.hasTerrain('Big Top POWER')) {
                bpMods.push(8192);
            }
            else if (field.hasTerrain('Big Top 9000')) {
                bpMods.push(12288);
            }
        }
    }
    else if (field.hasTerrain('Burning')) {
            if (move.named('Smack Down', 'Thousand Arrows')) {
                bpMods.push(8192);
            }
            else if (move.named('Clear Smog', 'Smog')) {
                bpMods.push(6144);
            }
            else if (move.named('Blizzard', 'Subzero Slammer', 'Splintered Stormshards', 'Gust', 'Hurricane', 'Muddy Water', 'Razor Wind', 'Sand Tomb', 'Sludge Wave', 'Sparkling Aria', 'Surf', 'Twister', 'Water Pledge', 'Water Spout', 'Continental Crush', 'Hydro Vortex', 'Oceanic Operetta', 'Splintered Stormshards', 'Supersonic Skystrike')) {
                bpMods.push(5325);
            }
            if ((0, util_2.isGrounded)(attacker, field) && move.hasType('Fire')) {
                bpMods.push(6144);
            }
            if ((move.hasType('Grass') && (0, util_2.isGrounded)(defender, field)) || move.hasType('Ice')) {
                bpMods.push(2048);
            }
        }
    else if (field.hasTerrain('Cave')) {
        if (move.named('Rock Tomb', 'Nature Power')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Diamond Storm', 'Power Gem')) {
            bpMods.push(5325);
        }
        if (move.hasType('Rock')) {
            bpMods.push(6144);
        }
        if (move.flags.sound) {
            bpMods.push(6144);
        }
        if (move.hasType('Flying') && !move.flags.contact) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Chess Board')) {
        if (move.named('Ancient Power', 'Nature Power', 'Psychic', 'Secret Power', 'Strength', 'Continental Crush', 'Shattered Psyche', 'Fake Out', 'Feint', 'Feint Attack', 'First Impression', 'Shadow Sneak', 'Smart Strike', 'Sucker Punch')) {
            bpMods.push(6144);
        }
        if (move.named('Stomping Tantrum', 'Tectonic Rage', 'Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if (move.named('Ancient Power', 'Psychic', 'Secret Power', 'Strength', 'Continental Crush', 'Shattered Psyche') && defender.hasAbility('Oblivious', 'Simple', 'Unaware', 'Klutz')) {
            bpMods.push(8192);
        }
        else if (move.named('Ancient Power', 'Psychic', 'Secret Power', 'Strength', 'Continental Crush', 'Shattered Psyche') && defender.hasAbility('Adaptability', 'Synchronize', 'Anticipation', 'Telepathy')) {
            bpMods.push(2048);
        }
        if (attacker.hasAbility('Queenly Majesty') || attacker.hasPiece('Q')) {
            bpMods.push(6144);
        }
        if (attacker.hasPiece('N') && defender.hasPiece('Q')) {
            bpMods.push(12288);
        }
    }
    else if (field.hasTerrain('Corrosive')) {
        if (move.named('Acid', 'Acid Spray', "Grass Knot", 'Nature Power')) {
            bpMods.push(8192);
        }
        else if (move.named('Mud Bomb', 'Mud Shot', 'Mud-Slap', 'Muddy Water', 'Smack Down', 'Thousand Arrows', 'Whirlpool')) {
            bpMods.push(6144);
        }
        else if (move.named('Seed Flare', 'Splintered Stormshards')) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('Corrosive Mist')) {
        if (move.named('Bubble', 'Bubble Beam', 'Sparkling Aria', 'Acid Spray', 'Clear Smog', 'Smog')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Eruption', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive', 'Explosion', 'Self-Destruct', 'Gust', 'Hurricane', 'Razor Wind', 'Seed Flare', 'Twister', 'Supersonic Skystrike')) {
            bpMods.push(5325);
        }
        if (move.hasType('Fire')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Crystal Fire', 'Crystal Water', 'Crystal Grass', 'Crystal Psychic')) {
        if (move.named('Judgment', 'Multi-Attack', 'Rock Climb', 'Strength', 'Ancient Power', 'Diamond Storm', 'Power Gem', 'Nature Power', 'Rock Smash', 'Rock Tomb')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Dark Pulse', 'Night Daze', 'Light That Burns the Sky', 'Bulldoze', 'Earthquake', 'Magnitude', 'Tectonic Rage', 'Aurora Beam', 'Dazzling Gleam', 'Doom Desire', 'Luster Purge', 'Mirror Shot', 'Moongeist Beam', 'Photon Geyser', 'Signal Beam', 'Techno Blast', 'Menacing Moonraze Maelstrom')) {
            bpMods.push(5325);
        }
        if (move.hasType('Rock') || move.hasType('Dragon')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Dark Crystal')) {
        if (move.named('Prismatic Laser', 'Black Hole Eclipse')) {
            bpMods.push(8192);
        }
        else if (move.named('Aurora Beam', 'Dark Pulse', 'Nature Power', 'Dazzling Gleam', 'Diamond Storm', 'Doom Desire', 'Flash Cannon', 'Luster Purge', 'Mirror Shot', 'Moongeist Beam', 'Night Daze', 'Night Slash', 'Photon Geyser', 'Power Gem', 'Shadow Ball', 'Shadow Bone', 'Shadow Claw', 'Shadow Force', 'Shadow Punch', 'Shadow Sneak', 'Signal Beam', 'Techno Blast', 'Menacing Moonraze Maelstrom')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Bulldoze', 'Earthquake', 'Magnitude', 'Tectonic Rage')) {
            bpMods.push(5325);
        }
        else if (move.named('Light That Burns the Sky')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Desert')) {
        if (move.named('Burn Up', 'Dig', 'Heat Wave', 'Needle Arm', 'Pin Missile', 'Sand Tomb', 'Nature Power', 'Solar Beam', 'Solar Blade', 'Thousand Waves', 'Searing Sunraze Smash')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if ((move.hasType('Water') && (0, util_2.isGrounded)(attacker, field)) || (move.hasType('Electric') && (0, util_2.isGrounded)(defender, field))) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain("Dragon's Den")) {
        if (move.named('Smack Down', 'Thousand Arrows', 'Continental Crush', 'Tectonic Rage', 'Dragon Ascent', 'Pay Day')) {
            bpMods.push(8192);
        }
        else if (move.named('Lava Plume', 'Magma Storm', 'Mega Kick')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Glaciate', 'Hydro Vortex', 'Oceanic Operetta', 'Subzero Slammer')) {
            bpMods.push(5325);
        }
        if (move.hasType('Dragon', 'Fire')) {
            bpMods.push(6144);
        }
        else if (move.hasType('Water', 'Ice')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Electric')) {
        if (move.named('Magnet Bomb')) {
            bpMods.push(8192);
        }
        else if (move.named('Explosion', 'Hurricane', 'Muddy Water', 'Self-Destruct', 'Smack Down', 'Surf', 'Thousand Arrows')) {
            bpMods.push(6144);
        }
        else if (move.named('Tectonic Rage', 'Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if ((0, util_2.isGrounded)(attacker, field) && move.hasType('Electric')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Factory')) {
        if (move.named('Flash Cannon', 'Gear Grind', 'Nature Power', 'Gyro Ball', 'Magnet Bomb')) {
            bpMods.push(8192);
        }
        else if (move.named('Steamroller', 'Techno Blast')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Bulldoze', 'Discharge', 'Earthquake', 'Explosion', 'Magnitude', 'Self-Destruct', 'Light That Burns the Sky', 'Tectonic Rage')) {
            bpMods.push(5325);
        }
        if (move.hasType('Electric')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Fairy Tale')) {
        if (move.hasType('Dragon')) {
            bpMods.push(8192);
        }
        if (move.named('Draining Kiss')) {
            bpMods.push(8192);
        }
        else if (move.named('Air Slash', 'Ancient Power', 'Fleur Cannon', 'Leaf Blade', 'Magical Leaf', 'Moongeist Beam', 'Mystical Fire', 'Night Slash', 'Psycho Cut', 'Relic Song', 'Smart Strike', 'Solar Blade', 'Sparkling Aria', 'Menacing Moonraze Maelstrom', 'Oceanic Operetta')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if (move.hasType('Fairy', 'Steel')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5')) {
        if (field.hasTerrain('Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5') && !field.hasWeather('Rain', 'Heavy Rain') && move.named('Eruption', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive')) {
            bpMods.push(5325);
        }
        if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if (move.named('Cut') && !field.hasTerrain('Flower Garden 1')) {
            bpMods.push(6144);
        }
        if (move.named('Petal Blizzard', 'Fleur Cannon', 'Petal Dance')) {
            if (field.hasTerrain('Flower Garden 3')) {
                bpMods.push(4915);
            }
            else if (field.hasTerrain('Flower Garden 4', 'Flower Garden 5')) {
                bpMods.push(6144);
            }
        }
        if (move.named('Nature Power') && field.hasTerrain('Flower Garden 5')) {
            bpMods.push(6144);
        }
        if (move.hasType('Grass')) {
            if (field.hasTerrain('Flower Garden 2')) {
                bpMods.push(4506);;
            }
            else if (field.hasTerrain('Flower Garden 3')) {
                bpMods.push(5325);
            }
            else if (field.hasTerrain('Flower Garden 4')) {
                bpMods.push(6144);
            }
            else if (field.hasTerrain('Flower Garden 5')) {
                bpMods.push(8192);
            }
        }
        else if (move.hasType('Bug')) {
            if (field.hasTerrain('Flower Garden 2', 'Flower Garden 3')) {
                bpMods.push(6144);
            }
            else if (field.hasTerrain('Flower Garden 4', 'Flower Garden 5')) {
                bpMods.push(8192);
            }
        }
        else if (move.hasType('Fire') && field.hasTerrain('Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Forest')) {
        if (move.named('Attack Order', 'Cut', 'Electroweb')) {
            if (move.named('Cut')) {
                bpMods.push(6144);
            }
        }
        else if (move.named('Muddy Water', 'Surf')) {
            bpMods.push(2048);
        }
        else if (move.named('Splintered Stormshards', 'Eruption', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive') && !field.hasWeather('Rain', 'Heavy Rain')) {
            bpMods.push(5325);
        }
        if (move.hasType('Grass') || (move.hasType('Bug') && move.category === 'Special')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Glitch')) {
        if (move.hasType('Psychic')) {
                bpMods.push(4915);
        }
        else if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('Grassy')) {
        if (move.named('Fairy Wind', 'Silver Wind')) {
            bpMods.push(6144);
        }
        else if (move.named('Bulldoze', 'Earthquake', 'Magnitude', 'Muddy Water', 'Surf')) {
            bpMods.push(2048);
        }
        if ((0, util_2.isGrounded)(attacker, field) && move.hasType('Grass')) {
            bpMods.push(6144);
        }
        if ((0, util_2.isGrounded)(defender, field) && move.hasType('Fire')) {
            bpMods.push(6144);
        }
        if (move.named('Sludge Wave', 'Acid Downpour') || (move.named('Splintered Stormshards', 'Eruption', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive') && !field.hasWeather('Rain', 'Heavy Rain'))) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('Holy')) {
        if (move.named('Ancient Power', 'Extreme Speed', 'Judgment', 'Nature Power', 'Magical Leaf', 'Mystical Fire', 'Return', 'Sacred Fire', 'Sacred Sword')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Light That Burns the Sky', 'Aeroblast', 'Crush Grip', 'Diamond Storm', 'Doom Desire', 'Dragon Ascent', 'Fleur Cannon', 'Hyperspace Hole', 'Land\'s Wrath', 'Luster Purge', 'Mist Ball', 'Moongeist Beam', 'Origin Pulse', 'Precipice Blades', 'Prismatic Laser', 'Psycho Boost', 'Psystrike', 'Relic Song', 'Roar of Time', 'Secret Sword', 'Spacial Rend', 'Sunsteel Strike', 'V-create', 'Genesis Supernova', 'Menacing Moonraze Maelstrom', 'Searing Sunraze Smash')) {
            bpMods.push(5325);
        }
        if (move.hasType('Normal', 'Fairy') && move.category === 'Special') {
            bpMods.push(6144);
        }
        else if (move.hasType('Psychic', 'Dragon')) {
                bpMods.push(4915);
        }
        else if (move.hasType('Ghost') || (move.hasType('Dark') && move.category === 'Special')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Icy')) {
        if (move.named('Scald', 'Steam Eruption')) {
            bpMods.push(2048);
        }
        else if (move.named('Splintered Stormshards', 'Eruption', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive')) {
            bpMods.push(5325);
        }
        if (move.hasType('Ice') || (move.flags.sound && attacker.hasAbility('Liquid Voice'))) {
            bpMods.push(6144);
        }
        else if (move.hasType('Fire')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Inverse')) {
        if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('Mirror')) {
        if (move.named('Mirror Shot', 'Nature Power')) {
            bpMods.push(8192);
        }
        else if (move.named('Aurora Beam', 'Dazzling Gleam', 'Doom Desire', 'Flash Cannon', 'Luster Purge', 'Photon Geyser', 'Prismatic Laser', 'Signal Beam', 'Techno Blast', 'Light That Burns the Sky')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Boomburst', 'Bulldoze', 'Earthquake', 'Explosion', 'Hyper Voice', 'Magnitude', 'Self-Destruct', 'Tectonic Rage')) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('Misty')) {
        if (move.named('Dark Pulse', 'Night Daze', 'Shadow Ball')) {
            bpMods.push(2048);
        }
        else if (move.named('Aura Sphere', 'Dazzling Gleam', 'Doom Desire', 'Fairy Wind', 'Icy Wind', 'Magical Leaf', 'Mist Ball', 'Nature Power', 'Moonblast', 'Moongeist Beam', 'Mystical Fire', 'Silver Wind', 'Steam Eruption')) {
            bpMods.push(6144);
        }
        else if (move.named('Acid Downpour')) {
            bpMods.push(5325);
        }
        if (move.hasType('Dragon')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Mountain')) {
        if (move.named('Avalanche', 'Circle Throw', 'Eruption', 'Fairy Wind', 'Hyper Voice', 'Icy Wind', 'Ominous Wind', 'Razor Wind', 'Silver Wind', 'Storm Throw', 'Thunder', 'Twister', 'Vital Throw')) {
            bpMods.push(6144);
        }
        if (move.named('Fairy Wind', 'Gust', 'Icy Wind', 'Ominous Wind', 'Razor Wind', 'Silver Wind', 'Twister') && field.hasWeather('Strong Winds')) {
            bpMods.push(6144);
        }
        if (move.named('Splintered Stormshards', 'Blizzard', 'Glaciate', 'Subzero Slammer')) {
            bpMods.push(5325);
        }
        if (move.hasType('Flying') && move.category == 'Special' && field.hasWeather('Strong Winds')) {
            bpMods.push(6144);
        }
        if (move.hasType('Rock', "Flying")) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Murkwater')) {
        if (move.named('Mud Bomb', 'Mud Shot', 'Mud-Slap', 'Thousand Waves', 'Acid', 'Acid Spray', 'Brine', 'Smack Down')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Whirlpool', 'Blizzard', 'Glaciate', 'Subzero Slammer')) {
            bpMods.push(5325);
        }
        if (move.hasType('Water')) {
            bpMods.push(6144);
        }
        else if (move.hasType('Poison')) {
            bpMods.push(6144);
        }
        else if (move.hasType('Electric') && (0, util_2.isGrounded)(defender, field)) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('New World')) {
        if (move.named('Doom Desire')) {
            bpMods.push(16384);
        }
        else if (move.named('Ancient Power', 'Comet Punch', 'Draco Meteor', 'Future Sight', 'Hyperspace Fury', 'Hyperspace Hole', 'Meteor Mash', 'Moonblast', 'Spacial Rend', 'Nature Power', 'Swift', 'Vacuum Wave', 'Black Hole Eclipse')) {
            bpMods.push(8192);
        }
        else if (move.named('Aeroblast', 'Aurora Beam', 'Bolt Strike', 'Blue Flare', 'Core Enforcer', 'Crush Grip', 'Dazzling Gleam', 'Diamond Storm', 'Dragon Ascent', 'Earth Power', 'Eruption', 'Flash Cannon', 'FleurCannon', 'Freeze Shock', 'Fusion Bolt', 'Fusion Flare', 'Ice Burn', 'Judgment', 'Land\'s Wrath', 'Luster Purge', 'Magma Storm', 'Mind Blown', 'Mirro Shot', 'Mist Ball', 'Moongeist Beam', 'Multi-Attack', 'Oblivion Wing', 'Origin Pulse', 'Photon Geyser', 'Plasma Fists', 'Power Gem', 'Precipice Blades', 'Prismatic Laser', 'Psycho Boost', 'Psystrike', 'Relic Song', 'Roar of Time', 'Sacred Fire', 'Sacred Sword', 'Searing Shot', 'Secret Sword', 'Seed Flare', 'Shadow Force', 'Signal Beam', 'Spectral Thief', 'Steam Eruption', 'Sunsteel Strike', 'Techno Blast', 'Thousand Arrows', 'Thousand Waves', 'V-create', 'Continental Crush', 'Genesis Supernova', 'Menacing Moonraze Maelstrom', 'Searing Sunraze Smash', 'Soul-Stealing 7-Star Strike')) {
            bpMods.push(6144);
        }
        else if (move.named('Bulldoze', 'Earthquake', 'Magnitude')) {
            bpMods.push(1024);
        }
        if (move.hasType('Dark')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Psychic')) {
        if (move.named('Aura Sphere', 'Hex', 'Magical Leaf', 'Mind Blown', 'Moonblast', 'Mystical Fire')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if ((0, util_2.isGrounded)(attacker, field) && move.hasType('Psychic')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Rainbow')) {
        if (move.named('Aurora Beam', 'Nature Power', 'Dazzling Gleam', 'Dragon Pulse', 'Fire Pledge', 'Fleur Cannon', 'Grass Pledge', 'Heart Stamp', 'Hidden Power', 'Hidden Power Bug', 'Hidden Power Dark', 'Hidden Power Dragon', 'Hidden Power Electric', 'Hidden Power Fairy', 'Hidden Power Fighting', 'Hidden Power Fire', 'Hidden Power Flying', 'Hidden Power Ghost', 'Hidden Power Grass', 'Hidden Power Ground', 'Hidden Power Ice', 'Hidden Power Poison', 'Hidden Power Psychic', 'Hidden Power Rock', 'Hidden Power Steel', 'Hidden Power Water', 'Judgment', 'Mist Ball', 'Moonblast', 'Mystical Fire', 'Prismatic Laser', 'Relic Song', 'Sacred Fire', 'Secret Power', 'Silver Wind', 'Solar Beam', 'Solar Blade', 'Sparkling Aria', 'Tri Attack', 'Water Pledge', 'Weather Ball', 'Zen Headbutt', 'Oceanic Operetta', 'Twinkle Tackle')) {
            bpMods.push(6144);
        }
        else if (move.named('Dark Pulse', 'Night Daze', 'Shadow Ball', 'Never-Ending Nightmare')) {
            bpMods.push(2048);
        }
        else if (move.named('Splintered Stormshards', 'Light That Burns the Sky')) {
            bpMods.push(5325);
        }
        if (move.hasType('Normal') && move.category === 'Special') {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Rocky')) {
        if (move.named('Rock Smash', 'Nature Power')) {
            bpMods.push(8192);
        }
        else if (move.named('Bulldoze', 'Earthquake', 'Magnitude', 'Rock Climb', 'Strength', 'Accelerock')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if (move.hasType('Rock')) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Short-Circuit 2', 'Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5')) {
        if (move.named('Flash Cannon', 'Gear Grind', 'Gyro Ball', 'Magnet Bomb', 'Magnet Bomb', 'Muddy Water', 'Surf', 'Dazzling Gleam', 'Hydro Vortex')) {
            bpMods.push(6144);
        }
        else if (move.named('Splintered Stormshards', 'Night Slash', 'Dark Pulse', 'Night Daze', 'Shadow Ball', 'Shadow Bone', 'Shadow Claw', 'Shadow Force', 'Shadow Punch', 'Shadow Sneak', 'Charge Beam', 'Discharge', 'Nature Power', 'Parabolic Charge', 'Wild Charge', 'Gigavolt Havoc')) {
            bpMods.push(5325);
        }
        else if (move.named('Light That Burns the Sky')) {
            bpMods.push(2048);
        }
        if (move.hasType('Electric')) {
            if (field.hasTerrain('Short-Circuit 0.5')) {
                bpMods.push(2048);
            }
            else if (field.hasTerrain('Short-Circuit 0.8')) {
                bpMods.push(3277);
            }
            else if (field.hasTerrain('Short-Circuit 1.2')) {
                    bpMods.push(4915);
            }
            else if (field.hasTerrain('Short-Circuit 1.5')) {
                bpMods.push(6144);
            }
            else if (field.hasTerrain('Short-Circuit 2')) {
                bpMods.push(8192);
            }
        }
    }
    else if (field.hasTerrain('Snowy Mt')) {
        if (move.named('Icy Wind')) {
            bpMods.push(8192);
        }
        if (move.named('Avalanche', 'Nature Power', 'Circle Throw', 'Fairy Wind', 'Glaciate', 'Hyper Voice', 'Ominous Wind', 'Powder Snow', 'Razor Wind', 'Silver Wind', 'Storm Throw', 'Twister', 'Vital Throw')) {
            bpMods.push(6144);
        }
        if (move.named('Fairy Wind', 'Gust', 'Icy Wind', 'Ominous Wind', 'Razor Wind', 'Silver Wind', 'Twister') && field.hasWeather('Strong Winds')) {
            bpMods.push(6144);
        }
        else if (move.named('Scald', 'Steam Eruption')) {
            bpMods.push(2048);
        }
        else if (move.named('Splintered Stormshards', 'Eruption', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive')) {
            bpMods.push(5325);
        }
        if (move.hasType('Flying') && move.category == 'Special' && field.hasWeather('Strong Winds')) {
            bpMods.push(6144);
        }
        if (move.hasType('Rock', 'Flying')) {
            bpMods.push(6144);
        }
        else if (move.hasType('Ice')) {
            bpMods.push(6144);
        }
        else if (move.hasType('Fire')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Starlight')) {
        if (!field.hasWeather('Sun', 'Harsh Sunshine', 'Rain', 'Heavy Rain', 'Hail', 'Strong Winds', 'Sand')) {
            if (move.named('Doom Desire')) {
                bpMods.push(16384);
            }
            else if (move.named('Comet Punch', 'Draco Meteor', 'Hyperspace Fury', 'Hyperspace Hole', 'Meteor Mash', 'Moongeist Beam', 'Spacial Rend', 'Swift', 'Black Hole Eclipse', 'Menacing Moonraze Maelstrom', 'Searing Sunraze Smash')) {
                bpMods.push(8192);
            }
            else if (move.named('Aurora Beam', 'Dazzling Gleam', 'Flash Cannon', 'Luster Purge', 'Mirror Shot', 'Moonblast', 'Nature Power', 'Photon Geyser', 'Signal Beam', 'Solar Beam', 'Techno Blast')) {
                bpMods.push(6144);
            }
            if (move.hasType('Dark', 'Psychic')) {
                bpMods.push(6144);
            }
            else if (move.hasType('Fairy')) {
                bpMods.push(5325);
            }
        }
        else if (move.named('Splintered Stormshards', 'Light That Burns the Sky')) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('Superheated')) {
        if (move.named('Scald', 'Steam Eruption')) {
            bpMods.push(6144);
        }
        else if (move.named('Eruption', 'Explosion', 'Self-Destruct', 'Fire Pledge', 'Flame Burst', 'Heat Wave', 'Nature Power', 'Incinerate', 'Lava Plume', 'Mind Blown', 'Searing Shot', 'Inferno Overdrive') && !field.hasWeather('Rain', 'Heavy Rain')) {
            bpMods.push(5325);
        }
        else if (move.named('Muddy Water', 'Sparkling Aria', 'Surf', 'Water Pledge', 'Water Spout', 'Hydro Vortex', 'Oceanic Operetta')) {
            bpMods.push(2048);
        }
        if (move.hasType('Water') && !move.named('Scald', 'Steam Eruption')) {
            bpMods.push(3686);
        }
        else if (move.hasType('Fire')) {
            bpMods.push(4506);;
        }
        else if (move.hasType('Ice')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Swamp')) {
        if (move.named('Smack Down', 'Thousand Arrows', 'Brine', 'Mud Bomb', 'Gunk Shot', 'Mud Shot', 'Mud-Slap', 'Muddy Water', 'Nature Power', 'Sludge Wave', 'Surf', 'Hydro Vortex')) {
            bpMods.push(6144);
        }
        else if (move.named('Bulldoze', 'Earthquake', 'Magnitude')) {
            bpMods.push(1024);
        }
        else if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
        if (move.hasType('Poison') && (0, util_2.isGrounded)(defender, field)) {
            bpMods.push(6144);
        }
    }
    else if (field.hasTerrain('Underwater')) {
        if (move.named('Anchor Shot')) {
            bpMods.push(8192);
        }
        else if (move.named('Water Pulse', 'Nature Power')) {
            bpMods.push(6144);
        }
        else if (move.named('Dive', 'Bounce', 'Fly', 'Sky Drop', 'Acid Downpour')) {
            bpMods.push(5325);
        }
        if (move.hasType('Electric')) {
            bpMods.push(8192);
        }
        else if (move.hasType('Water')) {
            bpMods.push(6144);
        }
        if (move.category === 'Physical' && !move.hasType('Water', "Ground") && !attacker.hasAbility('Steelworker')) {
            bpMods.push(2048);
        }
    }
    else if (field.hasTerrain('Wasteland')) {
        if (move.named('Spit Up')) {
            bpMods.push(8192);
        }
        if (move.named('Mud Bomb', 'Mud Shot', 'Mud-Slap', 'Power Whip', 'Vine Whip')) {
            bpMods.push(6144);
        }
        if (move.named('Gunk Shot', 'Nature Power', 'Octazooka', 'Sludge', 'Sludge Wave')) {
                bpMods.push(4915);
        }
        if (move.named('Bulldoze', 'Earthquake', 'Magnitude')) {
            bpMods.push(1024);
        }
        if (move.named('Splintered Stormshards')) {
            bpMods.push(5325);
        }
    }
    else if (field.hasTerrain('Water')) {
        if (move.named('Dive', 'Muddy Water', 'Surf', 'Whirlpool', 'Nature Power', 'Hydro Vortex')) {
            bpMods.push(6144);
        }
        if (move.named('Splintered Stormshards', "Dive", 'Blizzard', 'Glaciate', 'Subzero Slammer', 'Acid Downpour')) {
            bpMods.push(5325);
        }
        if (move.hasType('Water') || (move.hasType('Electric') && (0, util_2.isGrounded)(defender, field))) {
            bpMods.push(6144);
        }
        else if (move.hasType('Fire') && (0, util_2.isGrounded)(defender, field)) {
            bpMods.push(2048);
        }
    }
    return bpMods;
}
exports.calculateBPModsSMSSSV = calculateBPModsSMSSSV;
function calculateAttackSMSSSV(gen, attacker, defender, move, field, desc, isCritical) {
    if (isCritical === void 0) { isCritical = false; }
    var attack;
    var attackSource = move.named('Foul Play') ? defender : attacker;
    if (1 === 1) {
        // Glitch Field + Photon Geyser/LTBTS/Shell Side Arm BULLSHIT OH MY FUCKING GOD
        // Smart Category Attack
        var atkPrelim = attackSource.stats.atk;
        if (attackSource.boosts.atk === 0 || (isCritical && attackSource.boosts.atk < 0)) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else if (defender.hasAbility('Unaware')) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else {
            atkPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.atk, attackSource.boosts.atk);
        }
        if (attacker.hasAbility('Hustle')) {
            atkPrelimMods = (0, util_2.pokeRound)((atkPrelimMods * 3) / 2);
        }
        var atkPrelimMods = [];
        if (attacker.hasAbility('Guts') && attacker.status) {
            atkPrelimMods.push(6144);
        }
        else if (attacker.hasStatus('brn')) {
            atkPrelimMods.push(2048);
        }
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if ((attacker.hasAbility('Slow Start') && attacker.abilityOn) ||
            (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2)) {
            atkPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Gorilla Tactics') && !attacker.isDynamaxed)) {
            atkPrelimMods.push(6144);
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'atk') {
                atkPrelimMods.push(5325);
            }
        }
        if (attacker.hasAbility('Orichalcum Pulse') && field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) {
            atkPrelimMods.push(5461);
        }
        var isTabletsOfRuinActive = (defender.hasAbility('Tablets of Ruin') || field.isTabletsOfRuin) && !attacker.hasAbility('Tablets of Ruin');
        if (isTabletsOfRuinActive) {
            atkPrelimMods.push(3072);
        }
        if (attacker.hasAbility('Huge Power') || (attacker.hasAbility('Pure Power') && !field.hasTerrain('Psychic'))) {
            atkPrelimMods.push(8192);
        }
        if ((attacker.hasItem('Thick Club') && attacker.named('Cubone', 'Marowak', 'Marowak-Alola', 'Marowak-Alola-Totem')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            atkPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax &&
            (attacker.hasItem('Choice Band'))) {
            atkPrelimMods.push(6144);
        }
        if (field.hasTerrain('Underwater') && !move.hasType('Water') && attacker.hasAbility('Steelworker', 'Swift Swim')) {
            atkPrelimMods.push(6144);
        }
        atkPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((atkPrelim * (0, util_2.chainMods)(atkPrelimMods, 0, 524288)) / 4096)));
        var offense = atkPrelim;
        // Smart Category Defense
        var defPrelim = defender.stats.def;
        if (defender.boosts.def === 0 || (isCritical && defender.boosts.def > 0) || move.ignoreDefensive) {
            defPrelim = defender.rawStats.def;
        }
        else if (attacker.hasAbility('Unaware')) {
            defPrelim = defender.rawStats.def;
        }
        else {
            defPrelim = (0, util_2.getModifiedStat)(defender.rawStats.def, defender.boosts.def);
        }
        if (field.hasWeather('Snow') && defender.hasType('Ice')) {
            defPrelim = (0, util_2.pokeRound)((defPrelim * 3) / 2);
        }
        var defPrelimMods = [];
        if ((defender.hasAbility('Marvel Scale') && (defender.status || field.hasTerrain('Misty', 'Rainbow', 'Fairy Tale', "Dragon's Den", 'Starlight'))) ||
            (defender.hasAbility('Grass Pelt') && field.hasTerrain('Grassy', 'Forest'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasAbility('Fur Coat')) {
            defPrelimMods.push(8192);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'def') {
                defPrelimMods.push(5325);
            }
        }
        var isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        var isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && field.isWonderRoom) || (isSwordOfRuinActive && !field.isWonderRoom)) {
            defPrelimMods.push(3072);
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasItem('Metal Powder') && defender.named('Ditto')) {
            defPrelimMods.push(8192);
        }
        if (field.hasTerrain('Glitch') && move.named('Explosion', 'Self-Destruct')) {
            defPrelimMods.push(2048);
        }
        if (field.hasTerrain('Icy', 'Snowy Mt') && defender.hasType('Ice') && field.hasWeather('Hail')) {
            defPrelimMods.push(6144);
        }
        defPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((defPrelim * (0, util_2.chainMods)(defPrelimMods, 0, 524288)) / 4096)));
        var defense = defPrelim;
        // Glitch + Smart Category Spatk
        var spatkPrelim = attackSource.stats.spa;
        if (attackSource.boosts.spa === 0 || (isCritical && attackSource.boosts.spa < 0)) {
            spatkPrelim = attackSource.rawStats.spa;
        }
        else if (defender.hasAbility('Unaware')) {
            spatkPrelim = attackSource.rawStats.spa;
        }
        else {
            spatkPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.spa, attackSource.boosts.spa);
        }
        var spatkPrelimMods = [];
        if (attacker.hasAbility('Plus', 'Minus') && (attacker.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            spatkPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2) {
            spatkPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine'))) {
            spatkPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Pure Power') && field.hasTerrain('Psychic')) {
            spatkPrelimMods.push(8192);
        }
        var isVesselOfRuinActive = (defender.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) && !attacker.hasAbility('Vessel of Ruin');
        if (isVesselOfRuinActive) {
            spatkPrelimMods.push(3072);
        }
        if ((0, isQPActive)(attacker, field)) {
            if ((0, getQPBoostedStat)(attacker) === 'spa') {
                spatkPrelimMods.push(5325);
            }
        }
        if ((attacker.hasAbility('Hadron Engine') && field.hasTerrain('Electric'))) {
            spatkPrelimMods.push(5461);
        }
        if ((attacker.hasItem('Deep Sea Tooth') && attacker.named('Clamperl')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            spatkPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax && attacker.hasItem('Choice Specs')) {
            spatkPrelimMods.push(6144);
        }
        spatkPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spatkPrelim * (0, util_2.chainMods)(spatkPrelimMods, 1, 524288)) / 4096)));
        var spoffense = spatkPrelim;
        // Glitch Field Use Special Defense?
        var spdefAttackerPrelim = attacker.stats.spd;
        if (attacker.boosts.spd === 0 || (isCritical && attacker.boosts.spd > 0) || move.ignoreDefensive) {
            spdefAttackerPrelim = attacker.rawStats.spd;
        }
        else if (defender.hasAbility('Unaware')) {
            spdefAttackerPrelim = attacker.rawStats.spd;
        }
        else {
            spdefAttackerPrelim = (0, util_2.getModifiedStat)(attacker.rawStats.spd, attacker.boosts.spd);
        }
        if (field.hasWeather('Sand') && attacker.hasType('Rock')) {
            spdefAttackerPrelim = (0, util_2.pokeRound)((spdefAttackerPrelim * 3) / 2);
        }
        var spdefAttackerPrelimMods = [];
        if ((field.hasTerrain('Misty') && attacker.hasType('Fairy')) || (field.hasTerrain('Desert') && attacker.hasType('Ground'))) {
            spdefAttackerPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefAttackerPrelimMods.push(6144);
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefAttackerPrelimMods.push(6144);
        }
        isSwordOfRuinActive = (defender.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !attacker.hasAbility('Sword of Ruin');
        isBeadsOfRuinActive = (defender.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !attacker.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && !field.isWonderRoom) || (isSwordOfRuinActive && field.isWonderRoom)) {
            spdefAttackerPrelimMods.push(3072);
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'spd') {
                spdefAttackerPrelimMods.push(5324);
            }
        }
        if ((attacker.hasItem('Eviolite') && (attacker.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(attacker.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
                (attacker.hasItem('Assault Vest')) ||
                (attacker.hasItem('Light Ball', 'Pikanium Z') && attacker.name.includes('Pikachu'))) {
                spdefAttackerPrelimMods.push(6144);
            }
        else if (attacker.hasItem('Deep Sea Scale') && attacker.named('Clamperl')) {
            spdefAttackerPrelimMods.push(8192);
        }
        spdefAttackerPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spdefAttackerPrelim * (0, util_2.chainMods)(spdefAttackerPrelimMods, 1, 524288)) / 4096)));
        // Smart Category Special Defense
        var spdefDefenderPrelim = defender.stats.spd;
        if (defender.boosts.spd === 0 || (isCritical && defender.boosts.spd > 0) || move.ignoreDefensive) {
            spdefDefenderPrelim = defender.rawStats.spd;
        }
        else if (attacker.hasAbility('Unaware')) {
            spdefDefenderPrelim = defender.rawStats.spd;
        }
        else {
            spdefDefenderPrelim = (0, util_2.getModifiedStat)(defender.rawStats.spd, defender.boosts.spd);
        }
        if (field.hasWeather('Sand') && defender.hasType('Rock')) {
            spdefDefenderPrelim = (0, util_2.pokeRound)((spdefDefenderPrelim * 3) / 2);
        }
        var spdefDefenderPrelimMods = [];
        if ((field.hasTerrain('Misty') && defender.hasType('Fairy')) || (field.hasTerrain('Desert') && defender.hasType('Ground'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        if (defender.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !defender.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        if (field.defenderSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && !field.isWonderRoom) || (isSwordOfRuinActive && field.isWonderRoom)) {
            spdefDefenderPrelimMods.push(3072);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'spd') {
                spdefDefenderPrelimMods.push(5324);
            }
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (defender.hasItem('Assault Vest')) ||
            (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
            spdefDefenderPrelimMods.push(6144);
        }
        else if (defender.hasItem('Deep Sea Scale') && defender.named('Clamperl')) {
            spdefDefenderPrelimMods.push(8192);
        }
        spdefDefenderPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spdefDefenderPrelim * (0, util_2.chainMods)(spdefDefenderPrelimMods, 1, 524288)) / 4096)));
        var spdefense = spdefDefenderPrelim;
    }
    // Glitch Field Special Stat
    var attackerUseSpdef = false;
    if (field.hasTerrain('Glitch') && move.category === "Special") {
        attackerUseSpdef = spdefAttackerPrelim > spatkPrelim;
    }
    // ACTUAL FUCKING CODE
    var attackStat = 'atk';    
    move.named("Shell Side Arm") && (0, util_2.getShellSideArmCategory)(offense, defense, spoffense, spdefense) === "Physical" ? attackStat = "atk"
        : move.named('Photon Geyser', 'Light That Burns the Sky') && util_2.getPhotonGeyserCategory(offense, spoffense) === "Physical" ? attackStat = "atk"
            : move.named("Body Press") ? attackStat = "def"
                : attackerUseSpdef === true ? attackStat = 'spd'
                    : move.category === "Special" ? attackStat = "spa"
                        : attackStat = "atk";
    desc.attackEVs =
        move.named('Foul Play')
            ? (0, util_2.getStatDescriptionText)(gen, defender, attackStat, defender.nature)
            : (0, util_2.getStatDescriptionText)(gen, attacker, attackStat, attacker.nature);
    if (attackSource.boosts[attackStat] === 0 ||
        (isCritical && attackSource.boosts[attackStat] < 0)) {
        attack = attackSource.rawStats[attackStat];
    }
    else if (defender.hasAbility('Unaware')) {
        attack = attackSource.rawStats[attackStat];
        desc.defenderAbility = defender.ability;
    }
    else {
        attack = (0, util_2.getModifiedStat)(attackSource.rawStats[attackStat], attackSource.boosts[attackStat]);
        desc.attackBoost = attackSource.boosts[attackStat];
    }
    if (attacker.hasAbility('Hustle') && attackStat === 'atk' && move.category === 'Physical') {
        attack = (0, util_2.pokeRound)((attack * 3) / 2);
        desc.attackerAbility = attacker.ability;
    }
    var atMods = calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc, attackerUseSpdef);
    attack = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((attack * (0, util_2.chainMods)(atMods, 0, 524288)) / 4096)));
    return attack;
}
exports.calculateAttackSMSSSV = calculateAttackSMSSSV;
function calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc, attackerUseSpdef) {
    var _a;
    var atMods = [];
    var isSwordOfRuinActive = (defender.hasAbility('Sword of Ruin') || field.isSwordOfRuin) &&
        !attacker.hasAbility('Sword of Ruin');
    var isBeadsOfRuinActive = (defender.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) &&
        !attacker.hasAbility('Beads of Ruin');
    var isTabletsOfRuinActive = (defender.hasAbility('Tablets of Ruin') || field.isTabletsOfRuin) &&
        !attacker.hasAbility('Tablets of Ruin');
    var isVesselOfRuinActive = (defender.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) &&
        !attacker.hasAbility('Vessel of Ruin');
    if (move.category === 'Special' && attackerUseSpdef) {
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
            desc.weather = field.weather;
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atMods.push(6144);
            desc.weather = field.weather;
            desc.isFlowerGiftAttacker = true;
        }
        if (field.hasWeather('Sand') && attacker.hasType('Rock')) {
            atMods.push(6144);
            desc.weather = field.weather;
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'spd') {
                atMods.push(5325);
                desc.attackerAbility = attacker.ability;
            }
        }
        if ((field.isWonderRoom === false && isBeadsOfRuinActive) || (field.isWonderRoom === true && isSwordOfRuinActive)) {
            if (defender.hasAbility('Sword of Ruin') || defender.hasAbility('Beads of Ruin')) {
                desc.defenderAbility = defender.ability;
            }
            else {
                desc[field.isWonderRoom === false  ? 'isBeadsofRuin' : 'isSwordOfRuin'] = true;
            }   
            atMods.push(3072);
        }
        if ((attacker.hasItem('Eviolite') && (attacker.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(attacker.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (attacker.hasItem('Assault Vest')) ||
            (attacker.hasItem('Light Ball', 'Pikanium Z') && attacker.name.includes('Pikachu'))) {
            atMods.push(6144);
            desc.attackerItem = attacker.item;
        }
        if (attacker.hasItem('Deep Sea Scale') && attacker.named('Clamperl')) {
            atMods.push(8192);
            desc.attackerItem = attacker.item;
        }
    }
    else if (move.category === 'Special' && !attackerUseSpdef) {
        if (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2) {
            atMods.push(2048);
            desc.attackerAbility = attacker.ability;
        }
        if ((attacker.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine') && move.category === 'Special')) {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
            desc.weather = field.weather;
        }
        if ((attacker.hasAbility('Pure Power') && field.hasTerrain('Psychic'))) {
            atMods.push(8192);
            desc.attackerAbility = attacker.ability;
        }
        if (attacker.hasAbility('Plus', 'Minus') && (attacker.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'spa') {
                atMods.push(5325);
                desc.attackerAbility = attacker.ability;
            }
        }
        if (attacker.hasAbility('Hadron Engine') && field.hasTerrain('Electric')) {
            atMods.push(5461);
            desc.attackerAbility = attacker.ability;
        }
        if (isVesselOfRuinActive) {
            if (defender.hasAbility('Vessel of Ruin')) {
                desc.defenderAbility = defender.ability;
            }
            else {
                desc['isVesselOfRuin'] = true;
            }
            atMods.push(3072);
        }
        if ((attacker.hasItem('Deep Sea Tooth') && attacker.named('Clamperl')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            atMods.push(8192);
            desc.attackerItem = attacker.item;
        }
        if (!move.isZ && !move.isMax &&
            (attacker.hasItem('Choice Specs'))) {
            atMods.push(6144);
            desc.attackerItem = attacker.item;
        }
    }
    else if (move.category === 'Physical') {
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
            desc.weather = field.weather; 
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atMods.push(6144);
            desc.weather = field.weather;
            desc.isFlowerGiftAttacker = true;
        }
        if ((attacker.hasAbility('Slow Start') && attacker.abilityOn) ||
            (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2)) {
            atMods.push(2048);
            desc.attackerAbility = attacker.ability;
        }
        if (attacker.hasAbility('Guts') && attacker.status) {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
        }
        if (attacker.hasAbility('Huge Power') || (attacker.hasAbility('Pure Power') && !field.hasTerrain('Psychic'))) {
            atMods.push(8192);
            desc.attackerAbility = attacker.ability;
        }
        if ((attacker.hasAbility('Gorilla Tactics') && !attacker.isDynamaxed)) {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'atk') {
                atMods.push(5325);
                desc.attackerAbility = attacker.ability;
            }
        }
        if (attacker.hasAbility('Orichalcum Pulse') && field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) {
            atMods.push(5461);
            desc.attackerAbility = attacker.ability;
        }
        if (isTabletsOfRuinActive) {
            if (defender.hasAbility('Tablets of Ruin')) {
                desc.defenderAbility = defender.ability;
            }
            else {
                desc['isTabletsOfRuin'] = true;
            }
            atMods.push(3072);
        }
        if ((attacker.hasItem('Thick Club') && attacker.named('Cubone', 'Marowak', 'Marowak-Alola', 'Marowak-Alola-Totem')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            atMods.push(8192);
            desc.attackerItem = attacker.item;
        }
        if (!move.isZ && !move.isMax &&
            (attacker.hasItem('Choice Band'))) {
            atMods.push(6144);
            desc.attackerItem = attacker.item;
        }
        if (field.hasTerrain('Underwater') && !move.hasType('Water') && attacker.hasAbility('Steelworker', 'Swift Swim')) {
            atMods.push(6144);
        }
    }
    if ((attacker.curHP() <= attacker.maxHP() / 3 &&
            ((attacker.hasAbility('Overgrow') && move.hasType('Grass')) ||
                (attacker.hasAbility('Blaze') && move.hasType('Fire')) ||
                (attacker.hasAbility('Torrent') && move.hasType('Water')) ||
                (attacker.hasAbility('Swarm') && move.hasType('Bug')))) ||
        (attacker.hasAbility('Overgrow') && attacker.curHP() <= attacker.maxHP() * 0.75 && field.hasTerrain('Flower Garden 2') && move.hasType('Grass')) ||
        (attacker.hasAbility('Overgrow') && attacker.curHP() <= attacker.maxHP() && move.hasType('Grass') && field.hasTerrain('Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5')) ||
        (attacker.hasAbility('Swarm') && field.hasTerrain('Forest') && move.hasType('Bug')) ||
        (attacker.hasAbility('Overgrow') && field.hasTerrain('Forest') && move.hasType('Grass')) ||
        (attacker.hasAbility('Swarm') && move.hasType('Bug') && field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5')) ||
        (attacker.hasAbility('Blaze') && field.hasTerrain('Burning') && move.hasType('Fire')) ||
        (attacker.hasAbility('Torrent') && field.hasTerrain('Water', 'Underwater') && move.hasType('Water'))) {
        if (attacker.hasAbility('Swarm') && field.hasTerrain('Flower Garden 3')) {
            atMods.push(7373);
            desc.attackerAbility = attacker.ability;
        }
        else if (attacker.hasAbility('Swarm', 'Overgrow') && field.hasTerrain('Flower Garden 4')) {
            atMods.push(7373);
            desc.attackerAbility = attacker.ability;
        }
        else if (attacker.hasAbility('Swarm', 'Overgrow') && field.hasTerrain('Flower Garden 5')) {
            atMods.push(8192);
            desc.attackerAbility = attacker.ability;
        }
        else {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
        }
    }
    if (attacker.hasAbility('Flash Fire') && attacker.abilityOn && move.hasType('Fire')) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    if ((attacker.hasAbility('Dragon\'s Maw') && move.hasType('Dragon')) ||
        (attacker.hasAbility('Rocky Payload') && move.hasType('Rock'))) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    if (attacker.hasAbility('Transistor') && move.hasType('Electric')) {
        atMods.push(gen.num >= 9 ? 5325 : 6144);
        desc.attackerAbility = attacker.ability;
    }
    if (attacker.hasAbility('Steely Spirit') && move.hasType('Steel')) {
        atMods.push(field.hasTerrain('Fairy Tale') ? 8192 : 6144);
        desc.attackerAbility = attacker.ability;
    }
    if (field.attackerSide.isSteelySpirit && move.hasType('Steel')) {
        atMods.push(field.hasTerrain('Fairy Tale') ? 8192 : 6144);
        desc.isSteelySpiritAttacker = true;
    }
    if (attacker.hasAbility('Victory Star') && field.hasTerrain('Starlight', 'New World')) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    if (field.attackerSide.isVictoryStar && field.hasTerrain('Starlight', 'New World')) {
        atMods.push(6144);
        desc.isVictoryStarAttacker = true;
    }
    if (attacker.hasAbility('Propeller Tail') && field.hasTerrain('Water', 'Underwater') && move.priority >= 1) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    if ((field.hasTerrain('Mountain', 'Snowy Mt') && attacker.hasAbility('Long Reach')) ||
        (attacker.hasAbility('Queenly Majesty') && field.hasTerrain('Fairy Tale')) ||
        (attacker.hasAbility('Corrosion') && field.hasTerrain('Corrosive', 'Corrosive Mist'))) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    if (attacker.hasAbility('Stakeout') && attacker.abilityOn) {
        atMods.push(8192);
        desc.attackerAbility = attacker.ability;
    }
    if ((attacker.hasAbility('Water Bubble') && move.hasType('Water'))) {
        atMods.push(8192);
        desc.attackerAbility = attacker.ability;
    }
    if (attacker.hasAbility('Steelworker') && move.hasType('Steel')) {
        atMods.push(field.hasTerrain('Factory') ? 8192 : 6144);
        desc.attackerAbility = attacker.ability;
    }
    if (attacker.hasItem('Elemental Seed') && field.hasTerrain('Electric') && move.hasType('Electric')) {
        atMods.push(8192);
        desc.attackerItem = attacker.item;
    }
    if ((attacker.hasItem('Elemental Seed') && field.hasTerrain("Dragon's Den") && move.hasType('Fire')) && !(attacker.hasAbility('Flash Fire') && attacker.abilityOn && move.hasType('Fire'))) {
        atMods.push(6144);
        desc.attackerItem = attacker.item;
    }
    if (field.attackerSide.isFlowerGift &&
        field.hasWeather('Sun', 'Harsh Sunshine') &&
        move.category === 'Physical') {
        atMods.push(6144);
        desc.weather = field.weather;
        desc.isFlowerGiftAttacker = true;
    }
    if ((defender.hasAbility('Thick Fat') && move.hasType('Fire', 'Ice')) ||
        (defender.hasAbility('Water Bubble') && move.hasType('Fire')) ||
        (defender.hasAbility('Purifying Salt') && move.hasType('Ghost'))) {
        atMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    if (gen.num >= 9 && defender.hasAbility('Heatproof') && move.hasType('Fire')) {
        atMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    return atMods;
}
exports.calculateAtModsSMSSSV = calculateAtModsSMSSSV;
function calculateDefenseSMSSSV(gen, attacker, defender, move, field, desc, isCritical) {
    if (isCritical === void 0) { isCritical = false; }
    var defense;
    var attackSource = move.named('Foul Play') ? defender : attacker;
    if (1 === 1) {
        // Glitch Field + Photon Geyser/LTBTS/Shell Side Arm BULLSHIT OH MY FUCKING GOD
        // Smart Category Attack
        var atkPrelim = attackSource.stats.atk;
        if (attackSource.boosts.atk === 0 || (isCritical && attackSource.boosts.atk < 0)) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else if (defender.hasAbility('Unaware')) {
            atkPrelim = attackSource.rawStats.atk;
        }
        else {
            atkPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.atk, attackSource.boosts.atk);
        }
        if (attacker.hasAbility('Hustle')) {
            atkPrelimMods = (0, util_2.pokeRound)((atkPrelimMods * 3) / 2);
        }
        var atkPrelimMods = [];
        if (attacker.hasAbility('Guts') && attacker.status) {
            atkPrelimMods.push(6144);
        }
        else if (attacker.hasStatus('brn')) {
            atkPrelimMods.push(2048);
        }
        if (attacker.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if (field.attackerSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            atkPrelimMods.push(6144);
        }
        if ((attacker.hasAbility('Slow Start') && attacker.abilityOn) ||
            (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2)) {
            atkPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Gorilla Tactics') && !attacker.isDynamaxed)) {
            atkPrelimMods.push(6144);
        }
        if ((0, util_2.isQPActive)(attacker, field)) {
            if ((0, util_2.getQPBoostedStat)(attacker) === 'atk') {
                atkPrelimMods.push(5325);
            }
        }
        if (attacker.hasAbility('Orichalcum Pulse') && field.hasWeather('Sun', 'Harsh Sunshine') && !attacker.hasItem('Utility Umbrella')) {
            atkPrelimMods.push(5461);
        }
        var isTabletsOfRuinActive = (defender.hasAbility('Tablets of Ruin') || field.isTabletsOfRuin) && !attacker.hasAbility('Tablets of Ruin');
        if (isTabletsOfRuinActive) {
            atkPrelimMods.push(3072);
        }
        if (attacker.hasAbility('Huge Power') || (attacker.hasAbility('Pure Power') && !field.hasTerrain('Psychic'))) {
            atkPrelimMods.push(8192);
        }
        if ((attacker.hasItem('Thick Club') && attacker.named('Cubone', 'Marowak', 'Marowak-Alola', 'Marowak-Alola-Totem')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            atkPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax &&
            (attacker.hasItem('Choice Band'))) {
            atkPrelimMods.push(6144);
        }
        if (field.hasTerrain('Underwater') && !move.hasType('Water') && attacker.hasAbility('Steelworker', 'Swift Swim')) {
            atkPrelimMods.push(6144);
        }
        atkPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((atkPrelim * (0, util_2.chainMods)(atkPrelimMods, 0, 524288)) / 4096)));
        var offense = atkPrelim;
        // Smart Category Defense
        var defPrelim = defender.stats.def;
        if (defender.boosts.def === 0 || (isCritical && defender.boosts.def > 0) || move.ignoreDefensive) {
            defPrelim = defender.rawStats.def;
        }
        else if (attacker.hasAbility('Unaware')) {
            defPrelim = defender.rawStats.def;
        }
        else {
            defPrelim = (0, util_2.getModifiedStat)(defender.rawStats.def, defender.boosts.def);
        }
        if (field.hasWeather('Snow') && defender.hasType('Ice')) {
            defPrelim = (0, util_2.pokeRound)((defPrelim * 3) / 2);
        }
        var defPrelimMods = [];
        if ((defender.hasAbility('Marvel Scale') && (defender.status || field.hasTerrain('Misty', 'Rainbow', 'Fairy Tale', "Dragon's Den", 'Starlight'))) ||
            (defender.hasAbility('Grass Pelt') && field.hasTerrain('Grassy', 'Forest'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasAbility('Fur Coat')) {
            defPrelimMods.push(8192);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'def') {
                defPrelimMods.push(5325);
            }
        }
        var isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        var isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && field.isWonderRoom) || (isSwordOfRuinActive && !field.isWonderRoom)) {
            defPrelimMods.push(3072);
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
            defPrelimMods.push(6144);
        }
        if (defender.hasItem('Metal Powder') && defender.named('Ditto')) {
            defPrelimMods.push(8192);
        }
        if (field.hasTerrain('Glitch') && move.named('Explosion', 'Self-Destruct')) {
            defPrelimMods.push(2048);
        }
        if (field.hasTerrain('Icy', 'Snowy Mt') && defender.hasType('Ice') && field.hasWeather('Hail')) {
            defPrelimMods.push(6144);
        }
        defPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((defPrelim * (0, util_2.chainMods)(defPrelimMods, 0, 524288)) / 4096)));
        var smartdefense = defPrelim;
        // Smart Category Spatk
        var spatkAttackerPrelim = attackSource.stats.spa;
        if (attackSource.boosts.spa === 0 || (isCritical && attackSource.boosts.spa < 0)) {
            spatkAttackerPrelim = attackSource.rawStats.spa;
        }
        else if (defender.hasAbility('Unaware')) {
            spatkAttackerPrelim = attackSource.rawStats.spa;
        }
        else {
            spatkAttackerPrelim = (0, util_2.getModifiedStat)(attackSource.rawStats.spa, attackSource.boosts.spa);
        }
        var spatkAttackerPrelimMods = [];
        if (attacker.hasAbility('Plus', 'Minus') && (attacker.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            spatkAttackerPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Defeatist') && attacker.curHP() <= attacker.maxHP() / 2) {
            spatkAttackerPrelimMods.push(2048);
        }
        if ((attacker.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine'))) {
            spatkAttackerPrelimMods.push(6144);
        }
        if (attacker.hasAbility('Pure Power') && field.hasTerrain('Psychic')) {
            spatkAttackerPrelimMods.push(8192);
        }
        var isVesselOfRuinActive = (defender.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) && !attacker.hasAbility('Vessel of Ruin');
        if (isVesselOfRuinActive) {
            spatkAttackerPrelimMods.push(3072);
        }
        if ((0, isQPActive)(attacker, field)) {
            if ((0, getQPBoostedStat)(attacker) === 'spa') {
                spatkAttackerPrelimMods.push(5325);
            }
        }
        if ((attacker.hasAbility('Hadron Engine') && field.hasTerrain('Electric'))) {
            spatkAttackerPrelimMods.push(5461);
        }
        if ((attacker.hasItem('Deep Sea Tooth') && attacker.named('Clamperl')) ||
            (attacker.hasItem('Light Ball') && attacker.name.includes('Pikachu') && !move.isZ)) {
            spatkAttackerPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax && attacker.hasItem('Choice Specs')) {
            spatkAttackerPrelimMods.push(6144);
        }
        spatkAttackerPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spatkAttackerPrelim * (0, util_2.chainMods)(spatkAttackerPrelimMods, 1, 524288)) / 4096)));
        var spoffense = spatkAttackerPrelim;
        // Glitch Defender Use Special Attack?
        var spatkDefenderPrelim = defender.stats.spa;
        if (defender.boosts.spa === 0 || (isCritical && defender.boosts.spa < 0)) {
            spatkDefenderPrelim = defender.rawStats.spa;
        }
        else if (attacker.hasAbility('Unaware')) {
            spatkDefenderPrelim = defender.rawStats.spa;
        }
        else {
            spatkDefenderPrelim = (0, util_2.getModifiedStat)(defender.rawStats.spa, defender.boosts.spa);
        }
        var spatkDefenderPrelimMods = [];
        if (defender.hasAbility('Plus', 'Minus') && (defender.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            spatkDefenderPrelimMods.push(6144);
        }
        if (defender.hasAbility('Defeatist') && defender.curHP() <= defender.maxHP() / 2) {
            spatkDefenderPrelimMods.push(2048);
        }
        if ((defender.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine'))) {
            spatkDefenderPrelimMods.push(6144);
        }
        if (defender.hasAbility('Pure Power') && field.hasTerrain('Psychic')) {
            spatkDefenderPrelimMods.push(8192);
        }
        var isVesselOfRuinActive = (attacker.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) && !defender.hasAbility('Vessel of Ruin');
        if (isVesselOfRuinActive) {
            spatkDefenderPrelimMods.push(3072);
        }
        if ((0, isQPActive)(defender, field)) {
            if ((0, getQPBoostedStat)(defender) === 'spa') {
                spatkDefenderPrelimMods.push(5325);
            }
        }
        if ((defender.hasAbility('Hadron Engine') && field.hasTerrain('Electric'))) {
            spatkDefenderPrelimMods.push(5461);
        }
        if ((defender.hasItem('Deep Sea Tooth') && defender.named('Clamperl')) ||
            (defender.hasItem('Light Ball') && defender.name.includes('Pikachu') && !move.isZ)) {
            spatkDefenderPrelimMods.push(8192);
        }
        if (!move.isZ && !move.isMax && defender.hasItem('Choice Specs')) {
            spatkDefenderPrelimMods.push(6144);
        }
        spatkDefenderPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spatkDefenderPrelim * (0, util_2.chainMods)(spatkDefenderPrelimMods, 1, 524288)) / 4096)));
        // Glitch + Smart Category Spdef
        var spdefPrelim = defender.stats.spd;
        if (defender.boosts.spd === 0 || (isCritical && defender.boosts.spd > 0) || move.ignoreDefensive) {
            spdefPrelim = defender.rawStats.spd;
        }
        else if (defender.hasAbility('Unaware')) {
            spdefPrelim = defender.rawStats.spd;
        }
        else {
            spdefPrelim = (0, util_2.getModifiedStat)(defender.rawStats.spd, defender.boosts.spd);
        }
        if (field.hasWeather('Sand') && defender.hasType('Rock')) {
            spdefPrelim = (0, util_2.pokeRound)((spdefPrelim * 3) / 2);
        }
        var spdefPrelimMods = [];
        if ((field.hasTerrain('Misty') && defender.hasType('Fairy')) || (field.hasTerrain('Desert') && defender.hasType('Ground'))) {
            spdefPrelimMods.push(6144);
        }
        if (defender.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !defender.hasItem('Utility Umbrella')) 
            || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefPrelimMods.push(6144);
        }
        if (field.defenderSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') 
            || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            spdefPrelimMods.push(6144);
        }
        isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && !field.isWonderRoom) || (isSwordOfRuinActive && field.isWonderRoom)) {
            spdefPrelimMods.push(3072);
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'spd') {
                spdefPrelimMods.push(5324);
            }
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
                (defender.hasItem('Assault Vest')) ||
                (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
                spdefPrelimMods.push(6144);
            }
        else if (defender.hasItem('Deep Sea Scale') && defender.named('Clamperl')) {
            spdefPrelimMods.push(8192);
        }
        spdefPrelim = (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((spdefPrelim * (0, util_2.chainMods)(spdefPrelimMods, 1, 524288)) / 4096)));
        var spdefense = spdefPrelim;
    }
    var defenderUseSpatk = false;
    if (field.hasTerrain('Glitch')) {
        defenderUseSpatk = spatkDefenderPrelim > spdefPrelim;
    }
    // ACTUAL FUCKING CODE
    var hitsPhysical = move.overrideDefensiveStat === "def" || move.category === "Physical" || (move.named("Shell Side Arm") && (0, util_2.getShellSideArmCategory)(offense, smartdefense, spoffense, spdefense) === "Physical") || (move.named('Photon Geyser', 'Light That Burns the Sky') && util_2.getPhotonGeyserCategory(offense, spoffense) === "Physical");
    var defenseStat = hitsPhysical ? 'def'
        : (!hitsPhysical && field.hasTerrain('Glitch') && defenderUseSpatk) ? 'spa'
            : 'spd';
    desc.defenseEVs = (0, util_2.getStatDescriptionText)(gen, defender, defenseStat, defender.nature);
    if (defender.boosts[defenseStat] === 0 ||
        (isCritical && defender.boosts[defenseStat] > 0) ||
        move.ignoreDefensive) {
        defense = defender.rawStats[defenseStat];
    }
    else if (attacker.hasAbility('Unaware')) {
        defense = defender.rawStats[defenseStat];
        desc.attackerAbility = attacker.ability;
    }
    else {
        defense = (0, util_2.getModifiedStat)(defender.rawStats[defenseStat], defender.boosts[defenseStat]);
        desc.defenseBoost = defender.boosts[defenseStat];
    }
    if (field.hasWeather('Sand') && defender.hasType('Rock') && !hitsPhysical) {
        defense = (0, util_2.pokeRound)((defense * 3) / 2);
        desc.weather = field.weather;
    }
    if (field.hasWeather('Snow') && defender.hasType('Ice') && hitsPhysical) {
        defense = (0, util_2.pokeRound)((defense * 3) / 2);
        desc.weather = field.weather;
    }
    var dfMods = calculateDfModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical, defenderUseSpatk);
    return (0, util_2.OF16)(Math.max(1, (0, util_2.pokeRound)((defense * (0, util_2.chainMods)(dfMods, 0, 524288)) / 4096)));
}
exports.calculateDefenseSMSSSV = calculateDefenseSMSSSV;
function calculateDfModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical, defenderUseSpatk) {
    var _a;
    if (isCritical === void 0) { isCritical = false; }
    if (hitsPhysical === void 0) { hitsPhysical = false; }
    var dfMods = [];
    if (hitsPhysical === false && defenderUseSpatk) {
       if (defender.hasAbility('Plus', 'Minus') && (defender.abilityOn || field.hasTerrain('Short-Circuit 0.5', 'Short-Circuit 0.8', 'Short-Circuit 1.2', 'Short-Circuit 1.5', 'Short-Circuit 2'))) {
            dfMods.push(6144);
            desc.defenderAbility = defender.ability;
        }
        if (defender.hasAbility('Defeatist') && defender.curHP() <= defender.maxHP() / 2) {
            dfMods.push(2048);
            desc.defenderAbility = defender.ability;
        }
        if ((defender.hasAbility('Solar Power') && field.hasWeather('Sun', 'Harsh Sunshine'))) {
            dfMods.push(6144);
            desc.defenderAbility = defender.ability;
            desc.weather = field.weather;
        }
        if (defender.hasAbility('Pure Power') && field.hasTerrain('Psychic')) {
            dfMods.push(8192);
            desc.defenderAbility = defender.ability;
        }
        var isVesselOfRuinActive = (attacker.hasAbility('Vessel of Ruin') || field.isVesselOfRuin) && !defender.hasAbility('Vessel of Ruin');
        if (isVesselOfRuinActive) {
            dfMods.push(3072);
        }
        if ((0, isQPActive)(defender, field)) {
            if ((0, getQPBoostedStat)(defender) === 'spa') {
                dfMods.push(5325);
                desc.defenderAbility = defender.ability;
            }
        }
        if ((defender.hasAbility('Hadron Engine') && field.hasTerrain('Electric'))) {
            dfMods.push(5461);
            desc.defenderAbility = defender.ability;
        }
        if ((defender.hasItem('Deep Sea Tooth') && defender.named('Clamperl')) ||
            (defender.hasItem('Light Ball') && defender.name.includes('Pikachu') && !move.isZ)) {
            dfMods.push(8192);
            desc.defenderItem = defender.item;
        }
        if (!move.isZ && !move.isMax && defender.hasItem('Choice Specs')) {
            dfMods.push(6144);
            desc.defenderItem = defender.item;
        } 
    }
    else if (hitsPhysical === false && defenderUseSpatk === false) {
        if ((field.hasTerrain('Misty') && defender.hasType('Fairy')) || (field.hasTerrain('Desert') && defender.hasType('Ground'))) {
            dfMods.push(6144);
        }
        if (defender.hasAbility('Flower Gift') && ((field.hasWeather('Sun', 'Harsh Sunshine') && !defender.hasItem('Utility Umbrella')) || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            dfMods.push(6144);
            desc.defenderAbility = defender.ability;
            desc.weather = field.weather;
        }
        if (field.defenderSide.isFlowerGift && (field.hasWeather('Sun', 'Harsh Sunshine') || field.hasTerrain('Flower Garden 1', 'Flower Garden 2', 'Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5'))) {
            dfMods.push(6144);
            desc.weather = field.weather;
            desc.isFlowerGiftDefender = true;
        }
        var isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        var isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && !field.isWonderRoom) || (isSwordOfRuinActive && field.isWonderRoom)) {
            dfMods.push(3072);
            if (attacker.hasAbility('Sword of Ruin') || attacker.hasAbility('Beads of Ruin')) {
                desc.defenderAbility = defender.ability;
            }
            else {
                desc[field.isWonderRoom ? 'isSwordOfRuin' : 'isBeadsOfRuin'] = true;
            }
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'spd') {
                dfMods.push(5324);
                desc.defenderAbility = defender.ability;
            }
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
                (defender.hasItem('Assault Vest')) ||
                (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
                dfMods.push(6144);
                desc.defenderItem = defender.item;
            }
        else if (defender.hasItem('Deep Sea Scale') && defender.named('Clamperl')) {
            dfMods.push(8192);
            desc.defenderItem = defender.item;
        }
    }
    else if (hitsPhysical) {
        if ((defender.hasAbility('Marvel Scale') && (defender.status || field.hasTerrain('Misty', 'Rainbow', 'Fairy Tale', "Dragon's Den", 'Starlight'))) ||
            (defender.hasAbility('Grass Pelt') && field.hasTerrain('Grassy', 'Forest'))) {
            dfMods.push(6144);
            desc.defenderAbility = defender.ability;
        }
        if (defender.hasAbility('Fur Coat')) {
            dfMods.push(8192);
            desc.defenderAbility = defender.ability;
        }
        if ((0, util_2.isQPActive)(defender, field)) {
            if ((0, util_2.getQPBoostedStat)(defender) === 'def') {
                dfMods.push(5325);
                desc.defenderAbility = defender.ability;
            }
        }
        isSwordOfRuinActive = (attacker.hasAbility('Sword of Ruin') || field.isSwordOfRuin) && !defender.hasAbility('Sword of Ruin');
        isBeadsOfRuinActive = (attacker.hasAbility('Beads of Ruin') || field.isBeadsOfRuin) && !defender.hasAbility('Beads of Ruin');
        if ((isBeadsOfRuinActive && field.isWonderRoom) || (isSwordOfRuinActive && !field.isWonderRoom)) {
            dfMods.push(3072);
            if (attacker.hasAbility('Sword of Ruin') || attacker.hasAbility('Beads of Ruin')) {
                desc.defenderAbility = defender.ability;
            }
            else {
                desc[!field.isWonderRoom ? 'isSwordOfRuin' : 'isBeadsOfRuin'] = true;
            }
        }
        if ((defender.hasItem('Eviolite') && (defender.name === 'Dipplin' || ((_a = gen.species.get((0, util_1.toID)(defender.name))) === null || _a === void 0 ? void 0 : _a.nfe))) ||
            (defender.hasItem('Light Ball', 'Pikanium Z') && defender.name.includes('Pikachu'))) {
            dfMods.push(6144);
            desc.defenderItem = defender.item;
        }
        if (defender.hasItem('Metal Powder') && defender.named('Ditto')) {
            dfMods.push(8192);
            desc.defenderItem = defender.item;
        }
        if (field.hasTerrain('Glitch') && move.named('Explosion', 'Self-Destruct')) {
            dfMods.push(2048);
        }
        if (field.hasTerrain('Icy', 'Snowy Mt') && defender.hasType('Ice') && field.hasWeather('Hail')) {
            dfMods.push(6144);
            desc.weather = field.weather;
        }
    }
    if (field.hasTerrain('New World') && !(0, util_2.isGrounded)(defender, field)) {
        dfMods.push(3686);
    }
    if (field.hasTerrain('Dragon\'s Den') && defender.hasType('Dragon')) {
        dfMods.push(5325);
    }
    if (field.hasTerrain('Dark Crystal') && (defender.hasType('Dark', 'Ghost'))) {
        dfMods.push(6144);
    }
    if (defender.hasAbility('Prism Armor')) {
        if (field.hasTerrain('Dark Crystal', 'Rainbow', 'Crystal Fire', 'Crystal Water', 'Crystal Grass', 'Crystal Psychic')) {
            dfMods.push(5448);
            desc.defenderAbility = defender.ability;
        }
    }
    if ((defender.hasAbility('Punk Rock') && move.flags.sound) ||
        (defender.hasAbility('Ice Scales') && move.category === 'Special')) {
        dfMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    return dfMods;
}
exports.calculateDfModsSMSSSV = calculateDfModsSMSSSV;
function calculateBaseDamageSMSSSV(gen, attacker, defender, basePower, attack, defense, move, field, desc, isCritical) {
    if (isCritical === void 0) { isCritical = false; }
    var baseDamage = (0, util_2.getBaseDamage)(attacker.level, basePower, attack, defense);
    var isSpread = field.gameType !== 'Singles' &&
        ['allAdjacent', 'allAdjacentFoes'].includes(move.target);
    if (isSpread) {
        var spreadMod = (field.hasTerrain('Chess Board') && attacker.hasPiece('N')) ? 5120 : 3072
        baseDamage = (0, util_2.pokeRound)((0, util_2.OF32)(baseDamage * spreadMod) / 4096);
    }
    if (attacker.hasAbility('Parental Bond (Child)')) {
        baseDamage = (0, util_2.pokeRound)((0, util_2.OF32)(baseDamage * 1024) / 4096);
    }
    if (field.hasWeather('Sun') && move.named('Hydro Steam') && !attacker.hasItem('Utility Umbrella')) {
        baseDamage = (0, util_2.pokeRound)((0, util_2.OF32)(baseDamage * 6144) / 4096);
        desc.weather = field.weather;
    }
    else if (!defender.hasItem('Utility Umbrella')) {
        if ((field.hasWeather('Sun', 'Harsh Sunshine') && move.hasType('Fire')) ||
            (field.hasWeather('Rain', 'Heavy Rain') && move.hasType('Water'))) {
            baseDamage = (0, util_2.pokeRound)((0, util_2.OF32)(baseDamage * 6144) / 4096);
            desc.weather = field.weather;
        }
        else if ((field.hasWeather('Sun') && move.hasType('Water')) ||
            (field.hasWeather('Rain') && move.hasType('Fire'))) {
            baseDamage = (0, util_2.pokeRound)((0, util_2.OF32)(baseDamage * 2048) / 4096);
            desc.weather = field.weather;
        }
    }
    if (isCritical) {
        baseDamage = Math.floor((0, util_2.OF32)(baseDamage * 1.5));
        desc.isCritical = isCritical;
    }
    return baseDamage;
}
function calculateFinalModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness, hitCount) {
    if (isCritical === void 0) { isCritical = false; }
    if (hitCount === void 0) { hitCount = 0; }
    var finalMods = [];
    if (field.defenderSide.isReflect && move.category === 'Physical' &&
        !isCritical && !field.defenderSide.isAuroraVeil) {
        finalMods.push(field.gameType !== 'Singles' ? 2732 : 2048);
        desc.isReflect = true;
    }
    else if (field.defenderSide.isLightScreen && move.category === 'Special' &&
        !isCritical && !field.defenderSide.isAuroraVeil) {
        finalMods.push(field.gameType !== 'Singles' ? 2732 : 2048);
        desc.isLightScreen = true;
    }
    if (field.defenderSide.isAuroraVeil && !isCritical) {
        finalMods.push(field.gameType !== 'Singles' ? 2732 : 2048);
        desc.isAuroraVeil = true;
    }
    if (attacker.hasAbility('Neuroforce') && typeEffectiveness > 1) {
        finalMods.push(5120);
        desc.attackerAbility = attacker.ability;
    }
    else if (attacker.hasAbility('Sniper') && isCritical) {
        finalMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    else if (attacker.hasAbility('Tinted Lens') && typeEffectiveness < 1) {
        finalMods.push(8192);
        desc.attackerAbility = attacker.ability;
    }
    if (defender.isDynamaxed && move.named('Dynamax Cannon', 'Behemoth Blade', 'Behemoth Bash')) {
        finalMods.push(8192);
    }
    if (defender.hasAbility('Multiscale') && defender.curHP() === defender.maxHP() && hitCount === 0 &&
        (!field.defenderSide.isSR && (!field.defenderSide.spikes || defender.hasType('Flying')) || defender.hasItem('Heavy-Duty Boots')) && !attacker.hasAbility('Parental Bond (Child)')) {
        finalMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    if (defender.hasAbility('Fluffy') && move.flags.contact && !attacker.hasAbility('Long Reach')) {
        finalMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    if (defender.hasAbility('Solid Rock', 'Filter', 'Prism Armor') && typeEffectiveness > 1) {
        finalMods.push(3072);
        desc.defenderAbility = defender.ability;
    }
    if (defender.hasAbility('Shadow Shield')) {
        if (field.hasTerrain('Dark Crystal', 'Starlight', 'New World') && typeEffectiveness > 1) {
                finalMods.push(3072);
                desc.defenderAbility = defender.ability;
            }
        if (defender.curHP() === defender.maxHP() && hitCount === 0 &&
        (!field.defenderSide.isSR && (!field.defenderSide.spikes || defender.hasType('Flying')) || defender.hasItem('Heavy-Duty Boots')) && !attacker.hasAbility('Parental Bond (Child)')) {
            finalMods.push(2048);
            desc.defenderAbility = defender.ability;
        }
    }
    if (defender.hasAbility('Flower Veil') && field.hasTerrain('Flower Garden 3', 'Flower Garden 4', 'Flower Garden 5')) {
        finalMods.push(3072);
        desc.defenderAbility = defender.ability;
        desc.terrain = field.terrain;
    }
    if ((defender.hasAbility('Pastel Veil') || field.defenderSide.isPastelVeil) && field.hasTerrain('Misty', 'Rainbow') && move.hasType('Poison')) {
        finalMods.push(2048);
        field.defenderSide.isPastelVeil ? desc.isPastelVeil = true : desc.defenderAbility = defender.ability;
        desc.terrain = field.terrain;
    }
    if (field.defenderSide.isFriendGuard) {
        finalMods.push(3072);
        desc.isFriendGuard = true;
    }
    if (defender.hasAbility('Fluffy') && move.hasType('Fire')) {
        finalMods.push(8192);
        desc.defenderAbility = defender.ability;
    }
    if (attacker.hasItem('Expert Belt') && typeEffectiveness > 1 && !move.isZ) {
        finalMods.push(4915);
        desc.attackerItem = attacker.item;
    }
    else if (attacker.hasItem('Life Orb')) {
        finalMods.push(5324);
        desc.attackerItem = attacker.item;
    }
    else if (attacker.hasItem('Metronome') && move.timesUsedWithMetronome >= 1) {
        var timesUsedWithMetronome = Math.floor(move.timesUsedWithMetronome);
        if (timesUsedWithMetronome <= 4) {
            finalMods.push(4096 + timesUsedWithMetronome * 819);
        }
        else {
            finalMods.push(8192);
        }
        desc.attackerItem = attacker.item;
    }
    if (move.hasType((0, items_1.getBerryResistType)(defender.item)) &&
        (typeEffectiveness > 1 || move.hasType('Normal')) &&
        hitCount === 0 &&
        !attacker.hasAbility('Unnerve', 'As One (Glastrier)', 'As One (Spectrier)')) {
        if (defender.hasAbility('Ripen')) {
            finalMods.push(1024);
        }
        else {
            finalMods.push(2048);
        }
        desc.defenderItem = defender.item;
    }
    return finalMods;
}
exports.calculateFinalModsSMSSSV = calculateFinalModsSMSSSV;
function hasTerrainSeed(pokemon) {
    return pokemon.hasItem('Electric Seed', 'Misty Seed', 'Grassy Seed', 'Psychic Seed');
}
//# sourceMappingURL=gen9.js.map