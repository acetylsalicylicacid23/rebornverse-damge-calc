class PokeBattle_Battler
  ...
  SwitchEff = [
    :AfterYou, :AquaRing, :BindingBand, :BeakBlast, :BurnUp, :Curse,
    ...
    :MiracleEye, :Nightmare, :NoRetreat, :ParentalBond, :Powder, :MicleBerry,
    :PowerTrick, :QuickDrawSnipe, :Quash, :RagePowder, :Rage, :Roost,
    :ShellTrap, :ShellTrapAttacked, :SkyDrop, :SmackDown, :Snatch, :Tantrum, :TarShot, :Torment,
  ...
  # position vars
  PosEff = [
    :Attract, :BideTarget, :Counter, :MirrorCoat, :LeechSeed, :LockOnPos, :MeanLook,
    :MirrorCoatTarget, :CounterTarget, :MultiTurnUser, :Octolock, :Petrification,
  ]

  # other counter vars
  CountEff = [
    :BideDamage, :Damage, :FocusEnergy, :ProtectRate, :Rollout, :EchoedVoice, :SpeedSwap,
    :Stockpile, :StockpileDef, :StockpileSpDef, :WeightModifier,
  ]

  # weirdo shit (usually move objects)
  OtherEff = [
    :ChoicedMove, :DisableMove, :EncoreMove, :MagicBounce, :Protect, :BouncedMove, # :TwoTurnAttack
  ]
  ...
  PermEff = [
    :FutureSight, :FutureSightMove, :FutureSightUser, :FutureSightPokemonIndex,
  ...
  # All ability changing effects should use this to change abilities in battle
  def changeAbility(newability)
    oldability = @ability
    ...
    elsif fixedabilities.include?(oldability) && !fixedabilities.include?(newability)
      @ability = nil
      @backupability = newability
    ...
    if @effects[:Illusion] != nil && @ability != :ILLUSION && @crested != :ZOROARK
      # Animation should go here
      @effects[:Illusion] = nil
      @battle.scene.pbChangePokemon(self, self.pokemon)
      @battle.pbDisplay(_INTL("{1}'s {2} wore off!", self.pbThis, getAbilityName(:ILLUSION)))
    end
    @battle.neutralizingGasDisable(@index) if oldability == :NEUTRALIZINGGAS && @ability != :NEUTRALIZINGGAS

    @effects[:Illusion].ability = @ability if @effects[:Illusion] && @effects[:Illusion] != self
  end
  ...
  def pbCompatibleZMoveFromMove?(move, moveindex = false)
    ...
    return true if PBStuff::ZCRYSTALTOTYPE[pkmn.item] == move.type
  ...
  def pbUpdate(fullchange = false)
    ...
    @type1   = @pokemon.type1
    @type2   = @pokemon.type2
    self.changeAbility(@pokemon.ability) if @crested != :SILVALLY && @crested != :ZOROARK
  end
  ...
  def isAirborne?(moldbreakcheck: false)
    return false if self.item == :IRONBALL && @battle.FE != :DEEPEARTH
    ...
    if !moldbreakcheck || !self.moldbroken
      return true if [:LEVITATE, :SOLARIDOL, :LUNARIDOL].include?(self.ability)
    end
  ...
  def pbSpeed()
    ...
    speed *= 2   if self.telepathySpeedActive?
    ...
    speed *= 0.5 if self.ability == :SLOWSTART && self.turncount < 5 && @battle.FE != :DEEPEARTH
    speed *= 1.5 if self.ability == :QUARKDRIVE && self.effects[:Quarkdrive][0] == PBStats::SPEED
  ...
  def pbFaint(showMessage = true)
      ...
      if self.shieldCount > 0 && self.onBreakEffects
        ...
        case hpthreshold
          when 0
            ...
            if boss.sosDetails
              @battle.pbBossSOS(@battle.battlers, true)
            end
          when 0.1
            self.pbRecoverHP(self.totalhp, true)
            if onBreakdata
    ...
    if (pbPartner.ability == :POWEROFALCHEMY || pbPartner.ability == :RECEIVER) && pbPartner.hp > 0
      partnerability = self.ability.nil? ? self.backupability : self.ability
      if !PBStuff::ABILITYBLACKLIST.include?(partnerability) && !partnerability.nil?
        abilityname = getAbilityName(partnerability)
        if pbPartner.ability == :POWEROFALCHEMY
        ...
        pbPartner.changeAbility(partnerability)
    ...
    for i in @battle.battlers
      ...
      if i.ability == :SOULHEART
        if !i.pbTooHigh?(PBStats::SPATK) || ([:MISTY, :RAINBOW, :FAIRYTALE].include?(@battle.FE) && !i.pbTooHigh?(PBStats::SPDEF))
          @battle.pbDisplay(_INTL("{1}'s {2} activated!", i.pbThis, getAbilityName(i.ability)))
          i.pbIncreaseStat(PBStats::SPATK, 1) if !i.pbTooHigh?(PBStats::SPATK)
          i.pbIncreaseStat(PBStats::SPDEF, 1) if [:MISTY, :RAINBOW, :FAIRYTALE].include?(@battle.FE) && !i.pbTooHigh?(PBStats::SPDEF)
        end
      end
    end
    ...
    if droprelease != nil
      ...
      @battle.pbDisplay(_INTL("{1} was freed from the Sky Drop!", oppmon.pbThis))
    end
    @battle.party2.pop if self.issossmon
    # set ace message flag
    if self.index.odd? && !@battle.pbIsWild? && !@battle.opponent.is_a?(Array) && @battle.pbPokemonCount(@battle.party2) == 1 && !@battle.ace_message_handled
  ...
  def pbCheckStance(basemove)
    pbZoroCrestForms(basemove) if Rejuv
    # Aegislash
    transformed = false
    if self.pokemon && self.pokemon.species == :AEGISLASH && !self.isFainted?
      if self.ability == :STANCECHANGE && !@effects[:Transform]
        ...
        # in Blade Forme and used King's Shield
          ...
          self.form = 0
          self.index.even? ? @battle.pbCommonAnimation("StanceProtect", self, nil) : @battle.pbCommonAnimation("StanceProtectOpp", self, nil)
          transformed = true
        end
        if transformed == true
  ...
  def pbCheckFormRoundEnd
    # Wishiwashi
    if self.pokemon&.species == :WISHIWASHI && !self.isFainted?
    ...
    if self.pokemon&.species == :DARMANITAN && !self.isFainted?
      if self.ability == :ZENMODE || self.crested == :DARMANITAN
        ...
        if self.crested == :DARMANITAN && self.form == 0
          ...
          if self.hp < zenHP || (unovaZenFields.include?(@battle.FE) && self.form < 2) || (galarZenFields.include?(@battle.FE) && self.form > 1)
    ...
    # Zygarde
    if self.pokemon&.species == :ZYGARDE && !self.isFainted? && !self.effects[:Transform]
    ...
    if (((self.ability == :RKSSYSTEM && self.pokemon&.species == :SILVALLY) || self.crested == :SILVALLY) ||
      ...
      if @battle.field.effect == :NEWWORLD
        ...
        return
      elsif !(self.species == :SILVALLY && oldform == 19) # exception for Vulpes's dev Silvally
  ...
  def pbResetForm
    ...
    # ResetForm should not call into pbUpdate for the battler directly, as this causes Natural Cure and Regenerator to not interact correctly
    # and it's not necessary as ResetForm is only called when a battler leaves the field (faint, switch, battle end)
    # pbUpdate(true)
  end
  ...
  def pbAbilitiesOnField(onactive)
    ...
    if @battle.FE == :MIRROR
      if !pbTooHigh?(PBStats::EVASION)
        if (self.ability == :SNOWCLOAK || self.ability == :SANDVEIL || self.ability == :TANGLEDFEET || self.ability == :MAGICBOUNCE || self.ability == :COLORCHANGE) && onactive
          ...
          @battle.pbCommonAnimation("StatUp", self, nil)
          @battle.pbDisplay(_INTL("{1}'s Ability sharply boosted its Evasion!", pbThis))
        end
    ...
    # Flower Garden Entry
    flowergardenabils = [:FLOWERGIFT, :FLOWERVEIL, :DROUGHT, :DRIZZLE]
    flowergardenabils.push(:GRASSYSURGE) if Rejuv
    if @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN, 1, 4) && flowergardenabils.include?(ability) && onactive
    ...
    if [:MISTY, :CORROSIVEMIST].include?(@battle.FE) || @battle.state.effects[:MISTY] > 0
  ...
  def pbAbilitiesOnSwitchIn(onactive)
    ...
    if self.ability == :NEUTRALIZINGGAS && onactive
      @battle.pbDisplay(_INTL("{1}'s gas neutralized all other Pokémon's Abilities!", pbThis))
      for i in 0...4
        if !(PBStuff::FIXEDABILITIES + [:NEUTRALIZINGGAS]).include?(@battle.battlers[i].ability)
          @battle.battlers[i].changeAbility(:suppress)
        end
      end
    end
    ...
    if @battle.state.effects[:HeavyRain] || @battle.state.effects[:HarshSunlight] || @battle.weather == :STRONGWINDS
      ...
      if !@battle.pbCheckGlobalAbility(:DELTASTREAM) && !@battle.pbCheckGlobalAbility(:TEMPEST) &&
         ![:Winds, :BlowingLeaves, :SwirlingLeaves].include?($game_screen.weather_type) &&
         !((self.pbOwnSide.effects[:Tailwind] > 0 || self.pbOpposingSide.effects[:Tailwind] > 0) && [:MOUNTAIN, :SNOWYMOUNTAIN, :VOLCANICTOP, :SKY].include?(@battle.FE))
    ...
    if !@battle.pbCheckGlobalAbility(:DARKSURGE) && [:DARKNESS1, :DARKNESS2, :DARKNESS3].include?(@battle.FE)
    ...
    if self.ability == :TRACE
      if @effects[:Trace] || onactive
        ...
        for i in 0...4
          if pbIsOpposing?(i) && !@battle.battlers[i].isFainted?
            abilitycheck = true
            battlerability = @battle.battlers[i].ability.nil? ? @battle.battlers[i].backupability : @battle.battlers[i].ability
            abilitycheck = false if (PBStuff::ABILITYBLACKLIST - [:WONDERGUARD]).include?(battlerability) || battlerability.nil?
        ...
        if choices.length == 0
          ...
          battlerability = @battle.battlers[choice].ability.nil? ? @battle.battlers[choice].backupability : @battle.battlers[choice].ability
          self.changeAbility(battlerability)
    ...
    if self.ability == :ELECTRICSURGE && onactive &&
       ((!Rejuv && @battle.canChangeFE?(:ELECTERRAIN)) || @battle.canChangeFE?([:ELECTERRAIN, :DRAGONSDEN])) &&
    ...
    elsif self.ability == :GRASSYSURGE && onactive &&
          ((!Rejuv && @battle.canChangeFE?(:GRASSY)) || @battle.canChangeFE?([:GRASSY, :DRAGONSDEN])) &&
    ...
    elsif self.ability == :MISTYSURGE && onactive &&
          ((!Rejuv && @battle.canChangeFE?(:MISTY)) || @battle.canChangeFE?([:MISTY, :CORROSIVEMIST, :DRAGONSDEN])) &&
    ...
    elsif self.ability == :PSYCHICSURGE && onactive &&
          ((!Rejuv && @battle.canChangeFE?(:PSYTERRAIN)) || @battle.canChangeFE?([:PSYTERRAIN, :DRAGONSDEN])) &&
      ...
      if @battle.FE == :DARKNESS3
        ...
        @battle.setField(:DARKNESS2, add_on: false, growth: true)
        @battle.pbDisplay(_INTL("{1}'s Dark Surge deepened the darkness!", pbThis))
      elsif @battle.FE == :DARKNESS2
        @battle.setField(:DARKNESS3, add_on: false, growth: true)
        @battle.pbDisplay(_INTL("{1}'s Dark Surge deepened the darkness!", pbThis))
    ...
    if ability == :SNOWWARNING && onactive && @battle.weather != :HAIL
      if @battle.state.effects[:HeavyRain]
        ...
        @battle.pbDisplay(_INTL("You're too deep to notice the weather!"))
      elsif [:VOLCANIC, :VOLCANICTOP, :INFERNAL].include?(@battle.FE)
        ...
        @battle.weather = :HAIL
        @battle.weatherduration = 5
        @battle.weatherduration = 8 if self.hasWorkingItem(:ICYROCK) || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION, :SKY].include?(@battle.FE)
        @battle.weatherduration = -1 if $game_switches[:Gen_5_Weather] == true && !@battle.isOnline?
    ...
    # weather forms: castform and cherrim
    for i in 0...4
      @battle.battlers[i].pbCheckWeatherForm
    end
    ...
    if self.ability == :MIMICRY && onactive
      ...
      camotype = protype
      if camotype && !self.hasType?(camotype) || (!self.type2.nil? && self.type1 != self.type2)
        self.type1 = camotype
        self.type2 = nil
    ...
    if self.ability == :FOREWARN && onactive
      ...
      if moves.length > 0
        ...
        # AI CHANGES
        warnedMove = PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(move), self.pokemon)
    ...
    if self.ability == :QUARKDRIVE && onactive
      boostStat = self.getProtoDriveStat
  ...
  def pbEffectsOnDealingDamage(move, user, target, damage)
    ...
    if target.damagestate.calcdamage > 0 && !target.damagestate.substitute
      if target.ability != :SHIELDDUST || target.moldbroken
        if (user.hasWorkingItem(:KINGSROCK) || user.hasWorkingItem(:RAZORFANG)) && !move.canFlinch? && !(user.ability == :SHEERFORCE && move.effect > 0)
          sample = (@battle.FE == :RAINBOW || user.ability == :SERENEGRACE) ? 5 : 10
          if @battle.pbRandom(sample) == 0
            target.effects[:Flinch] = true
          end
        elsif user.ability == :STENCH && !move.canFlinch?
          sample = [:WASTELAND, :MURKWATERSURFACE, :BACKALLEY, :CITY].include?(@battle.FE) ? 5 : 10
          if @battle.pbRandom(sample) == 0
            target.effects[:Flinch] = true
          end
        end
    ...
    if target.damagestate.calcdamage > 0 && !target.damagestate.substitute && !move.zmove && move.contactMove? && user.ability != :LONGREACH
      eschance = 3
      eschance = 6 if @battle.FE == :CORRUPTED
      if user.ability == :POISONTOUCH && target.pbCanPoison?(false) && (target.ability != :SHIELDDUST || target.moldbroken) && @battle.pbRandom(10) < eschance
      ...
      if !user.hasWorkingItem(:PROTECTIVEPADS)
        ...
        # Effect Spore
        if !user.hasType?(:GRASS) && user.ability != :OVERCOAT && target.ability == :EFFECTSPORE && !user.hasWorkingItem(:SAFETYGOGGLES) && @battle.pbRandom(10) < eschance
        ...
        if target.ability == :FLAMEBODY && user.pbCanBurn?(false) && @battle.FE != :FROZENDIMENSION && @battle.pbRandom(10) < 3
          user.pbBurn(target)
        ...
        if target.ability == :POISONPOINT && user.pbCanPoison?(false) && @battle.pbRandom(10) < eschance
          user.pbPoison(target)
          @battle.pbDisplay(_INTL("{1}'s {2} poisoned {3}!", target.pbThis, getAbilityName(target.ability), user.pbThis(true)))
        end
        eschance = 3
        eschance = 6 if @battle.FE == :SHORTCIRCUIT || (Rejuv && @battle.FE == :ELECTERRAIN)
        if target.ability == :STATIC && user.pbCanParalyze?(false) && @battle.pbRandom(10) < eschance
          user.pbParalyze(target)
          @battle.pbDisplay(_INTL("{1}'s {2} paralyzed {3}! It may be unable to move!", target.pbThis, getAbilityName(target.ability), user.pbThis(true)))
        end
        ...
        if target.ability == :GOOEY
          if user.ability == :CONTRARY
            if [:SWAMP, :MURKWATERSURFACE].include?(@battle.FE)
          ...
          elsif [:SWAMP, :MURKWATERSURFACE].include?(@battle.FE)
            user.pbReduceStat(PBStats::SPEED, 2, statmessage: false, ignoreContrary: true)
        ...
        if target.ability == :MUMMY && !user.isFainted?
          userability = user.ability.nil? ? user.backupability : user.ability
          if !(PBStuff::FIXEDABILITIES + [:MUMMY]).include?(userability)
            @battle.pbDisplay(_INTL("{1} was mummified by {2}!", user.pbThis, target.pbThis(true)))
            user.changeAbility(:MUMMY)
          end
        end
        if target.ability == :WANDERINGSPIRIT && !user.isFainted? # && !@user.isBoss
          ...
          if !(PBStuff::ABILITYBLACKLIST + [:WANDERINGSPIRIT]).include?(userability) && !userability.nil?
            @battle.pbDisplay(_INTL("{1} swapped its {2} Ability with the attacker!", target.pbThis, getAbilityName(target.ability)))
            user.changeAbility(target.ability)
    ...
    if target.damagestate.calcdamage > 0
      ...
      if !target.damagestate.substitute
        if (target.ability == :CURSEDBODY && @battle.FE != :HOLY && (@battle.pbRandom(10) < 3 || (target.isFainted? && @battle.FE == :HAUNTED))) || target.crested == :BEHEEYEM
          if user.effects[:Disable] <= 0 && move.pp > 0 && !user.isFainted?
            ...
            if target.crested == :BEHEEYEM
              @battle.pbDisplay(_INTL("{1} disabled {2}!", target.pbThis, user.pbThis(true)))
            else
        ...
        if target.ability == :GULPMISSILE && target.species == :CRAMORANT && !user.isFainted? && target.form != 0
          ...
          if user.ability != :MAGICGUARD && !(user.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
            if @battle.FE == :UNDERWATER
              mult = PBTypes.twoTypeEff(:WATER, user.type1, user.type2).multiplier
              user.pbReduceHP((user.totalhp / 4.0 * mult).floor)
            else
              user.pbReduceHP((user.totalhp / 4.0).floor)
            end
          end
        ...
        if target.ability == :WEAKARMOR && move.pbIsPhysical?(user, movetypes[0])
        ...
        if target.ability == :COTTONDOWN
          ...
          for i in @battle.battlers
            ...
            statdrop = 1
            statdrop = 2 if [:BEWITCHED, :GRASSY].include?(@battle.FE)
        ...
        unless [:UNNERVE, :ASONECHILLING, :ASONEGRIM].intersect?([user.ability, user.pbPartner.ability])

          if target.hasWorkingItem(:JABOCABERRY, true) && !user.isFainted? && move.pbIsPhysical?(user, movetypes[0])
          ...
          if target.hasWorkingItem(:ROWAPBERRY, true) && !user.isFainted? && move.pbIsSpecial?(user, movetypes[0])
            @battle.pbCommonAnimation("Nom", target)
            @battle.scene.pbDamageAnimation(user, 0)
            user.pbReduceHP((user.totalhp / 8.0).floor)
            @battle.pbDisplay(_INTL("{1} was hurt by the {2}!", user.pbThis, getItemName(target.item)))
  ...
  def pbBerryRecoverAmount
    ...
    case self.item
      ...
      when :ENIGMABERRY then healing = (self.totalhp / 4.0).floor if self.damagestate.typemod.superEffective?
  ...
  def pbUseBerry(berry = nil, special = false)
    ...
    case berry
      ...
      when :ENIGMABERRY then healing = (self.totalhp / 4.0).floor if self.damagestate.typemod.superEffective? || special
      when :BERRYJUICE  then healing = 20 if self.hp <= (self.totalhp / 2.0).floor || special
      when :FIGYBERRY, :WIKIBERRY, :MAGOBERRY, :AGUAVBERRY, :IAPAPABERRY
        healing = (self.totalhp / (Gen <= 7 ? 2.0 : 3.0)).floor if self.hp <= health_threshold || special
        confu_berry = true
      when :CHERIBERRY
      ...
      when :LIECHIBERRY, :GANLONBERRY, :SALACBERRY, :PETAYABERRY, :APICOTBERRY, :STARFBERRY
        ...
        case berry
          ...
          when :GANLONBERRY then chosen_stat = PBStats::DEFENSE
          when :PETAYABERRY then chosen_stat = PBStats::SPATK
          when :APICOTBERRY then chosen_stat = PBStats::SPDEF
          when :SALACBERRY then chosen_stat = PBStats::SPEED
          when :STARFBERRY
            stats = []
            for i in 1..5
              stats.push(i) if pbCanIncreaseStatStage?(i)
            end
        ...
        return unless self.pbCanIncreaseStatStage?(chosen_stat)
      when :LANSATBERRY # unique berry, doing the processing now
        if @effects[:FocusEnergy] < 3 && (self.hp <= health_threshold || special)
          ...
          @effects[:FocusEnergy] = 3 if @effects[:FocusEnergy] > 3
        end
      when :LEPPABERRY # unique berry, doing the processing now
        zero_pp = @pokemon.moves.any? { |move| move.move && move.pp == 0 }
        if zero_pp || special
          healing = 0
      ...
      when :MICLEBERRY
        if self.hp <= health_threshold || special
          healing = 0
          message = _INTL("{1} boosted the accuracy of its next move using its {2}!", pbThis, itemname)
    ...
    if healing > 0 # this berry is ACTUALLY a healing berry
      ...
      case berry
        when :FIGYBERRY then flavor_text = "spicy"
        when :WIKIBERRY then flavor_text = "dry"
        when :MAGOBERRY then flavor_text = "sweet"
        when :AGUAVBERRY then flavor_text = "bitter"
        when :IAPAPABERRY then flavor_text = "sour"
      end
    ...
    case pbTarget(move)
      when :SingleNonUser
        if target >= 0
          ...
          if !pbIsOpposing?(targetBattler.index)
            if !move.pbAddTarget(targets, targetBattler, self) && move.move != :INSTRUCT
              move.pbAddTarget(targets, pbOpposing2, self) if !move.pbAddTarget(targets, pbOpposing1, self)
            end
          else
      ...
      when :SingleOpposing
        if target >= 0
          targetBattler = @battle.battlers[target]
          if !pbIsOpposing?(targetBattler.index)
            if !move.pbAddTarget(targets, targetBattler, self)
              move.pbAddTarget(targets, pbOpposing2, self) if !move.pbAddTarget(targets, pbOpposing1, self)
            end
          else
            move.pbAddTarget(targets, targetBattler.pbPartner, self) if !move.pbAddTarget(targets, targetBattler, self)
      ...
      when :AllNonUsers
        for i in 0...4 # not ordered by priority
          move.pbAddTarget(targets, @battle.battlers[i], self) if i != @index
        end
      when :DragonDarts
        if target >= 0
          targetBattler = @battle.battlers[target]
          if !pbIsOpposing?(targetBattler.index)
            ...
            move.pbAddTarget(targets, targetBattler.pbPartner, self)
          end
        else
          pbRandomTarget(targets, move)
        end
      ...
      when :AllBattlers # perish song, rototiller, flower shield
        move.pbAddTarget(targets, pbOpposing1, self)
        move.pbAddTarget(targets, pbOpposing2, self)
        move.pbAddTarget(targets, pbPartner, self)
        move.pbAddTarget(targets, self, self)
      else # :User, :UserSide, :BothSides, :OpposingSide
        move.pbAddTarget(targets, self, self)
    end
  end
  ...
  def pbTarget(move)
    ...
    elsif move.function == 0x167 && @effects[:HealBlock] > 0 # Pollen Puff when Heal Blocked
  ...
  def pbChangeTarget(basemove, user, targets)
    ...
    originalTarget = targets[0] # used to confirm if a redirection message needs to be displayed, inly applied to single target moves
    unless (basemove.function == 0x179 || basemove.function == 0xCE) || user.ability == :STALWART || user.ability == :PROPELLERTAIL ||
        ![:SingleNonUser, :SingleOpposing, :RandomOpposing, :OppositeOpposing, :DragonDarts].include?(targetchoices)
      for i in priority # use Pokémon earliest in priority
        next if i.index == user.index || i.isFainted?

        if i.ability == :LIGHTNINGROD && !i.moldbroken && movetypes.include?(:ELECTRIC)
          target = i
          changeeffect = 1
          break
        end
      ...
      for i in priority # use Pokémon earliest in priority
        ...
        if i.ability == :STORMDRAIN && !i.moldbroken && movetypes.include?(:WATER)
          target = i
          changeeffect = 2
          break
        end
    ...
    if changeeffect > -1
      targets = [target] if changeeffect > -1
      if originalTarget != target || targetchoices == :DragonDarts # a redirection involving Dragon Darts always has an effect so always displays a message
        if changeeffect == 1
          @battle.pbDisplay(_INTL("{1}'s Lightning Rod took the move!", changetarget.pbThis))
        elsif changeeffect == 2
          @battle.pbDisplay(_INTL("{1}'s Storm Drain took the move!", changetarget.pbThis))
        end
      end
    end
    return targets
  ...
  def pbFutureSightUserPlusMove()
    ...
    if @effects[:FutureSightMove] == :DOOMDESIRE
      move = PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(:DOOMDUMMY), moveuser.pokemon)
    elsif @effects[:FutureSightMove] == :FUTURESIGHT
      move = PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(:FUTUREDUMMY), moveuser.pokemon)
    elsif @effects[:FutureSightMove] == :HEX
      move = PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(:HEXDUMMY), moveuser.pokemon)
    end
  ...
  def pbObedienceCheck?(choice)
    ...
    if @battle.pbOwnedByPlayer?(@index) && @battle.internalbattle
      ...
      move = choice[2]
      disobedient = false
      a = ((@level + badgelevel) * @battle.pbRandom(256) / 256.0).floor
  ...
  def pbSuccessCheck(basemove, user, target, flags, accuracy = true)
    ...
    if user == target && targetchoices != :AllBattlers # the only way a move like that is going to fail is due to inherent failure conditions so check those otherwise success
      if !basemove.pbCanAffectTarget(user, target)
        return :MoveFailCat2
      elsif !basemove.pbEffectValid(user, target)
        return :MoveFailCat3
      else
        return :StatusSuccess
      end
    end
    unless target.effects[:LockOn] > 0 && target.effects[:LockOnPos] == user.index
      invulmiss = false
      invulmove = $cache.moves[target.effects[:TwoTurnAttack]].function if target.effects[:TwoTurnAttack] != 0 rescue nil
      case invulmove
        when 0xC9 # Fly
          invulmiss = true unless PBStuff::AIRHITMOVES.include?(basemove.move) || basemove.move == :WHIRLWIND
        when 0xCA # Dig
          invulmiss = true unless basemove.function == 0x76 || basemove.function == 0x95
        when 0xCB # Dive
          invulmiss = true unless basemove.function == 0x75 || basemove.function == 0xD0
        when 0xCD # Shadow Force
          invulmiss = true
        when 0xCE, 0xCC # Sky Drop, Bounce
          invulmiss = true unless PBStuff::AIRHITMOVES.include?(basemove.move)
      end
      if target.effects[:SkyDrop]
        invulmiss = true unless PBStuff::AIRHITMOVES.include?(basemove.move) || user.effects[:SkyDroppee] == target
      end
      invulmiss = false if basemove.function == 0x10D && !user.hasType?(:GHOST) # non ghost curse is not self targeted but should not get stopped by the nominal target being semi invulnerable
      invulmiss = false if basemove.move == :TOXIC && user.hasType?(:POISON)
      invulmiss = false if user.ability == :NOGUARD || target.ability == :NOGUARD
      invulmiss = false if user.ability == :FAIRYAURA && @battle.FE == :FAIRYTALE
      return :Invulmiss if invulmiss
    end
    # TODO: "Before Protect" applies to Counter/Mirror Coat
    if @battle.FE == :PSYTERRAIN && !target.isAirborne? && target.pbPartner != user && target != user
    ...
    if !basemove.zmove # Z-Moves handle protection stuff elsewhere
      ...
      if target.pbOwnSide.effects[:WideGuard] && (targetchoices == :AllOpposing || targetchoices == :AllNonUsers) && basemove.canProtect? && !unseenfist
        return :WideGuard
      end
      if target.pbOwnSide.effects[:QuickGuard] && basemove.priorityCheck(user) > 0 && basemove.canProtect? && !unseenfist
        return :QuickGuard
      end
      if target.pbOwnSide.effects[:CraftyShield] && basemove.basedamage == 0 && targetchoices != :AllBattlers
        return :CraftyShield
      end
      if target.pbOwnSide.effects[:MatBlock] && (basemove.pbIsPhysical?(user, basemove.type) || basemove.pbIsSpecial?(user, basemove.type)) && basemove.canProtect? && !unseenfist
        return :MatBlock
      end
      # Protect / King's Shield / Obstruct / Spiky Shield / Baneful Bunker
      if !unseenfist && basemove.canProtect? && basemove.function != 0x116 &&
         ((target.effects[:Protect] == :KingsShield && (basemove.basedamage > 0 || [:FAIRYTALE, :CHESS].include?(@battle.FE))) || target.effects[:Protect] == :Protect ||
         (target.effects[:Protect] == :Obstruct && (basemove.basedamage > 0 || [:DIMENSIONAL, :CHESS].include?(@battle.FE))) || target.effects[:Protect] == :SpikyShield || target.effects[:Protect] == :BanefulBunker)
        return target.effects[:Protect]
      end
    end
    if basemove.canMagicCoat? && !user.effects[:MagicBounced]
      if target.effects[:MagicCoat]
        return :MagicCoated
      end
      if target.ability == :MAGICBOUNCE && !target.moldbroken
        return :MagicBounced
      end
    end
    typeImmunity = basemove.pbTypeImmunities(user, target)
    if typeImmunity != :Success
      return typeImmunity
    end
    if basemove.basedamage > 0 && ((target.ability == :TELEPATHY && !target.moldbroken) || @battle.FE == :HOLY) && target.index == user.pbPartner.index
      return :Telepathy
    end
    if target.ability == :BULLETPROOF && !target.moldbroken
      if PBStuff::BULLETMOVE.include?(basemove.move)
        return :BulletProof
      end
    end
    if @battle.FE == :ROCKY && (target.effects[:Substitute] > 0 || target.stages[PBStats::DEFENSE] > 0)
      if PBStuff::BULLETMOVE.include?(basemove.move)
        return :RockyBulletProof
      end
    end
    if target.isbossmon
      return :BossImmunity if bossImmunityCheck(basemove, user, target)
    end
    if user.ability == :PRANKSTER && (!(basemove.pbIsPhysical?(user, basemove.type) || basemove.pbIsSpecial?(user, basemove.type)) || (!basemove.zmove && !flags[:instructed] && @battle.choices[user.index][2] != basemove))
      if target.hasType?(:DARK) && @battle.FE != :BEWITCHED
        return :PranksterDark
      end
    end
    if !basemove.pbCanAffectTarget(user, target)
      return :MoveFailCat2
      ...
      unless target.effects[:LockOn] > 0 && target.effects[:LockOnPos] == user.index
        if !basemove.pbAccuracyCheck(user, target) # Includes Counter/Mirror Coat
          if @battle.FE == :MIRROR && basemove.basedamage > 0 && (targetchoices == :SingleNonUser || basemove.move == :MIRRORCOAT) &&
           !basemove.contactMove? && basemove.function != 0x70 && basemove.pbIsSpecial?(user, basemove.type) && target.stages[PBStats::EVASION] > 0
            return :MirrorMissReflect
          end
          user.missAcc = true unless basemove.function == 0x70
          return :AccuracyMiss
        end
      end
    end
    if !basemove.pbEffectValid(user, target)
      return :MoveFailCat3
    end
    return (basemove.pbIsStatus? || user.effects[:TwoTurnAttack] != 0) ? :StatusSuccess : :Success
  end
  ...
  def moveFailureEffects(user, basemove, target, hitflag)
    ...
    case hitflag
      when :PranksterDark, :NoPriority then @battle.pbDisplay(_INTL("{1} wasn't affected!", target.pbThis))
      when :Soundproof then @battle.pbDisplay(_INTL("{1}'s {2} blocks {3}!", target.pbThis, getAbilityName(target.ability), basemove.name))
      when :MatBlock, :Protect, :KingsShield, :SpikyShield, :BanefulBunker, :Obstruct then @battle.pbDisplay(_INTL("{1} protected itself!", target.pbThis))
      when :Levitate then @battle.pbDisplay(_INTL("{1} makes Ground moves miss with {2}!", target.pbThis, getAbilityName(target.ability)))
      when :AirBalloon then @battle.pbDisplay(_INTL("{1}'s Air Balloon makes Ground moves miss!", target.pbThis))
      when :MagnetRise then @battle.pbDisplay(_INTL("{1} makes Ground moves miss with Magnet Rise!", target.pbThis))
      when :Telekinesis then @battle.pbDisplay(_INTL("{1} makes Ground moves miss with Telekinesis!", target.pbThis))
      when :WonderGuard then @battle.pbDisplay(_INTL("{1} avoided damage with Wonder Guard!", target.pbThis))
      when :TypeImmunity then @battle.pbDisplay(_INTL("It doesn't affect\r\n{1}...", target.pbThis(true)))
      ...
      when :MagicCoated then @battle.pbDisplay(_INTL("{1} bounced the {2} back!", target.pbThis, basemove.name))
      when :MagicBounced then @battle.pbDisplay(_INTL("{1} bounced the {2} back!", target.pbThis, basemove.name))
      when :BulletProof then @battle.pbDisplay(_INTL("{1}'s {2} blocked the attack!", target.pbThis, getAbilityName(target.ability)))
      when :RockyBulletProof then @battle.pbDisplay(_INTL("{1} hid behind a rock to dodge the attack!", target.pbThis))
      when :Telepathy then @battle.pbDisplay(_INTL("{1} avoids attacks by its ally Pokémon!", target.pbThis))
      when :MagmaArmor then @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", target.pbThis, getAbilityName(target.ability), basemove.name))
      when :CraftyShield
    ...
    case hitflag
      when :KingsShield
        if basemove.contactMove? && user.ability != :LONGREACH && !user.hasWorkingItem(:PROTECTIVEPADS)
          user.pbReduceStat(PBStats::ATTACK, Gen <= 7 ? 2 : 1)
          user.pbReduceStat(PBStats::SPATK, Gen <= 7 ? 2 : 1) if [:FAIRYTALE, :CHESS, :COLOSSEUM].include?(@battle.FE)
        end
      when :Obstruct
        if basemove.contactMove? && user.ability != :LONGREACH && !user.hasWorkingItem(:PROTECTIVEPADS)
      ...
      when :SpikyShield
        if basemove.contactMove? && user.ability != :LONGREACH && !user.hasWorkingItem(:PROTECTIVEPADS)
          ...
          user.pbReduceHP((user.totalhp / damagedivider).floor)
          @battle.pbDisplay(_INTL("{1}'s Spiky Shield hurt {2}!", target.pbThis, user.pbThis(true)))
        end
      when :BanefulBunker
        if basemove.contactMove? && user.ability != :LONGREACH && !user.hasWorkingItem(:PROTECTIVEPADS) && user.pbCanPoison?(false)
          user.pbPoison(target)
          @battle.pbDisplay(_INTL("{1}'s Baneful Bunker poisoned {2}!", target.pbThis, user.pbThis(true)))
        end
      when :MagicCoated
        target.effects[:MagicBounced] = true
        target.pbUseMoveSimple(basemove.move, -1, user.index, false)
      ...
      when :MagicBounced
        ...
        if @battle.FE == :MIRROR
          if target.pbCanIncreaseStatStage?(PBStats::EVASION)
            ...
            @battle.pbCommonAnimation("StatUp", target, nil)
            @battle.pbDisplay(_INTL("{1}'s Magic Bounce increased its evasion!", target.pbThis, basemove.name))
          end
        end
        target.pbUseMoveSimple(basemove.move, -1, user.index, false)
        target.effects[:MagicBounced] = false
      when :SapSipperAbility, :SapSipperItem
        negator = hitflag == :SapSipperAbility ? getAbilityName(target.ability) : getItemName(target.item)
        if target.pbCanIncreaseStatStage?(PBStats::ATTACK)
          target.pbIncreaseStatBasic(PBStats::ATTACK, 1)
          @battle.pbCommonAnimation("StatUp", target, nil)
          @battle.pbDisplay(_INTL("{1}'s {2} raised its Attack!", target.pbThis, negator))
        else
          @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", target.pbThis, negator, basemove.name))
        end
      when :DrainRod
        if target.pbCanIncreaseStatStage?(PBStats::SPATK)
          if (Rejuv && @battle.FE == :SHORTCIRCUIT) && target.ability == :LIGHTNINGROD
            ...
            arrStatTexts = [
              _INTL("{1}'s {2} raised its Special Attack!", target.pbThis, getAbilityName(target.ability)),
              _INTL("{1}'s {2} sharply raised its Special Attack!", target.pbThis, getAbilityName(target.ability)),
              _INTL("{1}'s {2} drastically raised its Special Attack!", target.pbThis, getAbilityName(target.ability))
            ]
            statboost = statboosts[PBStuff::SHORTCIRCUITROLLS.find_index(damageroll)]
            if statboost != 0
              target.pbIncreaseStatBasic(PBStats::SPATK, statboost)
              @battle.pbCommonAnimation("StatUp", target, nil)
              @battle.pbDisplay(arrStatTexts[statboost - 1])
            else
              @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", target.pbThis, getAbilityName(target.ability), user.name))
            end
          else
            target.pbIncreaseStatBasic(PBStats::SPATK, 1)
            @battle.pbCommonAnimation("StatUp", target, nil)
            @battle.pbDisplay(_INTL("{1}'s {2} raised its Special Attack!", target.pbThis, getAbilityName(target.ability)))
          end
        else
          @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", target.pbThis, getAbilityName(target.ability), user.name))
        end
      when :MotorDriveAbility, :MotorDriveItem
        negator = hitflag == :MotorDriveAbility ? getAbilityName(target.ability) : getItemName(target.item)
        if target.pbCanIncreaseStatStage?(PBStats::SPEED)
          if @battle.FE == :FACTORY
            target.pbIncreaseStatBasic(PBStats::SPEED, 2)
            @battle.pbCommonAnimation("StatUp", target, nil)
            @battle.pbDisplay(_INTL("{1}'s {2} sharply raised its Speed!", target.pbThis, negator))
          elsif Rejuv && @battle.FE == :SHORTCIRCUIT
            damageroll = @battle.field.getRoll(maximize_roll: @battle.state.effects[:ELECTERRAIN] > 0)
            statboosts = [1, 2, 0, 1, 3]
            arrStatTexts = [
              _INTL("{1}'s {2} raised its Speed!", target.pbThis, negator),
              _INTL("{1}'s {2} sharply raised its Speed!", target.pbThis, negator),
              _INTL("{1}'s {2} drastically raised its Speed!", target.pbThis, negator)
            ]
            statboost = statboosts[PBStuff::SHORTCIRCUITROLLS.find_index(damageroll)]
            if statboost != 0
              target.pbIncreaseStatBasic(PBStats::SPEED, statboost)
              @battle.pbCommonAnimation("StatUp", target, nil)
              @battle.pbDisplay(arrStatTexts[statboost - 1])
            else
              @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", target.pbThis, negator, user.name))
            end
          else
            target.pbIncreaseStatBasic(PBStats::SPEED, 1)
            @battle.pbCommonAnimation("StatUp", target, nil)
            @battle.pbDisplay(_INTL("{1}'s {2} raised its Speed!", target.pbThis, negator))
          end
        else
          @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", target.pbThis, negator, user.name))
        end
      when :HpAbsorbAbility, :HpAbsorbItem, :HpAbsorbField
        negator = getAbilityName(target.ability) if hitflag == :HpAbsorbAbility
        negator = getItemName(target.item) if hitflag == :HpAbsorbItem
        negator = "unquenchable thirst" if hitflag == :HpAbsorbField
        if (Rejuv && @battle.FE == :SHORTCIRCUIT) && target.ability == :VOLTABSORB
          damageroll = @battle.field.getRoll(maximize_roll: (@battle.state.effects[:ELECTERRAIN] > 0))
          if target.pbRecoverHP(((target.totalhp / 4.0) * damageroll).floor, true) > 0
            @battle.pbDisplay(_INTL("{1}'s {2} restored its HP!", target.pbThis, negator))
          else
            @battle.pbDisplay(_INTL("{1}'s {2} made {3} useless!", target.pbThis, negator, user.name))
          end
        elsif target.pbRecoverHP((target.totalhp / 4.0).floor, true) > 0
          @battle.pbDisplay(_INTL("{1}'s {2} restored its HP!", target.pbThis, negator))
        else
          @battle.pbDisplay(_INTL("{1}'s {2} made {3} useless!", target.pbThis, negator, user.name))
        end
      when :FlashFireAbility, :FlashFireItem
        negator = hitflag == :FlashFireAbility ? getAbilityName(target.ability) : getItemName(target.item)
        if !target.effects[:FlashFire]
          target.effects[:FlashFire] = true
          @battle.pbDisplay(_INTL("{1}'s {2} activated!", target.pbThis, negator))
        else
          @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", target.pbThis, negator, user.name))
        end
    end
  ...
  def moveCrashDamage(user, basemove)
    ...
    if [0x10B, 0x506].include?(basemove.function) # Hi Jump Kick, Jump Kick, Axe Kick
      @battle.pbDisplay(_INTL("{1} kept going and crashed!", user.pbThis))
      damage = [1, (user.totalhp / 2.0).floor].max
      damage = 0 if user.ability == :MAGICGUARD || (self.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      if damage > 0
        @battle.scene.pbDamageAnimation(user, 0)
        user.pbReduceHP(damage)
      end
    ...
    damage = 0
    # Rocky Field Crash
    if @battle.FE == :ROCKY && basemove.contactMove? && basemove.pbIsPhysical?(user, basemove.pbType) && user.ability != :ROCKHEAD
      @battle.pbDisplay(_INTL("{1} hit a rock instead!", user.pbThis))
      crashdivider = user.ability == :GORILLATACTICS ? 4.0 : 8.0
      damage = [1, (user.totalhp / crashdivider).floor].max
    # Mirror Arena Crash
    elsif @battle.FE == :MIRROR && basemove.contactMove? && basemove.pbIsPhysical?(user, basemove.pbType)
  ...
  def multiHitInterrupt(user, targets, basemove, hitflag, moveTypes)
    ...
    for target in targets
      ...
      if target.isbossmon && target.shieldCount > 0
        target.pbFaint unless target.shieldsBrokenThisTurn[user.index] > 0 || (target&.sosDetails[:playerMons] || target&.sosDetails[:playerParty])
      end
    end
    ...
    return false if hitflag.intersect?([:Success, :StatusSuccess])
    # Previous hit failed for reason other than type immunity
    return true if !hasTypeImmunityResult(hitflag)
    # next roll is getting predicted but not locked in yet, rolls only progress if the hit actually goes through
    @battle.phase = :tempTypeCheck
    predictedTypes = [basemove.pbType(user), *basemove.getSecondaryType(user)]
    @battle.phase = :attackPhase
    # Previous hit had same types
    return true if moveTypes == predictedTypes
    return false
  end

  def hasTypeImmunityResult(hitflag)
    ret = false
    hitflag.each do |result|
      ret = true if isTypeImmunityResult(result)
    end
    return ret
  end

  def isTypeImmunityResult(result)
    raise sprintf("Result %s should be processed earlier", result) if [:Success, :StatusSuccess].include?(result)

    typeImmunityResults = [
      :SapSipperAbility, :SapSipperItem, :DrainRod, :MotorDriveAbility, :MotorDriveItem, :HpAbsorbAbility,
      :HpAbsorbItem, :HpAbsorbField, :FlashFireAbility, :FlashFireItem, :MagmaArmor, :Levitate, :AirBalloon, :MagnetRise, :Telekinesis,
      :WonderGuard, :TypeImmunity, :PranksterDark,
    ]
    return true if typeImmunityResults.include?(result)

    otherKnownResults = [
      :MoveFailCat2, :MoveFailCat3, :MagicCoated, :MagicBounced, :Invulmiss, :NoPriority,
      :WideGuard, :QuickGuard, :CraftyShield, :MatBlock, :Protect, :KingsShield, :SpikyShield, :BanefulBunker, :Obstruct,
      :BulletProof, :RockyBulletProof, :BossImmunity, :MirrorMissReflect, :AccuracyMiss,
      :Soundproof, :Telepathy,
    ]
    return false if otherKnownResults.include?(result)

    # If you encounter this error please add the symbol to the correct one of the above arrays
    raise sprintf("Unknown result %s", result)
  end
  ...
  def pbTryUseMove(choice, basemove, flags = { passedtrying: false, instructed: false })
    ...
    if @battle.state.effects[:Gravity] != 0 && basemove.unusableInGravity? && !(basemove.move == :MAGNETRISE && @battle.FE == :DEEPEARTH)
      @battle.pbDisplay(_INTL("{1} can't use {2} under intense gravity!", pbThis, basemove.name))
      @battle.ai.addMoveToMemory(self, basemove)
      return false
    end
    if @effects[:Taunt] > 0 && basemove.betterCategory(self, basemove.type) == :status && !choice[2].zmove
      @battle.pbDisplay(_INTL("{1} can't use {2} after the taunt!", pbThis, basemove.name))
      @battle.ai.addMoveToMemory(self, basemove)
      return false
    end
    if @effects[:HealBlock] > 0 && (basemove.isHealingMove? || basemove.isDrainingMove?) && !choice[2].zmove
    ...
    if basemove.hasFlag?(:heavymove) && !flags[:instructed] && basemove.move == @lastMoveUsed && basemove.move != @battle.struggle.move && !choice[2].zmove
      @battle.pbDisplay(_INTL("{1} can't use {2} twice in a row!", pbThis, basemove.name))
      return false
    end
    ...
    if @effects[:Illusion] && choice[2].zmove
      # Animation should go here
      @effects[:Illusion] = nil
      @battle.scene.pbChangePokemon(self, self.pokemon)
  ...
  def applyPressure(basemove, user, targets)
    for target in targets
      if target.ability == :PRESSURE && user.pbIsOpposing?(target.index)
        ...
        user.pbReducePP(pressuredmove) # Reduce PP
        if [:DIMENSIONAL, :DEEPEARTH].include?(@battle.FE)
          user.pbReducePP(pressuredmove)
        end
      end
    end
  end
  ...
  def pbOnStartUse(user, targets, basemove, flags)
    types = [basemove.pbType(user), *basemove.getSecondaryType(user)]
    if basemove.basedamage > 0
      if @battle.pbWeather == :SUNNYDAY && @battle.state.effects[:HarshSunlight] && types[0] == :WATER
        @battle.pbDisplay(_INTL("The Water-type attack evaporated in the harsh sunlight!"))
        return false
      end
      if @battle.pbWeather == :RAINDANCE && @battle.state.effects[:HeavyRain] && types[0] == :FIRE
    ...
    if basemove.moveFieldBoost == 0
      @battle.pbDisplay(_INTL(basemove.moveFieldMessage))
      return false
    end
    if basemove.typeFieldBoost(types[0], user, nil) == 0
      @battle.pbDisplay(_INTL(basemove.typeFieldMessage(basemove.pbType(user))))
      return false
    end
    ...
    unless !targetsOpp
      if basemove.priorityCheck(user) > 0 || (user.ability == :PRANKSTER && !basemove.zmove && !flags[:instructed] && @battle.choices[user.index][2] != basemove)
        for i in [user.pbOpposing1, user.pbOpposing2]
          if (i.ability == :DAZZLING || i.ability == :QUEENLYMAJESTY || (@battle.FE == :STARLIGHT && i.ability == :MIRRORARMOR)) && !i.moldbroken
            @battle.pbDisplay(_INTL("{1} cannot use {2}!", user.pbThis, basemove.name))
            return false
          end
        end
      end
  ...
  def pbConfusionDamage
    ...
    damage = confmove.pbReduceHPDamage(self, [self], [damage])
    confmove.pbEffectMessages(self, self)
    confmove.pbOnDamageLost(damage, self, self)
    self.effects[:Tantrum] = true
    pbFaint if self.isFainted?
  end

  def pbResolveMoveEffects(user, basemove, targets, calcdamage, hitflag, hitcount, flags = { totaldamage: 0 })
    ...
    if user.ability != :SHEERFORCE
      targets.each_with_index do |target, i|
        ...
        next unless hitflag[i] == :Success && (target.ability != :SHIELDDUST || target.moldbroken)
    ...
    if user.species == :CRAMORANT && user.ability == :GULPMISSILE && !user.isFainted? && (basemove.move == :SURF || basemove.move == :DIVE) # Surf or Dive
      if user.form == 0
        if [:SWAMP, :WATERSURFACE, :UNDERWATER].include?(@battle.FE)
          user.form = 1 # Gulping Form
        elsif [:ELECTERRAIN, :FACTORY, :SHORTCIRCUIT].include?(@battle.FE)
          user.form = 2 # Gorging Form
        elsif user.hp * 2.0 > user.totalhp
          user.form = 1 # Gulping Form
        else
          user.form = 2 # Gorging Form
        end
    ...
    targets.each_with_index do |target, i|
      ...
      pbEffectsOnDealingDamage(basemove, user, target, calcdamage[i]) unless hitflag[i] == :StatusSuccess # Status moves are not dealing damage
  ...
  def pbUseMoveSimple(moveid, index = -1, target = -1, danced = false, specialZ = false)
    choice = [:move, index, PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(moveid.intern), self.pokemon), target]
    ...
    if @battle.zMove[side][owner] == self.index && choice[2].basedamage > 0 && !danced
      ...
      choice[2] = PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(zmoveID), self.pokemon, choice[2])
    end
  ...
  def pbGetHitNumber(basemove, user)
    targetchoices = pbTarget(basemove)
    numhits = basemove.pbNumHits(user)
    # Ledian and Cinccino crests
    if ![:AllNonUsers, :AllOpposing].include?(targetchoices) && numhits < 2
      case user.crested
        when :LEDIAN
    ...
    if numhits == 1 && !basemove.zmove && (user.ability == :PARENTALBOND || (basemove.contactMove? && user.crested == :TYPHLOSION))
      counter1 = 0
      counter2 = 0
      ...
      if user.ability == :PARENTALBOND
        unless (targetchoices == :AllNonUsers && counter1 != 2) || (targetchoices == :AllOpposing && counter2 != 1)
          user.effects[:ParentalBond] = true
          numhits = 2
        end
      elsif basemove.contactMove? && user.crested == :TYPHLOSION
        unless (targetchoices == :AllNonUsers && counter1 != 2) || (targetchoices == :AllOpposing && counter2 != 1)
          user.effects[:TyphBond] = true
          numhits = 2
        end
      end
    end
  ...
  def pbUseMove(choice, flags = { danced: false, totaldamage: 0, specialusage: false, specialZ: false })
    ...
    targets = []
    user = self
    moldbroken = [:MOLDBREAKER, :TERAVOLT, :TURBOBLAZE].include?(user.ability) || [0x166, 0x176, 0x200].include?(basemove.function) # Solgaluna/crozma signatures
    corroded = user.ability == :CORROSION
    for i in 0..3
      @battle.battlers[i].moldbroken = moldbroken
      @battle.battlers[i].corroded = corroded
    end
    ...
    pbZStatus(@battle, basemove.move, user) if choice[2].zmove && choice[2].category == :status && PBStuff::TYPETOZCRYSTAL[basemove.type] == user.item
    ...
    if [:PROTEAN, :LIBERO].include?(user.ability) && basemove.move != :STRUGGLE
      if !user.hasType?(moveTypes[0]) || (!user.type2.nil? && user.type1 != user.type2)
        user.type1 = moveTypes[0]
        user.type2 = nil
        @battle.pbDisplay(_INTL("{1} had its type changed to {3}!", user.pbThis, getAbilityName(user.ability), moveTypes[0].capitalize))
      end
    end
    ...
    if targets.length == 0
      ...
      hitflag = []
      targets.each_with_index do |target, i|
        hitflag[i] = pbSuccessCheck(basemove, user, target, flags)
      end

      if targetchoices == :DragonDarts && targets.length > 1
        targets.each_with_index do |target, i|
          unless [:Success, :MirrorMissReflect].include?(hitflag[i])
            targets.delete_at(i)
            hitflag.delete_at(i)
            break if targets.length == 1
          end
        end
      end
      ...
      numhits = pbGetHitNumber(basemove, user)
      killtracker = []
      calcdamage = []
      revanishtracker = []
      realnumhits = 0
      for hitcount in 0...numhits
        feedbackMessages = []
        if hitcount > 0
          fieldrollbackup = @battle.field.fieldroll.clone
          @battle.field.fieldroll = nil
          if multiHitInterrupt(user, targets, basemove, hitflag, moveTypes)
            @battle.field.fieldroll = fieldrollbackup
          ...
          moveTypes = [basemove.pbType(user), *basemove.getSecondaryType(user)]
          targets.each_with_index do |target, i|
            hitflag[i] = pbSuccessCheck(basemove, user, target, flags, basemove.checkAccuracyEachHit(user))
          end
          user.missAcc = false # if the move gets to this point atleast one hit has connected or at the very least not missed due to accuracy, no blunder policy
        end
        ...
        targets.each_with_index do |target, i|
          if target.effects[:Illusion] && hitflag[i] != :AccuracyMiss && hitflag[i] != pbSuccessCheck(basemove, user, target.effects[:Illusion], flags, false)
            target.effects[:IllusionDisclosure] = true
          end
          next unless [:Success, :StatusSuccess].include?(hitflag[i])
          calcdamage[i] = basemove.pbCalcDamage(user, target, hitcount, feedbackMessages)
          revanish = false
          if target.vanished && (basemove.function != 0x10D || user.hasType?(:GHOST)) && (basemove.function != 0xCE || target.effects[:TwoTurnAttack] != 0)
            revanish = true
        ...
        if [:Success, :StatusSuccess].intersect?(hitflag)
          ...
          animTarget = targets == [user] ? nil : targets[0]
          ...
          if !hitflag.include?(:StatusSuccess) # we may skip damage dealing and associated effects and messages if succesful move was a non-damage dealing move
            calcdamage = basemove.pbReduceHPDamage(user, targets, calcdamage) if calcdamage.max > 0
            targets.each_with_index do |target, i|
              if hitflag[i] == :Success # succesful damaging move
                flags[:totaldamage] += calcdamage[i]
                basemove.pbOnDamageLost(calcdamage[i], user, target)
                basemove.pbEffectMessages(user, target, false, targets.length > 1)
              end
            end
          end
        ...
        elsif hitcount == 0 # total failure conditions don't apply if a previous hit in a multihit connected
          ...
          nonTantrumFails = [:WideGuard, :QuickGuard, :CraftyShield, :MatBlock, :Protect, :KingsShield, :SpikyShield, :BanefulBunker, :Obstruct, :MoveFailCat3]
          ...
          # when spitup fails the stockpile effect is used up anyway, make sure of this:
          if basemove.function == 0x113 # Spit Up
            if user.effects[:StockpileDef] > 0
              user.pbReduceStat(PBStats::DEFENSE, user.effects[:StockpileDef], abilitymessage: false, statdropper: user)
            end
            if user.effects[:StockpileSpDef] > 0
              user.pbReduceStat(PBStats::SPDEF, user.effects[:StockpileSpDef], abilitymessage: false, statdropper: user)
            end
      ...
      if realnumhits > 1 && ![:Success, :StatusSuccess].intersect?(hitflag)
        # accounting for instances in which a multihit move fails on a later hit but connected atleast once, this grabs the last succesful set of hitflags
        hitflag = hitflagBackup if hitflagBackup
      end
      if numhits > 1
        for target in targets
          if target.damagestate.typemod.superEffective?
            @battle.pbDisplay(targets.length > 1 ? _INTL("It's super effective on {1}!", target.pbThis) : _INTL("It's super effective!"))
          elsif target.damagestate.typemod.resisted?
            @battle.pbDisplay(targets.length > 1 ? _INTL("It's not very effective on {1}...", target.pbThis) : _INTL("It's not very effective..."))
          end
        end
        if realnumhits == 1
      ...
      unless user.isFainted?
        basemove.pbRecoilDamage(user, flags[:totaldamage], basemove.recoil, hitflag, realnumhits)
        flags[:UserFaintCause] = flags[:UserFaintCause] + targets
      end
      if user.isFainted?
        user.pbFaint
      ...
      targets.each_with_index do |target, i|
        if target.isFainted?
          ...
          if !user.isFainted? && target.pbIsOpposing?(user.index)
            if grudge
      ...
      basemove.pbEffectAfterMove(user, targets, hitflag) if !user.isFainted? && [:Success, :StatusSuccess].intersect?(hitflag)
      targets.each_with_index do |target, i|
        next if target.isFainted?
        next unless [:Success, :StatusSuccess].include?(hitflag[i])
        ...
        if target.status == :FROZEN && (basemove.pbType(user) == :FIRE || basemove.function == 0x0A) && hitflag[i] == :Success
          target.pbCureStatus
        end
      end
      targets.each_with_index do |target, i|
        ...
        # Sheer Force affected abilities
        unless user.ability == :SHEERFORCE && basemove.effect > 0
          # Berserk
          if target.ability == :BERSERK && preMoveHP[i] > (target.totalhp / 2.0).floor && target.hp <= (target.totalhp / 2.0).floor && hitflag[i] == :Success
            if !pbTooHigh?(PBStats::SPATK)
              initial = target.stages[PBStats::SPATK]
              statgain = Rejuv && @battle.FE == :DRAGONSDEN ? 2 : 1
              target.pbIncreaseStatBasic(PBStats::SPATK, statgain)
              @battle.pbCommonAnimation("StatUp", target, nil)
              actualstatgain = target.stages[PBStats::SPATK] - initial
              str = actualstatgain == 2 ? "sharply " : ""
              @battle.pbDisplay(_INTL("{1}'s Berserk #{str}boosted its Special Attack!", target.pbThis))
            end
          end
          # Color Change
          movetype = basemove.pbType(user)
          if target.ability == :COLORCHANGE && hitflag[i] == :Success && ![:SHADOW, :QMARKS].include?(movetype) && !target.hasType?(movetype) && !target.damagestate.substitute
        ...
        # Magician
        if user.ability == :MAGICIAN && hitflag[i] == :Success && !target.damagestate.substitute && target.item
        ...
        # More Sheer Force affected things
        unless user.ability == :SHEERFORCE && basemove.effect > 0
          if hitflag[i] == :Success && !target.damagestate.substitute
            # kee and maranga berry
            unless [:UNNERVE, :ASONECHILLING, :ASONEGRIM].intersect?([user.ability, user.pbPartner.ability])
              if target.hasWorkingItem(:KEEBERRY) && move.pbIsPhysical?(user, movetypes[0])
                if target.pbCanIncreaseStatStage?(PBStats::DEFENSE)
                  @battle.pbCommonAnimation("Nom", target)
              ...
              if target.hasWorkingItem(:MARANGABERRY) && move.pbIsSpecial?(user, movetypes[0])
                if target.pbCanIncreaseStatStage?(PBStats::SPDEF)
                  @battle.pbCommonAnimation("Nom", target)
            ...
            # Red Card
            if target.hasWorkingItem(:REDCARD) && !target.isbossmon && !user.isFainted?
              choices = []
            ...
            unless target.userSwitch
              # Eject Button
              if target.hasWorkingItem(:EJECTBUTTON)
                if @battle.pbCanChooseNonActive?(target.index) && !@battle.pbAllFainted?(@battle.pbParty(target.pbOppositeOpposing.index))
              ...
              elsif preMoveHP[i] > (target.totalhp / 2.0).floor && (target.hp + target.pbBerryRecoverAmount) <= (target.totalhp / 2.0).floor && !target.isbossmon
                if @battle.FE == :COLOSSEUM
                  ...
                  if [:EMERGENCYEXIT, :WIMPOUT].include?(target.ability) &&
                    ...
                    blockwimp = user.isbossmon && (!@battle.pbCanChooseNonActive?(target.index) && !@battle.pbAllFainted?(@battle.pbParty(target.pbOppositeOpposing.index)))
                    if !blockwimp
      ...
      if user.effects[:TwoTurnAttack] != 0 && basemove.immediate
      ...
      for target in killtracker
        if user.hp > 0 && target.hp <= 0 && flags[:totaldamage] > 0
          ...
          if !@battle.pbAllFainted?(@battle.pbParty(target.index))
            ...
            if user.ability == :BATTLEBOND && !@battle.permanenteffects[user.pokemon][:BattleBond]
              if !(user.pbTooHigh?(PBStats::ATTACK) && user.pbTooHigh?(PBStats::SPATK) && user.pbTooHigh?(PBStats::SPEED))
                @battle.pbCommonAnimation("StatUp", self, nil)
                if !user.pbTooHigh?(PBStats::ATTACK)
                  user.pbIncreaseStatBasic(PBStats::ATTACK, 1)
                ...
                @battle.permanenteffects[user.pokemon][:BattleBond] = true
              end
            end
          end
        end
      ...
      if [:Success, :StatusSuccess].intersect?(hitflag)
        ...
        # Metronome item
        if user.hasWorkingItem(:METRONOME) || @battle.FE == :CONCERT4
      ...
      if @battle.FE == :DIMENSIONAL
        ...
        if combust != 0
          if [:DIG, :DIVE, :FLY, :BOUNCE].include?(basemove.move) && user.effects[:TwoTurnAttack] != 0
      ...
      unless user.ability == :SHEERFORCE && basemove.effect > 0
        ...
        # Life Orb
        if user.hasWorkingItem(:LIFEORB) && hitflag.include?(:Success) && user.ability != :MAGICGUARD && !(user.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      ...
      # Eject Pack
      for i in @battle.battlers
        if i.hasWorkingItem(:EJECTPACK) && i.statLowered && !i.userSwitch
          if !i.isFainted? && @battle.pbCanChooseNonActive?(i.index) && !@battle.pbAllFainted?(@battle.pbParty(i.pbOppositeOpposing.index))
            @battle.pbDisplay(_INTL("#{i.pbThis}'s Eject Pack activates!"))
            i.pbDisposeItem(false, false)
            if @battle.FE == :COLOSSEUM
              @battle.pbDisplay(_INTL("But #{i.pbThis} cannot retreat."))
            else
              i.userSwitch = true
            end
          end
      ...
      # Pickpocket
      unless (user.ability == :SHEERFORCE && basemove.effect > 0) || user.item.nil? || user.ability == :STICKYHOLD || battle.pbIsUnlosableItem(user, user.item)
        ...
        targets.each_with_index do |target, i|
          if target.ability == :PICKPOCKET
            ...
            next if !basemove.contactMove? || user.ability == :LONGREACH
            next if target.damagestate.substitute
            next if user.effects[:Substitute] == 0
            next if battle.pbIsUnlosableItem(target, user.item)
            next if user.ability != :STICKYHOLD
            ...
            if wildBattle && target.pokemon.itemInitial.nil? && user.pokemon.itemInitial == target.item # In a wild battle
      ...
      # Switch moves
      if user.userSwitch && @battle.battlers.any?{ |mon| mon != user && mon.userSwitch }
        @battle.scene.pbUnVanishSprite(user)
        user.userSwitch = false # if an pivot move directly causes a different pokemon on the field to pivot out then the pivot move user will not switch
      end
      unless @battle.pbAllFainted?(@battle.party1) || @battle.pbAllFainted?(@battle.party2) # if either side is fully beaten no switching needs to happen, the battle ends
        for i in @battle.battlers
          if i.forcedSwitch
            ...
            i.userSwitch = false # just in case
            @battle.neutralizingGasDisable(i.index) if i.ability == :NEUTRALIZINGGAS
            party = @battle.pbParty(i.index)
            choices = []
            for j in 0...party.length
              choices.push(j) if (i.isFainted? || j != i.pokemonIndex) && (pbPartner.isFainted? || j != i.pbPartner.pokemonIndex) &&
                                party[j] && !party[j].isEgg? && party[j].hp > 0 &&
                                @battle.pbCanSwitchLax?(i.index, j, false)
            end
            newpoke = @battle.sample(choices)
            i.vanished = false
            i.pbResetForm
            @battle.pbReplace(i.index, newpoke)
          ...
          if i.userSwitch
            ...
            for j in priority
              ...
              # if Pursuit was chosen (regardless of target)
              if !j.hasMovedThisRound? && @battle.pbChoseMoveFunctionCode?(j.index, 0x88) && !j.effects[:Pursuit]
                if i.vanished
                  i.vanished = false
                  @battle.pbCommonAnimation("Fade in", i, nil)
                end
                @battle.pbPursuitInterrupt(j, i)
              end
  ...
  def getSpecialStat(unaware = false, which_is_higher: false, attacker: false, crit: false)
    ...
    # Mold Breaker can't ignore Flower Gift defense boost on Glitch Field
    spdefmult *= 1.5 if self.flowerGiftActive?
    spdefmult *= 1.5 if self.pbPartner.flowerGiftActive?
  ...
  def pbProcessTurn(choice)
    ...
    if $game_variables[:LuckShinies] != 0
      for battler in @battle.battlers
        if @battle.pbIsWild? && battler.index.odd? && !battler.isFainted? && battler.pokemon.isShiny? && !battler.isbossmon && !battler.issossmon
  ...
  def getCrowdBuff(isDebuff: false)
    choices = isDebuff ? PBStuff::CROWDDEBUFFS : PBStuff::CROWDBUFFS
    return choices[@roll]
  end
  ...
  def cheer
    if self.hp > 0
      ...
      messageroll = [
        self.name + " received the crowd's cheers! Full power!",
        self.name + " received the crowd's cheers! Unyielding as the earth!",
        self.name + " received the crowd's cheers! Full speed ahead!",
      ][self.roll]
      effectroll = self.getCrowdBuff(isDebuff: self.ability == :CONTRARY)
      eval(effectroll)
  ...
  def boo
    if self.hp > 0
      ...
      statmod = 1
      if self.ability != :CONTRARY
        if !self.pbTooLow?(statindex)
          @battle.pbCommonAnimation("StatDown", self, nil)
          self.pbReduceStatBasic(statindex, statmod)
          @battle.pbDisplay(_INTL("The Crowd's booing lowered its {1}!", pbGetStatName(statindex)))
        end
      else
        if !self.pbTooHigh?(statindex)
          @battle.pbCommonAnimation("StatUp", self, nil)
          self.pbIncreaseStatBasic(statindex, statmod)
          @battle.pbDisplay(_INTL("The Crowd's booing raised its {1}!", pbGetStatName(statindex)))
        end
      end
    end
  end
  ...
  def releaseSkyDroppee
    ...
    if droprelease != nil
      ...
      @battle.pbDisplay(_INTL("{1} was freed from the Sky Drop!", droprelease.pbThis))
    end
  end
end
